...
# ─────────────────────────────────────────────────────────────────────────────
# OpenMarketTerminal — multi-stage, multi-arch Docker build
#
...
#
#   docker build -t openmarketterminal/terminal:4.0.2 .
#       → auto-detects host arch via BuildKit
#
#   docker buildx build --platform linux/amd64,linux/arm64 \
#       -t openmarketterminal/terminal:4.0.2 --push .
#       → cross-build both archs in one go
#
# Pins (must match openmarketterminal-qt/CMakeLists.txt + release.yml):
...
#     openmarketterminal/terminal:4.0.2
# ─────────────────────────────────────────────────────────────────────────────
...
# ── Build OpenMarketTerminal ───────────────────────────────────────────────────
WORKDIR /src
COPY openmarketterminal-qt/ ./openmarketterminal-qt/

WORKDIR /src/openmarketterminal-qt
RUN . /etc/build.env \
    ...
    && strip build/OpenMarketTerminal
...
WORKDIR /app
COPY --from=builder /src/openmarketterminal-qt/build/OpenMarketTerminal ./OpenMarketTerminal
COPY --from=builder /src/openmarketterminal-qt/scripts              ./scripts
COPY --from=builder /src/openmarketterminal-qt/resources            ./resources
...
COPY --from=builder /src/openmarketterminal-qt/build/_deps/qgeoview-build/lib/ /usr/local/lib/
RUN ldconfig \
    && chmod +x ./OpenMarketTerminal
...
RUN { \
      ...
      echo 'exec /app/OpenMarketTerminal "$@"'; \
    } > /usr/local/bin/openmarketterminal-entrypoint.sh \
    && chmod +x /usr/local/bin/openmarketterminal-entrypoint.sh

ENTRYPOINT ["/usr/local/bin/openmarketterminal-entrypoint.sh"]
