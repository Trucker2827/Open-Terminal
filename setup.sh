...
echo ""
echo "================================================"
echo "  OpenMarketTerminal v4.0.1 — Setup"
...
QT_INSTALL_ROOT="${OPENMARKETTERMINAL_QT_ROOT:-$SCRIPT_DIR/.qt}"
...
if [ -n "${Qt6_DIR:-}" ] && [ -f "$Qt6_DIR/lib/cmake/Qt6/Qt6Config.cmake" ]; then
    ...
    # Modules required to compile OpenMarketTerminal (match find_package COMPONENTS)
...
APP_DIR="$SCRIPT_DIR/openmarketterminal-qt"
[ -d "$APP_DIR" ] || fail "openmarketterminal-qt directory not found. Ensure you cloned the full repository."
cd "$APP_DIR"
...
# ── Done ────────────────────────────────────────────────────
BIN="$APP_DIR/build/$PRESET/OpenMarketTerminal"
[ "$PLATFORM" = "macos" ] && BIN="$APP_DIR/build/$PRESET/OpenMarketTerminal.app/Contents/MacOS/OpenMarketTerminal"
