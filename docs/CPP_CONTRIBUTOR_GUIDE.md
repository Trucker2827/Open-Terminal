...
This guide covers C++ development for Open Terminal — 40+ screens, core infrastructure, trading engine, and Qt6 UI.
...
Debug variants: `win-debug`, `linux-debug`, `macos-debug`. See `openmarketterminal-qt/CMakePresets.json`.

**Emergency override only** — pass `-DOPENMARKETTERMINAL_ALLOW_QT_DRIFT=ON` to bypass the Qt `EXACT` check for local experiments. Never commit work built this way; CI rejects it.
...
### 3. Use UI Widgets
- `openmarketterminal::ui::Card` for panel containers
- `openmarketterminal::ui::DataTable` for tabular data
- `openmarketterminal::ui::SearchBar` for search inputs
- `openmarketterminal::ui::StatusBadge` for status indicators
- Apply stylesheets via `openmarketterminal::ui::apply_global_stylesheet()`
- Follow `DESIGN_SYSTEM.md` (Obsidian) — color tokens, spacing, typography

### 4. Namespace Convention
```cpp
namespace openmarketterminal {         // top-level
namespace openmarketterminal::ui {     // UI components
namespace openmarketterminal::auth {   // auth module
// etc.
}
```
...
**Questions?** Open an issue on [GitHub](https://github.com/your-org/open-terminal).
