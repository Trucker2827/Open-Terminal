...
Open Terminal is an open-source native C++20/Qt6 financial intelligence platform with 50+ screens, embedded Python analytics, and 100+ data connectors. This guide is the canonical **how-to** for contributors — build, architecture, conventions.
...
All day-to-day development uses presets from `openmarketterminal-qt/CMakePresets.json`.

```bash
cd openmarketterminal-qt
...
```bash
.\build\win-release\Open Terminal.exe                                           # Windows
./build/linux-release/Open Terminal                                             # Linux
./build/macos-release/Open Terminal.app/Contents/MacOS/Open Terminal          # macOS
```
...
```
openmarketterminalTerminal/
├── openmarketterminal-qt/                 # Main C++ application (all development happens here)
│   ├── src/                    # C++ source
...
### C++ source layout (`openmarketterminal-qt/src/`)
...
### Python scripts (`openmarketterminal-qt/scripts/`)
...
See `openmarketterminal-qt/CLAUDE.md` for the detailed service catalog and architecture rules.
...
- **Namespaces:** `namespace openmarketterminal {}`, `namespace openmarketterminal::ui {}`; never `using namespace std;`
...
Follow `openmarketterminal-qt/DESIGN_SYSTEM.md` (Obsidian design system). Do not introduce ad-hoc color values, typography, or spacing — use the tokens.

### Mandatory architecture rules (summary)

Full detail in `openmarketterminal-qt/CLAUDE.md`. These are **non-negotiable** — PRs that violate them will be sent back:
...
- **D1–D5.** Streaming data flows through the DataHub — never spawn Python directly from a screen. See `openmarketterminal-qt/DATAHUB_ARCHITECTURE.md`.
...
| Channel      | Link                                                                          |
|--------------|-------------------------------------------------------------------------------|
| Issues       | [GitHub Issues](https://github.com/your-org/open-terminal/issues) |
| Discussions  | [GitHub Discussions](https://github.com/your-org/open-terminal/discussions) |
...
**Repository:** https://github.com/your-org/open-terminal
