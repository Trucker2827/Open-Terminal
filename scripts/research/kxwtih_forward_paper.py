#!/usr/bin/env python3
"""Hash-locked, paper-only KXWTIH hourly trial.

One family, one 10-cent executable-edge floor, hold to settlement. Never
reads 15m, gold, or silver reports. Never calls an order API. Creation of
the state file is a clean freeze: older calibrator snapshots cannot enter.
Does not share a hash or ledger with the KXGOLDH trial.
"""
...
POLICY = {
    ...
    "selection": "one highest-edge KXWTIH strike per event",
...
def initial_state(now_ms):
    return {
        "schema": 1,
        "event": "kxwtih_forward_paper_state",
        "policy": POLICY,
...
def atomic_write(path, payload):
    ...
    fd, temporary = tempfile.mkstemp(prefix=".kxwtih-forward-", dir=directory, text=True)
    try:
...
def outcomes_from_hourly_state(state=None, settlements=None):
    """Map KXWTIH ticker -> YES won. Gold/silver rows never enter."""
    out = {}
    if state is None:
...
def _apply_report(state, report, now_ms):
    ...
    if now_ms - generated > POLICY["max_report_age_ms"]:
        return
    for found in best_by_event(family_predictions(report)):
...
def run_postmortem(state_path):
    ...
    trades = postmortem_trades(completed, load_family_quotes())
    winners = [row for row in trades if row["won"]]
    losers = [row for row in trades if not row["won"]]
    return {
        "event": "kxwtih_forward_postmortem",
        "read_only": True,
...
def default_state_path():
    return common.evidence_path("kxwtih-forward-paper.json")
