...
complete -c openterminalcli -n "__fish_seen_subcommand_from daemon service launchd" -a "status health readiness ready safety trade-gate logs log audit security jobs job list history hist runs failures failed stats metrics add show run enable disable remove repair clear-running clear-failures clear-fails ack acknowledge monitors monitor notify notification ai paper paper-strategy install start stop restart uninstall remove rm plist path --replace --force --start --dry-run --all --limit --lines --name --every-sec --interval-sec --timeout-sec --tick-stale-sec --market-stale-sec --model-stale-sec --train-stale-sec --min-samples --min-fresh-sources --disabled --target --workflow --selector --cell --strategy --symbols --max-iters --provider --level --title --message --job"
