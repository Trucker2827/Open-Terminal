...
# Repo root derived from this script's location (it lives in openmarketterminal-qt/).
REPO="$(cd "$(dirname "$0")/.." && pwd)"
TIDY=/opt/homebrew/opt/llvm@18/bin/clang-tidy
CFG=/tmp/openmarketterminal-tidy18.yaml
SDK=$(xcrun --show-sdk-path)

cd "$REPO/openmarketterminal-qt" || exit 2

# Extract the target's compile flags from a unity blob entry (identical for all
# files in the OpenMarketTerminal target). Drop compiler, -c/-o, and source file.
FLAGS=()
...
if [ "$#" -gt 0 ]; then
  ...
  while IFS= read -r line; do [ -n "$line" ] && FILES+=("$line"); done < <(
    git -C "$REPO" diff --name-only --diff-filter=d \
      | grep -E 'openmarketterminal-qt/src/.*\.cpp$' | grep -vE '/moc_|/qrc_' \
      | sed 's#openmarketterminal-qt/##'
  )
fi
