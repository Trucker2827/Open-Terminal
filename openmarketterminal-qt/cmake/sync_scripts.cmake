...
# CMake's top-level copy_directory_if_different can miss brand-new immediate
# subdirectories on some local builds. Keep the fast path above, then make a
# cheap pass that guarantees newly added script packages are present.
file(GLOB _src_top_level_dirs LIST_DIRECTORIES true "${SRC_DIR}/*")
foreach(_src_child IN LISTS _src_top_level_dirs)
    if(IS_DIRECTORY "${_src_child}")
        get_filename_component(_child_name "${_src_child}" NAME)
        set(_dst_child "${DST_DIR}/${_child_name}")
        execute_process(
            COMMAND "${CMAKE_COMMAND}" -E copy_directory "${_src_child}" "${_dst_child}"
            RESULT_VARIABLE _child_copy_rc
        )
        if(NOT _child_copy_rc EQUAL 0)
            message(FATAL_ERROR "sync_scripts: copying '${_child_name}' failed (${_child_copy_rc})")
        endif()
    endif()
endforeach()
