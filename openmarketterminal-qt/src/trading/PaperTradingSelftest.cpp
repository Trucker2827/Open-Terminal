...
int run_paper_trading_selftest() {
    ...
    // assertable; OPRA option symbols exercise the option-leverage paths.
    const QString name = QStringLiteral("__selftest_paper__");
    const QString pf_exchange = QStringLiteral("OPRA");
    if (auto existing = pt_find_portfolio(name, pf_exchange))
        pt_delete_portfolio(existing->id);
    PtPortfolio pf = pt_create_portfolio(name, /*balance=*/1'000'000.0, /*currency=*/QStringLiteral("USD"),
    ...
    const QString NASDAQ = QStringLiteral("NASDAQ");
    const QString OPRA = QStringLiteral("OPRA");

    // ── 1. Long equity marks to market with the right sign + magnitude ──────
    {
        const QString sym = QStringLiteral("AAPL");
        open_market(sym, QStringLiteral("buy"), 10, 100.0, QStringLiteral("MIS"), NASDAQ);
        ...
        open_market(sym, QStringLiteral("sell"), 10, 110.0, QStringLiteral("MIS"), NASDAQ);
        check("long: exits on opposite fill", !position_for(pid, sym).has_value());
    }

    // ── 2. Short option opens flat (P&L 0 at entry) ─────────────────────────
    const QString opt = QStringLiteral("AAPL24DEC25000150CE");
    {
        open_market(opt, QStringLiteral("sell"), 50, 200.0, QStringLiteral("NRML"), OPRA);
    ...
    // ── 4. Square-off a short: it actually exits AND books the right realized ──
    {
        ...
        open_market(opt, QStringLiteral("buy"), 50, 150.0, QStringLiteral("NRML"), OPRA);
    ...
    {
        const QString sym = QStringLiteral("MSFT");
        open_market(sym, QStringLiteral("buy"), 5, 1000.0, QStringLiteral("MIS"), NASDAQ);
        open_market(sym, QStringLiteral("sell"), 5, 1100.0, QStringLiteral("MIS"), NASDAQ);
    ...
    {
        const QString pe = QStringLiteral("AAPL24DEC25000145PE");
        const QString ce = QStringLiteral("AAPL24DEC25000155CE");
        open_market(pe, QStringLiteral("buy"), 50, 120.0, QStringLiteral("NRML"), OPRA);
        open_market(ce, QStringLiteral("sell"), 50, 90.0, QStringLiteral("NRML"), OPRA);
    ...
    {
        const QString sym = QStringLiteral("NVDA");
        const double bal0 = pt_get_portfolio(pid).balance;
        open_market(sym, QStringLiteral("buy"), 10, 500.0, QStringLiteral("CNC"), NASDAQ); // CNC 1x -> locks 5000
        ...
        open_market(sym, QStringLiteral("sell"), 10, 520.0, QStringLiteral("CNC"), NASDAQ); // realize +200, release
