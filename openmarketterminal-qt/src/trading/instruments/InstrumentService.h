...
///   InstrumentService::instance().refresh("ibkr", creds);
///   InstrumentService::instance().refresh("alpaca", creds);
///
///   // Anywhere else (instant, in-memory):
///   auto token = InstrumentService::instance().instrument_token("AAPL", "NASDAQ", "ibkr");
///   auto sym   = InstrumentService::instance().to_brsymbol("AAPL", "NASDAQ", "ibkr");
///
