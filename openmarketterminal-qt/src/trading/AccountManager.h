...
class AccountManager : public QObject {
  ...
  public:
    ...
    /// First-run convenience: if the user has NO accounts, create one ready-to-use
    /// $100k paper account (so the Equity screen populates out of the box, like
    /// Crypto's default paper portfolio). Guarded by a one-time settings flag, so
    /// it is NOT re-created after the user deliberately removes their accounts.
    /// Call once at startup, after reload_from_db().
    void ensure_default_paper_account();
