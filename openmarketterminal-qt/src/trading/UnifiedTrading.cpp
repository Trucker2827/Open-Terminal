...
UnifiedOrderResponse UnifiedTrading::place_order(const QString& account_id, UnifiedOrder order) {
    ...
    // Fail-SAFE routing: an unrecognized or blank trading_mode must NEVER reach
    // a live broker with real money. Only explicitly recognized broker modes
    // (live + broker test endpoints) take the broker path; everything else —
    // "paper", "backtest", empty, or any unexpected value — routes to the
    // internal paper simulator. (Was: anything != "paper" fell through to live.)
    const QString& mode = account.trading_mode;
    const bool broker_route = (mode == "live" || mode == "sandbox" || mode == "demo");
    if (!broker_route && mode != "paper" && mode != "backtest") {
        LOG_WARN("UnifiedTrading",
                 QString("Unrecognized trading_mode '%1' for account %2 — routing to PAPER, "
                         "refusing to place a live order.").arg(mode, account_id));
    }
    UnifiedOrderResponse resp = broker_route
