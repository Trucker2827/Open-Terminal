...
namespace openmarketterminal::trading::hyperliquid {

class HyperliquidVenue : public QObject,
                         public openmarketterminal::services::alpha_arena::IExchangeVenue {
    Q_OBJECT
  public:
    ...
    void place_order(const openmarketterminal::services::alpha_arena::OrderRequest& req,
                     std::function<void(openmarketterminal::services::alpha_arena::OrderAck)> ack_cb) override;
    void cancel_order(const QString& venue_order_id) override;
    double last_mark(const QString& coin) const override;
    void on_fill(std::function<void(openmarketterminal::services::alpha_arena::FillEvent)> cb) override {
        fill_cb_ = std::move(cb);
    }
    void on_funding(std::function<void(openmarketterminal::services::alpha_arena::FundingEvent)> cb) override {
        funding_cb_ = std::move(cb);
    }
    void on_liquidation(std::function<void(openmarketterminal::services::alpha_arena::LiquidationEvent)> cb) override {
        liq_cb_ = std::move(cb);
    }
  ...
  private:
    ...
    std::function<void(openmarketterminal::services::alpha_arena::FillEvent)> fill_cb_;
    std::function<void(openmarketterminal::services::alpha_arena::FundingEvent)> funding_cb_;
    std::function<void(openmarketterminal::services::alpha_arena::LiquidationEvent)> liq_cb_;
