...
using openmarketterminal::services::alpha_arena::FillEvent;
using openmarketterminal::services::alpha_arena::FundingEvent;
using openmarketterminal::services::alpha_arena::IExchangeVenue;
using openmarketterminal::services::alpha_arena::LiquidationEvent;
...
void HyperliquidVenue::reconcile_tick() {
    ...
    client_->info(body, [self, comp_id](Result<QJsonDocument> r) {
        ...
        auto agents = openmarketterminal::services::alpha_arena::AlphaArenaRepo::instance()
                          .agents_for(comp_id);
        if (agents.is_err()) return;
        for (const auto& agent : agents.value()) {
            auto local = openmarketterminal::services::alpha_arena::AlphaArenaRepo::instance()
            ...
            for (const auto& p : local.value()) {
                ...
                if (std::fabs(r_qty - p.qty) > 1e-9) {
                    ...
                    openmarketterminal::services::alpha_arena::AlphaArenaRepo::instance()
