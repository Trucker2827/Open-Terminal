...
void AccountManager::ensure_default_paper_account() {
    // One-time, flag-guarded. Runs after reload_from_db()/migration, so by here a
    // returning user already has their accounts; only a brand-new user (or one who
    // never added a broker) hits the seed.
    auto flag = SettingsRepository::instance().get("default_paper_account.seeded");
    ...
    {
        QMutexLocker lock(&mutex_);
        empty = accounts_.isEmpty();
    }

    if (empty) {
        // Paper accounts are tied to a broker profile (for exchange/currency/symbol
        // format) but trade only against the local paper portfolio — no broker
        // credentials or network needed. Prefer Alpaca (US equities); fall back to
        // whatever is registered.
        auto& reg = BrokerRegistry::instance();
        QString broker_id = reg.has("alpaca") ? QStringLiteral("alpaca") : QString();
        if (broker_id.isEmpty()) {
            const auto brokers = reg.list_brokers();
            if (!brokers.isEmpty())
                broker_id = brokers.first();
        }
        if (!broker_id.isEmpty()) {
            auto acct = add_account(broker_id, QStringLiteral("Paper Trading"));
            if (!acct.account_id.isEmpty())
                LOG_INFO("AccountManager",
                         QString("Seeded first-run default paper account %1 ($100k, broker %2)")
                             .arg(acct.account_id, broker_id));
        } else {
            LOG_WARN("AccountManager", "No broker registered — cannot seed a default paper account");
        }
    }

    SettingsRepository::instance().set("default_paper_account.seeded", "done", "system");
}
