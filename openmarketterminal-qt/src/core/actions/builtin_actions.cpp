...
namespace openmarketterminal::actions {
...
Result<void> handler_screenshot(const CommandContext& ctx) {
    ...
    const QString path = QFileDialog::getSaveFileName(
        ...
        QDir::homePath() + "/openmarketterminal_" +
...
Result<void> handler_replay_tour(const CommandContext& ctx) {
    ...
    openmarketterminal::screens::OnboardingTour::reset_seen();
    QWidget* parent = nullptr;
    if (ctx.focused_frame)
        parent = ctx.focused_frame;
    openmarketterminal::screens::OnboardingTour::show_for(parent);
    return Result<void>::ok();
}

Result<void> handler_open_symbol(const CommandContext& ctx) {
    const QString symbol = ctx.args.value(QStringLiteral("symbol")).toString().trimmed().toUpper();
    if (symbol.isEmpty())
        return Result<void>::err("Missing 'symbol' argument");
    if (!ctx.focused_frame || ctx.focused_frame->is_locked())
        return Result<void>::ok();

    ctx.focused_frame->dock_router()->navigate(QStringLiteral("equity_research"));
    EventBus::instance().publish(QStringLiteral("equity_research.load_symbol"),
                                 {{QStringLiteral("symbol"), symbol}});
    LOG_INFO(kBuiltinTag, QString("Opened symbol in Equity Research: %1").arg(symbol));
    return Result<void>::ok();
}

Result<void> open_screen(const QString& screen_id, const CommandContext& ctx) {
    if (!ctx.focused_frame || ctx.focused_frame->is_locked())
        return Result<void>::ok();
    if (screen_id.isEmpty())
        return Result<void>::err("Missing screen id");

    ctx.focused_frame->dock_router()->navigate(screen_id);
    return Result<void>::ok();
}
...
void register_builtins() {
    ...
    register_one(ActionDef{
        "symbol.open",
        "Open Symbol in Equity Research",
        "Screen",
        {"ticker", "symbol", "quote", "research symbol"},
        QKeySequence(),
        require_unlocked_frame(),
        &handler_open_symbol,
        {ParameterSlot{"symbol", "Ticker symbol", "symbol", true, "symbol", {}}},
    });

    const auto register_screen = [](const QString& screen_id,
                                    const QString& title,
                                    const QStringList& aliases) {
        register_one(ActionDef{
            QStringLiteral("screen.") + screen_id,
            QStringLiteral("Open ") + title,
            QStringLiteral("Screen"),
            aliases,
            QKeySequence(),
            require_unlocked_frame(),
            [screen_id](const CommandContext& ctx) { return open_screen(screen_id, ctx); },
            {},
        });
    };

    // Bloomberg-style terminal mnemonics and plain-language screen launchers.
    // These are intentionally action-backed, not parser-only, so Ctrl+K,
    // the quick command bar, menus, telemetry, and future permission gates
    // all share one route.
    register_screen("dashboard", "Dashboard", {"home", "dash", "dashboard"});
    register_screen("dinero", "Dinero", {"dinero", "din", "dinero network", "dinero chain"});
    ...
    register_screen("dbnomics", "DBnomics", {"fred", "dbnomics", "economic series"});
