...
//
// When OpenMarketTerminal.exe crashes (access violation, stack overflow,
...
LONG WINAPI unhandled_exception_filter(EXCEPTION_POINTERS* ep) {
    ...
    if (file != INVALID_HANDLE_VALUE) {
        ...
        // SECURITY: keep the dump to stacks + thread/module info only. We must
        // NOT capture heap/data segments: this process holds DECRYPTED broker
        // access tokens and crypto wallet private keys in memory, and the
        // diagnostics UI invites users to attach these .dmp files to bug
        // reports. MiniDumpWithDataSegs (global/static data) and
        // MiniDumpWithIndirectlyReferencedMemory (pointed-to heap, incl. those
        // secrets) are therefore deliberately omitted — a stack-only dump still
        // gives a usable crash backtrace without exfiltrating credentials.
...
namespace openmarketterminal::crash {
...
void install() {
    ...
    const QString dir = openmarketterminal::AppPaths::crashdumps();
    QDir().mkpath(dir);
