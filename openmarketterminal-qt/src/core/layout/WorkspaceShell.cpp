...
namespace openmarketterminal::layout {
...
Workspace WorkspaceShell::capture(const QString& name, const QString& kind,
    ...
    const MonitorTopologyKey current = openmarketterminal::MonitorTopology::current_key();
    ...
    for (auto* frame : frames) {
        ...
        if (auto* scr = frame->screen())
            variant.frame_screens.insert(id, openmarketterminal::MonitorTopology::stable_id(scr));
    }
...
int WorkspaceShell::apply(const Workspace& workspace) {
    ...
    int applied = 0;
    auto frames_now = openmarketterminal::WindowRegistry::instance().frames();
    QSet<openmarketterminal::WindowFrame*> bound; // frames already paired with a FrameLayout this call
    ...
    for (const auto& fl : workspace.frames) {
        ...
        openmarketterminal::WindowFrame* target = nullptr;
        bool spawned = false;
        ...
        if (!target) {
            ...
            target = new openmarketterminal::WindowFrame(
                openmarketterminal::WindowFrame::next_window_id(),
                /*parent=*/nullptr,
        ...
        if (variant && spawned) {
            ...
            if (scr_it != variant->frame_screens.constEnd()) {
                if (QScreen* saved_screen =
                        openmarketterminal::MonitorTopology::find_screen_by_stable_id(scr_it.value())) {
