...
#define OPENMARKETTERMINAL_LOG_IMPL(level_enum, level_fn, tag, msg)                                                     \
    do {                                                                                                    \
        if (openmarketterminal::Logger::instance().is_enabled(openmarketterminal::LogLevel::level_enum, tag))                     \
            openmarketterminal::Logger::instance().level_fn(tag, msg);                                                  \
    } while (0)

#define LOG_TRACE(tag, msg) OPENMARKETTERMINAL_LOG_IMPL(Trace, trace, tag, msg)
#define LOG_DEBUG(tag, msg) OPENMARKETTERMINAL_LOG_IMPL(Debug, debug, tag, msg)
#define LOG_INFO(tag, msg)  OPENMARKETTERMINAL_LOG_IMPL(Info,  info,  tag, msg)
#define LOG_WARN(tag, msg)  OPENMARKETTERMINAL_LOG_IMPL(Warn,  warn,  tag, msg)
#define LOG_ERROR(tag, msg) OPENMARKETTERMINAL_LOG_IMPL(Error, error, tag, msg)

// LOG_FATAL writes the line, then aborts in debug builds (P3.20).
#ifndef NDEBUG
#    define LOG_FATAL(tag, msg)                                                                              \
        do {                                                                                                \
            openmarketterminal::Logger::instance().fatal(tag, msg);                                                    \
            openmarketterminal::Logger::instance().flush_and_close();                                                  \
            qFatal("FATAL [%s] %s", qUtf8Printable(QString(tag)), qUtf8Printable(QString(msg)));            \
        } while (0)
#else
#    define LOG_FATAL(tag, msg) OPENMARKETTERMINAL_LOG_IMPL(Fatal, fatal, tag, msg)
#endif
...
namespace openmarketterminal::detail {
...
#define OPENMARKETTERMINAL_LOG_F_IMPL(level_enum, level_fn, tag, fmt, ...)                                              \
    do {                                                                                                    \
        if (openmarketterminal::Logger::instance().is_enabled(openmarketterminal::LogLevel::level_enum, tag))                     \
            openmarketterminal::Logger::instance().level_fn(tag, openmarketterminal::detail::log_fmt(fmt, ##__VA_ARGS__));        \
    } while (0)

#define LOG_TRACE_F(tag, fmt, ...) OPENMARKETTERMINAL_LOG_F_IMPL(Trace, trace, tag, fmt, ##__VA_ARGS__)
#define LOG_DEBUG_F(tag, fmt, ...) OPENMARKETTERMINAL_LOG_F_IMPL(Debug, debug, tag, fmt, ##__VA_ARGS__)
#define LOG_INFO_F(tag, fmt, ...)  OPENMARKETTERMINAL_LOG_F_IMPL(Info,  info,  tag, fmt, ##__VA_ARGS__)
#define LOG_WARN_F(tag, fmt, ...)  OPENMARKETTERMINAL_LOG_F_IMPL(Warn,  warn,  tag, fmt, ##__VA_ARGS__)
#define LOG_ERROR_F(tag, fmt, ...) OPENMARKETTERMINAL_LOG_F_IMPL(Error, error, tag, fmt, ##__VA_ARGS__)
