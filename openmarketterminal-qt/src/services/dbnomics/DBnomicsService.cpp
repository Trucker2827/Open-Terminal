...
void DBnomicsService::fetch_providers() {
    {
        const QVariant cached = openmarketterminal::CacheManager::instance().get("dbnomics:providers");
        if (!cached.isNull()) {
    ...
    openmarketterminal::HttpClient::instance().get(url, [this](openmarketterminal::Result<QJsonDocument> result) {
        if (result.is_err()) {
...
void DBnomicsService::fetch_datasets(const QString& provider_code, int offset) {
    ...
    if (first_page) {
        ...
        const QVariant cached = openmarketterminal::CacheManager::instance().get(ds_key);
        if (!cached.isNull()) {
    ...
    openmarketterminal::HttpClient::instance().get(url, [this, provider_code, offset](openmarketterminal::Result<QJsonDocument> result) {
        if (result.is_err()) {
            LOG_ERROR("DBnomicsService",
...
void DBnomicsService::fetch_series(const QString& provider_code, const QString& dataset_code, const QString& query,
    ...
    if (first_page && no_query) {
        ...
        const QVariant cv = openmarketterminal::CacheManager::instance().get(series_ckey);
        if (!cv.isNull()) {
    ...
    openmarketterminal::HttpClient::instance().get(
        build_url(path), [this, cache_key, offset, no_query, first_page](openmarketterminal::Result<QJsonDocument> result) {
            if (result.is_err()) {
                LOG_ERROR("DBnomicsService",
...
void DBnomicsService::fetch_observations(const QString& provider_code, const QString& dataset_code,
    ...
    openmarketterminal::HttpClient::instance().get(build_url(path), [this, full_id, provider_code, dataset_code,
                                                          series_code](openmarketterminal::Result<QJsonDocument> result) {
        if (result.is_err()) {
        ...
        if (hub_registered_) {
            ...
            openmarketterminal::datahub::DataHub::instance().publish(topic, QVariant::fromValue(dp));
        }
    }, this);
}
...
void DBnomicsService::global_search(const QString& query, int offset) {
    ...
    openmarketterminal::HttpClient::instance().get(build_url(path), [this, offset](openmarketterminal::Result<QJsonDocument> result) {
        if (result.is_err()) {
