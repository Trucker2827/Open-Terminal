...
namespace openmarketterminal::services::options::analytics {

using openmarketterminal::trading::InstrumentType;
...
OptionGreeks combined_greeks(const Strategy& s, const OptionChain& chain) {
    ...
    QHash<QPair<double, openmarketterminal::trading::InstrumentType>, OptionGreeks> by_strike_type;
    ...
    for (const auto& row : chain.rows) {
        if (row.ce_token != 0 && row.ce_greeks.valid) {
            ...
            by_strike_type.insert({row.strike, openmarketterminal::trading::InstrumentType::CE}, row.ce_greeks);
        }
        if (row.pe_token != 0 && row.pe_greeks.valid) {
            by_token.insert(row.pe_token, row.pe_greeks);
            by_strike_type.insert({row.strike, openmarketterminal::trading::InstrumentType::PE}, row.pe_greeks);
        }
    }
