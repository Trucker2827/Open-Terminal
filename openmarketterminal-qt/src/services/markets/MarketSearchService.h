...
class MarketSearchService : public QObject {
  ...
  signals:
    ...
    /// Fires alongside results_ready/search_failed with the source provenance of
    /// the underlying fetch (source, key requirement, timestamp, live/stale,
    /// cache). Non-breaking — consumers opt in to show a ProvenanceBadge.
    void provenance_updated(openmarketterminal::connectors::Provenance provenance);
...
Q_DECLARE_METATYPE(openmarketterminal::services::MarketSearchService::Item)
