...
// LOCAL-FIRST: maritime/AIS data is not pointed at any vendor cloud.
// The base URL comes from settings ("connectors.maritime_url",
// e.g. a self-hosted AIS proxy). When unset, marine_base() is empty so requests
// resolve to a host-less URL that fails locally — no data ever leaves to a
// third party. (A free AISStream.io websocket connector can be wired here later.)
static QString marine_base() {
    auto r = openmarketterminal::SettingsRepository::instance().get(
        QStringLiteral("connectors.maritime_url"));
    QString base = r.is_ok() ? r.value().trimmed() : QString();
    if (base.endsWith('/'))
        base.chop(1);
    return base.isEmpty() ? QString() : base + QStringLiteral("/marine");
}
...
int MaritimeService::max_requests_per_sec() const {
    return 2;  // OpenMarketTerminal marine API — conservative
}
...
void MaritimeService::ensure_registered_with_hub() {
    ...
    openmarketterminal::datahub::TopicPolicy vessel_policy;
    ...
    // History: 5 min TTL.
    openmarketterminal::datahub::TopicPolicy history_policy;
    ...
    openmarketterminal::datahub::TopicPolicy health_policy;
