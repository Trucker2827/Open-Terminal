...
void AgentService::discover_agents() {
    {
        ...
        if (!cv.isNull()) {
            ...
            if (root["agents"].toArray().isEmpty()) {
                openmarketterminal::CacheManager::instance().remove("agents:list");
...
void AgentService::get_system_info() {
    {
        const QVariant cv = openmarketterminal::CacheManager::instance().get("agents:sysinfo");
        if (!cv.isNull()) {
...
void AgentService::list_tools() {
    {
        const QVariant cv = openmarketterminal::CacheManager::instance().get("agents:tools");
        if (!cv.isNull()) {
            const QJsonObject r = QJsonDocument::fromJson(cv.toString().toUtf8()).object();
            AgentToolsInfo info;
...
void AgentService::list_models() {
    {
        const QVariant cv = openmarketterminal::CacheManager::instance().get("agents:models");
        if (!cv.isNull()) {
            const QJsonObject r = QJsonDocument::fromJson(cv.toString().toUtf8()).object();
