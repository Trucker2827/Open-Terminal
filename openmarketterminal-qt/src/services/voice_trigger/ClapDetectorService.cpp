...
void ClapDetectorService::start() {
    ...
    env.insert("OPENMARKETTERMINAL_DATA_DIR", python::PythonSetupManager::instance().install_dir());
    ...
    env.insert("OPENMARKETTERMINAL_CLAP_MODE", mode);
    env.insert("OPENMARKETTERMINAL_CLAP_PEAK_MIN", peak);
    env.insert("OPENMARKETTERMINAL_CLAP_PR_RATIO", ratio);
    env.insert("OPENMARKETTERMINAL_CLAP_MAX_GAP_MS", gap);
    env.insert("OPENMARKETTERMINAL_CLAP_DEBOUNCE_MS", debounce);
    env.insert("OPENMARKETTERMINAL_STT_DEVICE", device);
