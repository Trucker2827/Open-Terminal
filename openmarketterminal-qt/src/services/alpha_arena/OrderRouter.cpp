...
double hitl_notional_threshold() {
    auto r = openmarketterminal::SettingsRepository::instance().get(QStringLiteral("alpha_arena.hitl.notional_usd"));
    bool ok = false;
    if (r.is_ok()) {
...
int hitl_leverage_threshold() {
    auto r = openmarketterminal::SettingsRepository::instance().get(QStringLiteral("alpha_arena.hitl.max_leverage"));
    bool ok = false;
    if (r.is_ok()) {
