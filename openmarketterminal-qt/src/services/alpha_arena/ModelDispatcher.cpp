...
QString tmp_request_dir() {
    QString dir = QStandardPaths::writableLocation(QStandardPaths::TempLocation) +
                  QStringLiteral("/openmarketterminal-aa");
    QDir().mkpath(dir);
    return dir;
}
...
QString new_pipe_name(const QString& comp_id, int seq) {
    return QStringLiteral("openmarketterminal-aa-%1-%2-%3")
        .arg(comp_id.left(8))
        .arg(seq)
...
void ModelDispatcher::dispatch_tick(Tick t,
    ...
    for (const auto& agent : agents_) {
        ...
        openmarketterminal::python::PythonRunner::instance().run(
            ...
            (const openmarketterminal::python::PythonResult& result) {
                if (!self) return;
                ...
                if (!result.success) {
                    ...
                    const QString out_json = openmarketterminal::python::extract_json(result.output);
