...
namespace openmarketterminal::services::asia_markets {
...
void AsiaMarketsService::fetch_endpoints(const QString& script, EndpointsCallback cb) {
    openmarketterminal::python::PythonRunner::instance().run(
        ...
        [cb = std::move(cb), script](const openmarketterminal::python::PythonResult& result) {
            EndpointsResult out;
            if (!result.success) {
            ...
            const QString json_str = openmarketterminal::python::extract_json(result.output);
            if (json_str.isEmpty()) {
...
void AsiaMarketsService::query(const QString& script, const QString& endpoint,
    ...
    openmarketterminal::python::PythonRunner::instance().run(
        script, args,
        [cb = std::move(cb), script, endpoint](const openmarketterminal::python::PythonResult& result) {
            QueryResult out;
            if (!result.success) {
