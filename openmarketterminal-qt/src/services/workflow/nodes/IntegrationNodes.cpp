...
void register_integration_nodes(NodeRegistry& registry) {
    // ── MCP Tool Call ──────────────────────────────────────────────
    // Single node that routes to any internal OpenMarketTerminal MCP tool.
    // "tool" parameter accepts:
    //   - bare tool name (e.g. "get_quote")        → routed to openmarketterminal server
    //   - "serverId__toolName" OpenAI function form → routed via execute_openai_function
    registry.register_type({
        .type_id = "mcp.tool_call",
        .display_name = "MCP Tool",
        .category = "MCP",
        .description = "Call any OpenMarketTerminal internal MCP tool by name",
        .icon_text = "M",
        .accent_color = "#6366f1",
        .version = 2,
            ...
            [](const QJsonObject& params, const QVector<QJsonValue>& inputs,
               std::function<void(bool, QJsonValue, QString)> cb) {
                ...
                PythonRunner::instance().run(
                    "spreadsheet.py", {"--args", args_json}, [cb](const openmarketterminal::python::PythonResult& res) {
                        if (!res.success) {
