...
void register_agent_nodes(NodeRegistry& registry) {
        ...
        .execute =
            [](const QJsonObject& params, const QVector<QJsonValue>& inputs,
               std::function<void(bool, QJsonValue, QString)> cb) {
                ...
                QJsonArray tools;
                if (!agent_id.isEmpty()) {
                    auto res = openmarketterminal::AgentConfigRepository::instance().get(agent_id);
                    if (res.is_ok()) {
                ...
                // Resolve LLM profile
                using openmarketterminal::ai_chat::LlmService;
                using openmarketterminal::LlmProfileRepository;
                using openmarketterminal::ResolvedLlmProfile;
                ...
                guard->conn = QObject::connect(
                    &svc, &openmarketterminal::services::AgentService::agent_result, guard,
                    [guard](const openmarketterminal::services::AgentExecutionResult& result) {
    ...
    // configured LLM which OpenMarketTerminal MCP tool best handles the request, and
        ...
        .execute =
            [](const QJsonObject& params, const QVector<QJsonValue>& inputs,
               std::function<void(bool, QJsonValue, QString)> cb) {
                ...
                QString system_prompt = "You are a tool-selection assistant for the OpenMarketTerminal.\n"
                ...
                (void)QtConcurrent::run([system_prompt, query, cb]() {
                    using openmarketterminal::ai_chat::LlmService;
                    using openmarketterminal::ai_chat::ConversationMessage;
