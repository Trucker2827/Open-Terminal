...
void register_datasource_nodes(NodeRegistry& registry) {
    ...
    openmarketterminal::screens::datasources::register_all_connectors();

    const auto& connectors = openmarketterminal::screens::datasources::ConnectorRegistry::instance().all();
    for (const auto& c : connectors) {
        ...
        for (const auto& f : c.fields) {
            ...
            switch (f.type) {
                case openmarketterminal::screens::datasources::FieldType::Number:
                    p.type = "number";
                    break;
                case openmarketterminal::screens::datasources::FieldType::Checkbox:
                    p.type = "boolean";
                    break;
                case openmarketterminal::screens::datasources::FieldType::Select:
                    p.type = "select";
                    for (const auto& opt : f.options) {
                        p.options.append(opt.value); // use raw value
                    }
                    break;
                case openmarketterminal::screens::datasources::FieldType::File:
                    p.type = "file_managed";
                    break;
                default:
                    p.type = "string";
        ...
        registry.register_type({
            ...
            .display_name = c.name,
            .category = "Data Source: " + openmarketterminal::screens::datasources::category_label(c.category),
            ...
            .execute = [connector_id = c.id, connector_name = c.name](const QJsonObject& params, const QVector<QJsonValue>&,
                ...
                if (!script_name.isEmpty()) {
                    ...
                    PythonRunner::instance().run(script_name, {"--args", args_json}, [cb, connector_name](const PythonResult& res) {
                        ...
                        QString out = openmarketterminal::python::extract_json(res.output).trimmed();
