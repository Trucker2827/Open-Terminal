...
using openmarketterminal::python::extract_json;
using openmarketterminal::python::PythonResult;
using openmarketterminal::python::PythonRunner;
...
static bool gate_or_queue(const QString& account_id, const openmarketterminal::trading::UnifiedOrder& order,
                          const std::function<void(bool, QJsonValue, QString)>& cb) {
    if (!openmarketterminal::trading::ActionCenter::instance().should_queue(account_id, "placeorder"))
        return false;
    const QString pid = openmarketterminal::trading::ActionCenter::instance().queue_order(
        account_id, "placeorder", openmarketterminal::trading::ActionCenter::serialize_unified_order(order));
    QJsonObject out;
...
void wire_market_data_bridges(NodeRegistry& registry) {
    ...
    if (crypto_price_def) {
        crypto_price_def->execute = [](const QJsonObject& params, const QVector<QJsonValue>&,
            ...
            QString symbol = base + "/" + quote; // ccxt unified format: BTC/USD

            (void)QtConcurrent::run([symbol, cb]() {
                auto& svc = trading::ExchangeService::instance();
                const QString exchange = svc.get_exchange();
...
static openmarketterminal::trading::ProductType parse_product(const QString& p) {
    using namespace openmarketterminal::trading;
    QString s = p.toLower();
    if (s == "mis" || s == "intraday")
...
void wire_all_bridges(NodeRegistry& registry) {
    ...
    LOG_INFO("ServiceBridges", "All bridges wired (unwired node types now fail loudly)");
}
