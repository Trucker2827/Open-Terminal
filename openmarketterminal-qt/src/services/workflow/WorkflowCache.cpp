...
QJsonValue WorkflowCache::get(const QString& key) const {
    ...
    const QVariant cv = openmarketterminal::CacheManager::instance().get(kPrefix + key);
    if (cv.isNull()) {
        misses_++;
        return {};
    }
...
bool WorkflowCache::has(const QString& key) const {
    if (!enabled_)
        return false;
    return openmarketterminal::CacheManager::instance().has(kPrefix + key);
}
...
void WorkflowCache::put(const QString& key, const QJsonValue& value, int ttl_ms) {
    ...
    openmarketterminal::CacheManager::instance().put(kPrefix + key, QVariant(serialized), ttl_sec, kCategory);
}

// ── Remove ────────────────────────────────────────────────────────────────────

void WorkflowCache::remove(const QString& key) {
    openmarketterminal::CacheManager::instance().remove(kPrefix + key);
}

void WorkflowCache::invalidate_prefix(const QString& prefix) {
    openmarketterminal::CacheManager::instance().remove_prefix(kPrefix + prefix);
    LOG_DEBUG("WorkflowCache", "Invalidated prefix: " + prefix);
}

void WorkflowCache::clear() {
    openmarketterminal::CacheManager::instance().clear_category(kCategory);
    hits_ = 0;
    misses_ = 0;
...
int WorkflowCache::size() const {
    ...
    return openmarketterminal::CacheManager::instance().entry_count();
}
