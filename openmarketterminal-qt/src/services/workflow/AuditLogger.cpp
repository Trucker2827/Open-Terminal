...
void AuditLogger::ensure_table() {
    auto r = openmarketterminal::Database::instance().exec("CREATE TABLE IF NOT EXISTS workflow_audit_log ("
    ...
    openmarketterminal::Database::instance().exec("CREATE INDEX IF NOT EXISTS idx_audit_ts ON workflow_audit_log(timestamp)");
    openmarketterminal::Database::instance().exec("CREATE INDEX IF NOT EXISTS idx_audit_action ON workflow_audit_log(action)");
    openmarketterminal::Database::instance().exec("CREATE INDEX IF NOT EXISTS idx_audit_wf ON workflow_audit_log(workflow_id)");
}
...
void AuditLogger::log(AuditAction action, const QString& workflow_id, const QString& node_id, const QString& symbol,
    ...
    auto& db = openmarketterminal::Database::instance();
    auto r =
...
Result<QVector<AuditEntry>> AuditLogger::query(const QDateTime& from, const QDateTime& to, AuditAction action_filter,
    ...
    auto r = openmarketterminal::Database::instance().execute(sql, params);
    if (r.is_err())
...
Result<QVector<AuditEntry>> AuditLogger::by_workflow(const QString& workflow_id) const {
    auto r = openmarketterminal::Database::instance().execute(
...
int AuditLogger::count() const {
    auto r = openmarketterminal::Database::instance().execute("SELECT COUNT(*) FROM workflow_audit_log", {});
    if (r.is_err())
        return 0;
    auto& q = r.value();
