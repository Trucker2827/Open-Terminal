...
void WorkflowExecutor::launch_single_node(const QString& node_id) {
    ...
    if (!type_def || !type_def->execute || (type_def && !type_def->available)) {
        NodeExecutionResult nr;
        nr.node_id = node_id;
        nr.success = false;
        if (type_def && !type_def->available) {
            nr.kind = openmarketterminal::OutcomeKind::Unavailable;
            nr.error = type_def->unavailable_reason.isEmpty()
                           ? QStringLiteral("Unavailable: %1 is not available.").arg(nd.type)
                           : type_def->unavailable_reason;
        } else {
            nr.kind = openmarketterminal::OutcomeKind::Failed;
            nr.error = QStringLiteral("Node type '%1' is not implemented. No work was performed.").arg(nd.type);
        }
        LOG_WARN("Executor", QString("Node %1 (%2): %3").arg(node_id, nd.type, nr.error));
        results_.insert(node_id, nr);
        emit node_completed(node_id, nr);
        completed_count_++;

        // Honesty: a non-Ok node aborts the run unless it opted into continue_on_fail.
        if (!nd.continue_on_fail) {
            ExecutionHooks::instance().emit_node_error(workflow_.id, node_id, nr.error);
            stop_requested_ = true;
            if (pending_count_ == 0)
                finish_execution();
            return;
    ...
    if (inputs.isEmpty() && !incoming_edges_.value(node_id).isEmpty()) {
        ...
        if (has_upstream_results) {
            ...
            // success stays true so graph flow/propagation continues (a not-taken
            // branch is not a failure), but kind=Skipped so the UI never shows green.
            NodeExecutionResult nr;
            nr.node_id = node_id;
            nr.success = true;
            nr.kind = openmarketterminal::OutcomeKind::Skipped;
...
void WorkflowExecutor::on_node_done(const QString& node_id, bool success, const QJsonValue& output,
    ...
    nr.node_id = node_id;
    nr.success = success;
    nr.kind = success ? openmarketterminal::OutcomeKind::Ok : openmarketterminal::OutcomeKind::Failed;
    nr.output = output;
    nr.error = error;
