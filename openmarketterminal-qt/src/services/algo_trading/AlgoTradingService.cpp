...
void AlgoTradingService::save_strategy(const AlgoStrategy& strategy) {
    ...
    if (resolved_id.isEmpty() && !strategy.name.isEmpty()) {
        auto q = openmarketterminal::Database::instance().execute(
...
void AlgoTradingService::seed_library() {
    ...
    for (const auto& s : algo_library_strategies()) {
        ...
        if (force) {
            ...
            openmarketterminal::Database::instance().execute(
                "INSERT INTO algo_strategies "
                "(id, name, description, timeframe, entry_conditions, exit_conditions, "
...
// Use openmarketterminal::algo::AlgoEngine::instance() for all deployment operations.
...
void AlgoTradingService::run_backtest(const AlgoStrategy& strategy, const QString& symbol,
    ...
    const openmarketterminal::algo::DataSource source =
        broker_id.isEmpty() ? openmarketterminal::algo::DataSource::YFinance : openmarketterminal::algo::DataSource::Auto;
    ...
    openmarketterminal::algo::CandleDataFetcher::instance().fetch(
        ...
        [this, entry, exit, entry_logic, exit_logic, sl, tp, trail, size_pct, capital, timeframe, symbol](
            bool ok, const QVector<openmarketterminal::algo::OhlcvCandle>& candles, const QString& err) {
            if (!ok || candles.isEmpty()) {
                emit error_occurred("backtest", err.isEmpty() ? QStringLiteral("No data") : err);
                return;
            }
            const QJsonObject result = openmarketterminal::algo::BacktestEngine::run(
