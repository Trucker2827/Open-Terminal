...
bool resolve_connected_data_broker(const QString& symbol, QString& broker_id, QString& account_id) {
    ...
    if (region.isEmpty())
        return false;
    const auto accounts = openmarketterminal::trading::AccountManager::instance().active_accounts();
    for (const auto& a : accounts) {
        if (a.state != openmarketterminal::trading::ConnectionState::Connected)
            continue;
        auto* b = openmarketterminal::trading::BrokerRegistry::instance().get(a.broker_id);
...
QString broker_candles_to_json(const QVector<openmarketterminal::trading::BrokerCandle>& candles) {
    struct Row {
        qint64 ts;
        const openmarketterminal::trading::BrokerCandle* c;
    };
    std::vector<Row> rows;
...
void EquityResearchService::load_quote_only(const QString& symbol) {
    if (symbol.isEmpty())
        return;
    const QVariant qcv = openmarketterminal::CacheManager::instance().get("equity:quote:" + symbol);
    if (!qcv.isNull()) {
...
void EquityResearchService::load_info_only(const QString& symbol) {
    if (symbol.isEmpty())
        return;
    const QVariant icv = openmarketterminal::CacheManager::instance().get("equity:info:" + symbol);
    if (!icv.isNull()) {
...
void EquityResearchService::load_historical_only(const QString& symbol, const QString& period) {
    ...
    const QVariant hcv = openmarketterminal::CacheManager::instance().get(cache_key);
    if (!hcv.isNull()) {
...
void EquityResearchService::fetch_news(const QString& symbol, int count, NewsProvider provider) {
    ...
    if (provider == NewsProvider::NewsApi) {
        ...
        if (!key.isEmpty()) {
            const QVariant ncv = openmarketterminal::CacheManager::instance().get("equity:news:newsapi:" + symbol);
            if (!ncv.isNull()) {
    ...
    {
        const QVariant ncv = openmarketterminal::CacheManager::instance().get("equity:news:" + symbol);
        if (!ncv.isNull()) {
            const QJsonArray arr = QJsonDocument::fromJson(ncv.toString().toUtf8()).array();
            emit news_loaded(symbol, parse_news(arr));
            return;
...
QString EquityResearchService::configured_newsapi_key() const {
    const auto res = openmarketterminal::DataSourceRepository::instance().list_all();
    if (res.is_err())
        return {};
...
void EquityResearchService::fetch_news_newsapi(const QString& symbol, int count, const QString& api_key) {
    ...
    req.setHeader(QNetworkRequest::UserAgentHeader, QStringLiteral("OpenMarketTerminal"));
...
void EquityResearchService::ensure_candles(const QString& symbol, const QString& period,
    ...
    auto run_yfinance = [self, symbol, period, cache_key, done]() {
        ...
        self->run_python("yfinance_data.py", {"historical_period", symbol, period, "1d"},
                         [cache_key, done](bool ok, const QString& out) {
                             ...
                             openmarketterminal::CacheManager::instance().put(cache_key, QVariant(raw), kHistoricalTtlSec, "equity");
                             done(true, raw);
                         });
    };
    ...
    if (resolve_connected_data_broker(symbol, broker_id, account_id)) {
        ...
        openmarketterminal::trading::HistoricalDataService::instance().fetch(
            bare, QStringLiteral("1d"), lookback, broker_id, account_id,
            [cache_key, done, run_yfinance](bool ok, const QVector<openmarketterminal::trading::BrokerCandle>& candles,
                                            const QString& /*err*/) {
                if (ok && !candles.isEmpty()) {
                    const QString json = broker_candles_to_json(candles);
                    if (!json.isEmpty() && json != QLatin1String("[]")) {
                        openmarketterminal::CacheManager::instance().put(cache_key, QVariant(json), kHistoricalTtlSec, "equity");
                        done(true, json);
                        return;
                    }
                }
