...
// LOCAL-FIRST FORK: QuantLib is no longer pointed at the vendor
// cloud. It reads a self-hosted base URL from settings ("connectors.quantlib_url",
// e.g. a local QuantLib REST server at http://127.0.0.1:8800). When unset, the
// client is inert — it never makes a network request (see QuantLibClient::call).
static QString quantlib_base() {
    auto r = openmarketterminal::SettingsRepository::instance().get(
        QStringLiteral("connectors.quantlib_url"));
...
static QNetworkRequest build_request(const QString& endpoint, const QJsonObject& query_params = {}) {
    QString base = quantlib_base();
    if (base.endsWith('/'))
        base.chop(1);
    QString url = base + "/quantlib/" + endpoint;
    ...
    req.setRawHeader("User-Agent", "OpenMarketTerminal/0.1.0");
...
void QuantLibClient::call(const QString& endpoint, const QJsonObject& body, QuantLibCallback callback) {
    // LOCAL-FIRST FORK: never call out unless a self-hosted QuantLib server is
    // configured. No backend → return a clear error, make zero network requests.
    if (quantlib_base().isEmpty()) {
        callback(mcp::ToolResult::fail(
            "QuantLib pricing service not configured. It is free and open-source — run a "
            "local QuantLib REST server and set connectors.quantlib_url in Settings."));
        return;
    }
    ...
    if (cacheable) {
        ...
        connect(reply, &QNetworkReply::finished, this, [self, reply, nam, endpoint, cache_key, callback]() {
            ...
            if (result.success) {
                ...
                if (!to_cache.isEmpty())
                    openmarketterminal::CacheManager::instance().put(cache_key, QVariant(to_cache), kRefDataTtlSec, "quantlib");
            }
            LOG_DEBUG(
                kQuantLibClientTag,
