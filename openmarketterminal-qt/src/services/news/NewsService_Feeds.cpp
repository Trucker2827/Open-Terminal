...
QVector<RSSFeed> NewsService::default_feeds() {
    return {
        ...
        //  not RSS. OpenMarketTerminal's macro coverage is already provided by WSJ /
...
openmarketterminal::RssFeedRow to_row(const openmarketterminal::services::RSSFeed& f, bool is_builtin, bool enabled) {
    openmarketterminal::RssFeedRow r;
    r.id = f.id;
    r.name = f.name;
    r.url = f.url;
...
openmarketterminal::services::RSSFeed apply_overlay(const openmarketterminal::services::RSSFeed& base,
                                          const openmarketterminal::RssFeedRow& patch) {
    openmarketterminal::services::RSSFeed merged = base;
...
QVector<RSSFeed> NewsService::list_effective_feeds() const {
    ...
    auto repo_res = openmarketterminal::RssFeedRepository::instance().list_all();
    if (repo_res.is_err()) {
    ...
    QHash<QString, openmarketterminal::RssFeedRow> by_id;
...
bool NewsService::add_user_feed(const RSSFeed& f) {
    ...
    auto r = openmarketterminal::RssFeedRepository::instance().upsert(row);
    if (r.is_err()) {
...
bool NewsService::remove_or_reset_feed(const QString& id) {
    auto r = openmarketterminal::RssFeedRepository::instance().remove(id);
    if (r.is_err()) {
...
bool NewsService::set_feed_enabled(const QString& id, bool enabled) {
    ...
    auto& repo = openmarketterminal::RssFeedRepository::instance();
...
void NewsService::reload_feeds() {
    openmarketterminal::CacheManager::instance().clear_category("news");
    emit feeds_changed();
}
