...
void NewsMonitorService::load_from_db() {
    monitors_.clear();
    auto result = openmarketterminal::NewsMonitorRepository::instance().list_all();
    if (result.is_err())
        return;
...
void NewsMonitorService::save_to_db(const NewsMonitor& m) {
    openmarketterminal::NewsMonitorRow row{m.id, m.label, m.keywords, m.color, m.enabled};
    openmarketterminal::NewsMonitorRepository::instance().upsert(row);
}

void NewsMonitorService::remove_from_db(const QString& id) {
    openmarketterminal::NewsMonitorRepository::instance().remove(id);
}
