...
void NewsService::connect_live_feed(const QString& ws_url) {
    ...
    // LOCAL-FIRST FORK: the default live feed was the vendor
    // cloud news websocket. With that backend gone, do NOT connect to the
    // api_base_url default — the RSS feeds (NewsService_Feeds) are the free,
    // transparent news source. Only connect if the caller explicitly supplies a
    // websocket URL (e.g. your own/self-hosted free news stream).
    if (ws_url.isEmpty()) {
        LOG_INFO("NewsService",
                 "Live feed not started — using RSS sources (no cloud news websocket).");
        return;
    }
    ...
    connect(live_ws_, &QWebSocket::textMessageReceived, this, [this](const QString& msg) {
        ...
        {
            const QVariant cv = openmarketterminal::CacheManager::instance().get("news:articles");
            if (!cv.isNull()) {
    ...
    // ws_url is guaranteed non-empty here (see the early return above).
