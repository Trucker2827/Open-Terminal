...
// "OpenMarketTerminal/0.1.0" as scraper traffic. Browser UA gets us 200s on
// the same endpoints.
...
void NewsService::fetch_all_news(bool force, ArticlesCallback cb) {
    if (!force) {
        const QVariant cached = openmarketterminal::CacheManager::instance().get("news:articles");
        if (!cached.isNull()) {
...
// ── AI Analysis via OpenMarketTerminal API ─────────────────────────────────────────────
...
std::optional<NewsAnalysis> NewsService::cached_analysis(const QString& url) {
    auto r = openmarketterminal::NewsArticleRepository::instance().load_analysis(url);
...
void NewsService::analyze_article(const QString& url, AnalysisCallback cb) {
    ...
    HttpClient::instance().post("/news/analyze", body, [url, cb](Result<QJsonDocument> result) {
        ...
        auto wr = openmarketterminal::NewsArticleRepository::instance().save_analysis(url, json);
        if (wr.is_err())
...
void NewsService::summarize_headlines(const QVector<NewsArticle>& articles, int count, SummaryCallback cb) {
...

    {
        ...
        const QVariant cached = openmarketterminal::CacheManager::instance().get(sum_key);
        if (!cached.isNull()) {
    ...
    HttpClient::instance().post("/news/summarize", body, [cb, sig](Result<QJsonDocument> result) {
        ...
        if (!summary.isEmpty()) {
            openmarketterminal::CacheManager::instance().put("news:summary:" + sig.left(200), QVariant(summary),
...
void NewsService::ensure_registered_with_hub() {
    ...
    openmarketterminal::datahub::TopicPolicy general;
    ...
    openmarketterminal::datahub::TopicPolicy derived = general;
    ...
    openmarketterminal::datahub::TopicPolicy cluster_policy;
