...
QString LlmService::get_models_url(const QString& provider, const QString& api_key, const QString& base_url) {
    ...
    // Custom base_url (except openmarketterminal) — assume OpenAI-compatible /v1/models.
    if (!base_url.isEmpty() && p != "openmarketterminal") {
    ...
    if (p == "openmarketterminal") return openmarketterminal::AppConfig::instance().api_base_url() + "/research/llm/models";
...
QMap<QString, QString> LlmService::get_models_headers(const QString& provider, const QString& api_key) {
    ...
    if (p == "anthropic") {
        ...
        if (resolved_key.isEmpty())
            resolved_key = openmarketterminal::auth::AuthManager::instance().openmarketterminal_api_key();
        if (!resolved_key.isEmpty())
...
void LlmService::fetch_models(const QString& provider, const QString& api_key, const QString& base_url) {
    // No public /models endpoint for openmarketterminal — return known list immediately.
    if (provider.toLower() == "openmarketterminal") {
