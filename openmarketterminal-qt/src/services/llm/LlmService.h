#pragma once
// Multi-provider LLM client (OpenAI-compatible, Anthropic, Gemini, OpenMarketTerminal). Streams via SSE.
...
inline bool provider_supports_streaming(const QString& provider) {
    return provider == "openai" || provider == "anthropic" || provider == "gemini" || provider == "google" ||
           ...
           provider == "kimi" || provider == "ollama" || provider == "xai" || provider == "openmarketterminal" ||
           provider == "astraflow" || provider == "astraflow_cn" || provider == "aihubmix" ||
           provider == "atlascloud";
}

inline bool provider_requires_api_key(const QString& provider) {
    return provider != "ollama" && provider != "openmarketterminal";
}
...
class LlmService : public QObject {
  ...
  private:
    ...
    QJsonObject build_openmarketterminal_request(const QString& user_message, const std::vector<ConversationMessage>& history,
                                      bool with_tools);
    ...
    /// QEventLoop-based path used for Cloudflare-protected OpenMarketTerminal endpoints.
    ...
    LlmResponse openmarketterminal_async_request(const QString& user_message, const std::vector<ConversationMessage>& history);
};
