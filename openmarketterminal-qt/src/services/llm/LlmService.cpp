...
void LlmService::ensure_config() const {
    // Called with mutex_ held. Cache is one-shot except for OpenMarketTerminal credentials —
    // those re-resolve every call so a login that happens after first ensure_config() doesn't 401 forever.
    if (config_loaded_) {
        if (provider_ == "openmarketterminal") {
            // Resolve via AuthManager (session → SecureStorage). Never the
            // legacy plaintext settings row (CR-08).
            const QString key = openmarketterminal::auth::AuthManager::instance().openmarketterminal_api_key();
            if (!key.isEmpty())
                api_key_ = key;
        }
        return;
    ...
    // LOCAL-FIRST FORK: the hosted OpenMarketTerminal LLM proxy is removed. Default to a local
    // Ollama endpoint instead of OpenMarketTerminal, and coerce any stale "openmarketterminal" selection
    ...
    if (system_prompt_.trimmed().isEmpty() && local_model) {
        ...
        system_prompt_ =
            ...
            "    Report'), author (e.g. 'OpenMarketTerminal Research'), company, and date. Don't leave 'Analyst' "
...
QString LlmService::get_endpoint_url() const {
    ...
    // OpenMarketTerminal sync chat endpoint (async lives in openmarketterminal_async_request).
    if (p == "openmarketterminal") {
        return openmarketterminal::AppConfig::instance().api_base_url() + "/research/chat";
    }
...
QMap<QString, QString> LlmService::get_headers() const {
    ...
    } else if (p == "openmarketterminal") {
        if (!api_key_.isEmpty())
            h["X-API-Key"] = api_key_;
        // session_token only lives in AuthManager — read live, never cached.
        const auto& sess = openmarketterminal::auth::AuthManager::instance().session();
        if (!sess.session_token.isEmpty())
            h["X-Session-Token"] = sess.session_token;
        h["User-Agent"] = "OpenTerminal/0.1.0"; // Cloudflare requires it.
    } else {
        if (!api_key_.isEmpty())
            h["Authorization"] = "Bearer " + api_key_;
        if (p == "openrouter") {
            // Attribution for the openrouter.ai/rankings leaderboard.
            h["HTTP-Referer"] = "https://github.com/your-org/open-terminal";
            h["X-Title"] = "Open Terminal";
        }
    }
    return h;
}
...
LlmResponse LlmService::do_request(const QString& user_message, const std::vector<ConversationMessage>& history) {
    ...
    if (provider_ == "anthropic") {
        ...
        // OpenMarketTerminal uses two separate endpoints:
        // Primary response → async (submit + poll, returns richer model output)
        // Follow-ups (tool results) → sync /research/chat
        LOG_INFO(kLlmSvcTag, "do_request: routing to openmarketterminal_async_request");
        return openmarketterminal_async_request(user_message, history);
    } else {
    ...
    if (provider_ == "anthropic") {
        ...
        if (resp.content.isEmpty()) {
            resp.error = "The LLM returned an empty response. Please try again.";
            LOG_WARN(kLlmSvcTag, "OpenMarketTerminal /research/chat returned empty choices or content");
            return resp;
        }
...
LlmResponse LlmService::do_streaming_request(const QString& user_message,
    ...
    // Gemini uses :streamGenerateContent, OpenMarketTerminal uses async submit/poll — neither
    // fits this OpenAI-compat SSE path. Fall back to do_request and emit as one chunk.
    if (provider_ == "gemini" || provider_ == "google" || provider_ == "openmarketterminal") {
        ...
        while (true) {
            ...
            if (!tool_call_detected) {
                ...
                if (!doc.isNull() && doc.isObject()) {
                    ...
                    // OpenMarketTerminal can also surface tool_calls at top level.
                    if (!obj["tool_calls"].isUndefined() && !obj["tool_calls"].isNull() &&
                        obj["tool_calls"].toArray().size() > 0) {
                        LOG_INFO(kLlmSvcTag, "STREAM: top-level tool_calls detected (openmarketterminal)");
                        tool_call_detected = true;
                        loop.quit();
                        return;
                    }
