...
class DeepgramSttProvider final : public PythonSttProvider {
  ...
  protected:
    ...
    void extend_environment(QProcessEnvironment& env) const override {
        ...
        env.insert("OPENMARKETTERMINAL_STT_MODEL", model);
        env.insert("OPENMARKETTERMINAL_STT_LANGUAGE", lang);
        env.insert("OPENMARKETTERMINAL_STT_KEYTERMS", keyterms);
        env.insert("OPENMARKETTERMINAL_STT_GAIN", gain);
