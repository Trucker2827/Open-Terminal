...
std::optional<PolymarketCredentials> PredictionCredentialStore::load_polymarket() {
    auto r = openmarketterminal::SecureStorage::instance().retrieve(QString::fromLatin1(kPolymarketKey));
...
bool PredictionCredentialStore::save_polymarket(const PolymarketCredentials& creds) {
    ...
    auto r = openmarketterminal::SecureStorage::instance().store(QString::fromLatin1(kPolymarketKey),
...
bool PredictionCredentialStore::clear_polymarket() {
    auto r = openmarketterminal::SecureStorage::instance().remove(QString::fromLatin1(kPolymarketKey));
    return r.is_ok();
}
...
std::optional<kalshi_ns::KalshiCredentials> PredictionCredentialStore::load_kalshi() {
    auto r = openmarketterminal::SecureStorage::instance().retrieve(QString::fromLatin1(kKalshiKey));
    if (r.is_err()) return std::nullopt;
    auto obj = decode(r.value());
    if (!obj) return std::nullopt;
...
bool PredictionCredentialStore::save_kalshi(const kalshi_ns::KalshiCredentials& creds) {
    if (!creds.is_valid()) return false;
    auto r = openmarketterminal::SecureStorage::instance().store(QString::fromLatin1(kKalshiKey),
                                                      encode(to_json(creds)));
    if (r.is_err()) {
        LOG_ERROR("PredictionCreds",
...
bool PredictionCredentialStore::clear_kalshi() {
    auto r = openmarketterminal::SecureStorage::instance().remove(QString::fromLatin1(kKalshiKey));
    return r.is_ok();
}
