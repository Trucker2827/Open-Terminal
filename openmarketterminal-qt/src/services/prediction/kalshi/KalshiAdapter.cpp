...
KalshiAdapter::KalshiAdapter(QObject* parent)
    : openmarketterminal::services::prediction::PredictionExchangeAdapter(parent),
...
static bool contains_ci(const QString& haystack, const QString& needle) {
    return !needle.isEmpty() && haystack.contains(needle, Qt::CaseInsensitive);
}

static QString market_search_text(const pr::PredictionMarket& m) {
    QStringList parts;
    parts << m.key.market_id << m.key.event_id << m.question << m.description << m.category
          << m.tags.join(' ');
    for (const auto& o : m.outcomes) parts << o.name << o.asset_id;
    for (auto it = m.extras.cbegin(); it != m.extras.cend(); ++it) {
        parts << it.key() << it.value().toString();
    }
    return parts.join(' ');
}

static QString event_search_text(const pr::PredictionEvent& e) {
    QStringList parts;
    parts << e.key.event_id << e.title << e.description << e.category << e.tags.join(' ');
    for (auto it = e.extras.cbegin(); it != e.extras.cend(); ++it) {
        parts << it.key() << it.value().toString();
    }
    for (const auto& m : e.markets) parts << market_search_text(m);
    return parts.join(' ');
}

static QVector<pr::PredictionMarket> filter_markets(const QVector<pr::PredictionMarket>& markets,
                                                    const QString& query,
                                                    int limit) {
    QVector<pr::PredictionMarket> out;
    out.reserve(qMin(limit, markets.size()));
    for (const auto& m : markets) {
        if (!contains_ci(market_search_text(m), query)) continue;
        out.push_back(m);
...
static QVector<pr::PredictionEvent> filter_events(const QVector<pr::PredictionEvent>& events,
                                                  const QString& query,
                                                  int limit) {
    QVector<pr::PredictionEvent> out;
    out.reserve(qMin(limit, events.size()));
    for (const auto& e : events) {
        if (!contains_ci(event_search_text(e), query)) continue;
        out.push_back(e);
        if (out.size() >= limit) break;
    }
    return out;
...
void KalshiAdapter::search(const QString& query, int limit) {
    ...
    search_markets_done_ = false;
    search_events_done_ = false;
    search_query_ = query.trimmed();
    search_limit_ = qBound(1, limit, 100);
    search_markets_.clear();
    search_events_.clear();
    ...
    // Kalshi has no broad free-text endpoint. Make the visible UI feel
    // functional by pulling a reasonable first page of active markets/events
    // and filtering locally across title, ticker, event, series/category,
    // rules, subtitles, and outcome labels.
    rest_->fetch_markets(QStringLiteral("open"), QString(), QString(), QString(), 500, QString());
    rest_->fetch_events(QStringLiteral("open"), QString(), /*with_nested_markets=*/true, 250, QString());
}
...
void KalshiAdapter::maybe_emit_search_results() {
    if (!search_pending_ || !search_markets_done_ || !search_events_done_) return;
    search_pending_ = false;
    emit search_results_ready(search_markets_, search_events_);
}
...
void KalshiAdapter::run_py(const QString& command, const QJsonObject& extra,
    ...
    openmarketterminal::python::PythonRunner::instance().run_with_stdin(
        ...
        [self, on_ok, ctx](openmarketterminal::python::PythonResult r) {
            ...
            const QString json_str = openmarketterminal::python::extract_json(r.output);
...
void KalshiAdapter::place_order(const pr::OrderRequest& req) {
    ...
    QPointer<KalshiAdapter> self = this;
    auto submit_order = [self, extra]() {
        if (!self) return;
        self->run_py(QStringLiteral("place_order"), extra,
           [self](const QJsonObject& resp) {
               if (!self) return;
               pr::OrderResult out;
    ...
    if (order_type != QStringLiteral("limit")) {
        submit_order();
        return;
    }

    run_py(QStringLiteral("preflight_order"), extra,
           [self, submit_order](const QJsonObject& resp) {
               if (!self) return;
               const auto preflight = resp.value("preflight").toObject();
               const auto warnings = preflight.value("warnings").toArray();
               if (!warnings.isEmpty()) {
                   QStringList warning_parts;
                   warning_parts.reserve(warnings.size());
                   for (const auto& warning : warnings) warning_parts.push_back(warning.toString());
                   LOG_WARN("KalshiAdapter",
                            QStringLiteral("Order preflight warnings: ") + warning_parts.join(", "));
               }
               submit_order();
           },
           QStringLiteral("preflight_order"));
}
...
void KalshiAdapter::wire() {
    ...
    connect(rest_.get(), &KalshiRestClient::markets_ready, this,
            [this](const QVector<pr::PredictionMarket>& markets, const QString& /*cursor*/) {
                ...
                if (search_pending_) {
                    search_markets_ = filter_markets(markets, search_query_, search_limit_);
                    search_markets_done_ = true;
                    maybe_emit_search_results();
                } else {
    ...
    connect(rest_.get(), &KalshiRestClient::events_ready, this,
            [this](const QVector<pr::PredictionEvent>& events, const QString& /*cursor*/) {
                ...
                if (search_pending_) {
                    search_events_ = filter_events(events, search_query_, search_limit_);
                    search_events_done_ = true;
                    maybe_emit_search_results();
                } else {
