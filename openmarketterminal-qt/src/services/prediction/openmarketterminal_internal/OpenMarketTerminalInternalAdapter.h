...
/// Internal OpenMarketTerminal prediction-market exchange adapter (Phase 4).
///
/// Joins `PolymarketAdapter` and `KalshiAdapter` as a third member of
/// `PredictionExchangeRegistry`. The exchange is operated by OpenMarketTerminal;
/// demo markets settle in USD.
///
/// **Endpoint resolution:** the adapter reads `openmarketterminal.markets_endpoint`
/// from `SecureStorage`. Examples: `https://markets.example.com/api/v1`,
/// `wss://markets.example.com/ws`. When unset, the adapter operates in
/// **demo mode** — every read method emits a curated mock dataset (the
/// three example markets: "Fed cuts in May 2026",
/// "NYC max 80F+ on May 1", "BTC above $100k by month end") and every
/// authenticated method emits an `error_occurred("not deployed", …)`
/// signal so screens can surface a "service unavailable" banner.
///
/// **On-chain dependency.** Real settlement requires the `openmarketterminal_market`
/// Anchor program (`solana/programs/openmarketterminal_market/`) which lives in a
...
/// is configured (`openmarketterminal.market_program_id`), `place_order` and the
...
/// to `prediction:openmarketterminal:*` topics following the same family pattern
/// Polymarket / Kalshi already use. In demo mode, no topics are published
/// — screens read from the adapter's Qt signals directly.
class OpenMarketTerminalInternalAdapter : public PredictionExchangeAdapter {
    Q_OBJECT
  public:
    explicit OpenMarketTerminalInternalAdapter(QObject* parent = nullptr);
    ~OpenMarketTerminalInternalAdapter() override;
    ...
    /// True iff `openmarketterminal.markets_endpoint` is configured. Screens can use
