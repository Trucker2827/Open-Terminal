...
void EconomicsService::invalidate(const QString& request_id) {
    openmarketterminal::CacheManager::instance().remove(request_id);
}
...
void EconomicsService::execute(const QString& source_id, const QString& script, const QString& command,
    ...
    {
        const QVariant cached = openmarketterminal::CacheManager::instance().get(key);
        if (!cached.isNull()) {
            ...
            if (hub_registered_)
                openmarketterminal::datahub::DataHub::instance().publish(topic, QVariant::fromValue(res));
            return;
        }
    }
...
void EconomicsService::refresh(const QStringList& topics) {
    ...
    for (const auto& topic : topics) {
        ...
        openmarketterminal::CacheManager::instance().remove(cache_key(rec.script, rec.command, rec.args));
