...
void DebugOverlay::refresh() {
    QString frame_info;
    if (auto* frame = qobject_cast<openmarketterminal::WindowFrame*>(window())) {
        const auto frame_uuid = frame->frame_uuid();
        const QString uuid_short = frame_uuid.to_string().left(8);
        const int frame_panels =
            openmarketterminal::PanelRegistry::instance().find_by_frame(frame_uuid).size();
    ...
    const int total_panels = openmarketterminal::PanelRegistry::instance().size();
    ...
    int topic_count = 0;
    {
        const auto stats = openmarketterminal::datahub::DataHub::instance().stats();
