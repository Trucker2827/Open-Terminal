...
class OpenMarketTerminalTileLayer : public QGVLayerTilesOnline {
  public:
    OpenMarketTerminalTileLayer(QString url, int min_zoom, int max_zoom)
...
static void ensure_network_manager() {
    ...
    QString cache_dir = openmarketterminal::AppPaths::cache() + "/map_tiles";
...
void WorldMapWidget::apply_basemap(int index) {
    ...
    auto* layer = new OpenMarketTerminalTileLayer(def.url, def.min_zoom, def.max_zoom);
    map_->addItem(layer);
