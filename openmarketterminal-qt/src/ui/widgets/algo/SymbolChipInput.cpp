...
QStringList loaded_broker_ids() {
    QStringList ids;
    for (const auto& a : openmarketterminal::trading::AccountManager::instance().list_accounts())
        if (!ids.contains(a.broker_id) && openmarketterminal::trading::InstrumentService::instance().is_loaded(a.broker_id))
            ids << a.broker_id;
    return ids;
}
} // namespace
...
SymbolChipInput::SymbolChipInput(QWidget* parent) : QFrame(parent) {
    ...
    edit_->setStyleSheet(QString("QLineEdit { background:transparent; border:none; color:%1; font-family:%2; font-size:%3px; }")
        ...
        .arg(QString(openmarketterminal::ui::fonts::DATA_FAMILY))
    ...
    if (auto* popup = completer_->popup()) {
        ...
        popup->setStyleSheet(
            QString("QListView { background:%1; color:%2; border:1px solid %3; outline:none;"
                ...
                .arg(openmarketterminal::ui::fonts::SMALL)
                .arg(openmarketterminal::ui::colors::BG_HOVER(), openmarketterminal::ui::colors::AMBER()));
...
// Show only canonical alphabetic exchanges (NYSE/NASDAQ/LSE…); hide broker numeric
...
void SymbolChipInput::on_text_changed(const QString& text) {
    ...
    const auto results = openmarketterminal::trading::InstrumentService::instance().search_all(q, "", loaded_broker_ids(), 16);
    QStringList labels;
    sugg_map_.clear();
    for (const auto& r : results) {
        ...
        const QString friendly = openmarketterminal::trading::norm::display_name(
...
void SymbolChipInput::add_chip(const QString& symbol) {
    ...
    chip->setStyleSheet(QString("QFrame { background:%1; border:1px solid %2; border-radius:3px; }")
                            .arg(openmarketterminal::ui::colors::BG_RAISED(), openmarketterminal::ui::colors::AMBER()));
...
void SymbolChipInput::fetch_price(const QString& symbol) {
    // Namespace is openmarketterminal::services (not openmarketterminal::services::markets).
    // QuoteCallback = std::function<void(bool, QVector<QuoteData>)> — by value, not const-ref.
    openmarketterminal::services::MarketDataService::instance().fetch_quotes(
        {symbol}, [this, symbol](bool ok, QVector<openmarketterminal::services::QuoteData> quotes) {
