...
void DesktopNotifier::init() {
    ...
    tray_->setIcon(ico);
    tray_->setToolTip(QStringLiteral("OpenMarketTerminal"));

    auto* menu = new QMenu();
    menu->addAction(QStringLiteral("Show OpenMarketTerminal"), qApp, []() {
    ...
    connect(&ToastService::instance(), &ToastService::posted, this,
            [this](const ToastService::Notification& n) {
                const QString title = n.source.startsWith(QStringLiteral("scan:"))
                                          ? QStringLiteral("OpenMarketTerminal Alert")
                                          : QStringLiteral("OpenMarketTerminal");
