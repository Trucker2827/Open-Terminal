...
constexpr int kArgsRole = Qt::UserRole + 1;
} // namespace
...
void CommandPalette::on_text_changed(const QString& text) {
    ...
    const auto parsed = CommandParser::parse(text);
    if (parsed.kind == ParsedCommand::Kind::Symbol) {
        const QString symbol = parsed.args.value(QStringLiteral("symbol")).toString();
        auto* item = new QListWidgetItem(tr("Open %1 in Equity Research").arg(symbol));
        item->setData(kActionRole, QStringLiteral("symbol.open"));
        item->setData(kArgsRole, parsed.args);
        item->setToolTip(tr("Ticker shortcut"));
        suggestions_->addItem(item);
    }

    auto matches = SuggestionIndex::instance().query(
        text.isEmpty() ? QStringLiteral("a") : text, /*limit=*/25);
    if (matches.isEmpty() && !text.isEmpty() && suggestions_->count() == 0) {
        ...
        for (const QString& id : ActionRegistry::instance().all_ids()) {
            ...
            item->setData(kActionRole, def->id);
            suggestions_->addItem(item);
...
void CommandPalette::on_accept() {
    ...
    QVariantMap args;
    if (item) {
        action_id = item->data(kActionRole).toString();
        args = item->data(kArgsRole).toMap();
    } else {
        ...
        if (parsed.kind == ParsedCommand::Kind::Action) {
            ...
            args = parsed.args;
        } else if (parsed.kind == ParsedCommand::Kind::Symbol) {
            action_id = QStringLiteral("symbol.open");
            args = parsed.args;
        }
    }
