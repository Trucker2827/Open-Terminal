...
qint64 StorageManager::main_db_size() const {
    QFileInfo fi(AppPaths::data() + "/openmarketterminal.db");
    if (!fi.exists())
        return 0;
    // Include WAL + SHM files
    qint64 total = fi.size();
    QFileInfo wal(AppPaths::data() + "/openmarketterminal.db-wal");
    if (wal.exists())
        total += wal.size();
    QFileInfo shm(AppPaths::data() + "/openmarketterminal.db-shm");
    if (shm.exists())
        total += shm.size();
    return total;
}
...
Result<void> StorageManager::clear_qsettings() {
    ...
    QSettings settings("Open Terminal", "Open Terminal");
    settings.clear();
    settings.sync();
