...
// database/historify_db.py) onto OpenMarketTerminal's EXISTING SQLite stack.
//
// DECISION — SQLite, not DuckDB:
//   OpenAlgo's Historify uses DuckDB (columnar OLAP). OpenMarketTerminal has NO DuckDB
...
//
// Conventions:
//   - namespace openmarketterminal::storage (matches the src/storage/ directory).
