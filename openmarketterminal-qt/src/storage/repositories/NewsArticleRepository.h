...
class NewsArticleRepository : public BaseRepository<openmarketterminal::services::NewsArticle> {
  public:
    ...
    Result<void> upsert_batch(const QVector<openmarketterminal::services::NewsArticle>& articles);

    /// Load articles newer than since_ts (unix seconds).
    /// Optional category filter ("" = all). Ordered newest-first.
    Result<QVector<openmarketterminal::services::NewsArticle>> load_recent(int64_t since_ts, const QString& category = {},
    ...
    Result<QVector<openmarketterminal::services::NewsArticle>> load_saved() const;
    ...
    Result<QVector<openmarketterminal::services::NewsArticle>> search_fts(const QString& query, int64_t since_ts = 0,
                                                               int limit = 500) const;
