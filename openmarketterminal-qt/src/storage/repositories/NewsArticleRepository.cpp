...
using openmarketterminal::services::Impact;
using openmarketterminal::services::NewsArticle;
using openmarketterminal::services::Priority;
using openmarketterminal::services::Sentiment;
using openmarketterminal::services::SourceFlag;
using openmarketterminal::services::ThreatLevel;
...
Result<QVector<openmarketterminal::services::NewsArticle>> NewsArticleRepository::load_saved() const {
    return query_list("SELECT id, headline, summary, source, region, category, link, sort_ts, "
                      "       priority, sentiment, impact, tickers, tier, lang, "
                      "       threat_level, threat_cat, threat_conf, source_flag "
...
Result<QVector<openmarketterminal::services::NewsArticle>> NewsArticleRepository::search_fts(const QString& query,
    ...
    if (r.is_ok()) {
        QVector<openmarketterminal::services::NewsArticle> result;
        auto& q = r.value();
        while (q.next())
            result.append(map_row(q));
        return Result<QVector<openmarketterminal::services::NewsArticle>>::ok(std::move(result));
    }
