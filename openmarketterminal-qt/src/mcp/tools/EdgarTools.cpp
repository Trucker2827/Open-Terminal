...
static ToolResult run_edgar(const QStringList& args) {
    ...
    auto* runner = &openmarketterminal::python::PythonRunner::instance();
    detail::run_async_wait(runner, [&](auto signal_done) {
        runner->run(SCRIPT, args, [&, signal_done](const openmarketterminal::python::PythonResult& r) {
            if (!r.success) {
                error = r.error.isEmpty() ? r.output : r.error;
            } else {
                QString text = openmarketterminal::python::extract_json(r.output);
                QJsonParseError pe;
