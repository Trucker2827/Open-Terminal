...
void on_llm_mutation_start() {
    ...
    const QString chat_id = openmarketterminal::ai_chat::detail::t_chat_session_id;
    if (chat_id.isEmpty())
        return;
...
std::vector<ToolDef> get_report_builder_tools() {
    ...
    // ── report_set_theme ────────────────────────────────────────────────────
    {
        ...
        t.description = "Set the report color theme. Valid names: 'Light Professional', 'Dark Corporate', "
                        "'OpenMarketTerminal', 'Midnight Blue'.";
        t.category = "report-builder";
        t.input_schema.properties =
    ...
    // ── report_save ────────────────────────────────────────────────────────
    {
        ...
        t.category = "report-builder";
        t.input_schema.properties =
            QJsonObject{{"path", QJsonObject{{"type", "string"}, {"description", "Absolute file path (.openmarketterminal)"}}}};
        t.handler = [](const QJsonObject& args) -> ToolResult {
    ...
    // ── report_export_pdf ───────────────────────────────────────────────────
    {
        ...
        t.handler = [](const QJsonObject& args) -> ToolResult {
            ...
            QMetaObject::invokeMethod(QCoreApplication::instance(), [&]() {
                auto* scr = ::openmarketterminal::screens::ReportBuilderScreen::current();
                if (!scr) {
