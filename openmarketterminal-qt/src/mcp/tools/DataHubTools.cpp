...
std::vector<ToolDef> get_datahub_tools() {
    ...
    // ── datahub_list_topics ─────────────────────────────────────────────
    {
        ...
        t.handler = [](const QJsonObject&) -> ToolResult {
            const auto snap = openmarketterminal::datahub::DataHub::instance().stats();
    ...
    // ── datahub_request ─────────────────────────────────────────────────
    {
        ...
        t.handler = [](const QJsonObject& args) -> ToolResult {
            ...
            openmarketterminal::datahub::DataHub::instance().request(topic, force);
    ...
    // payload. No worker-thread blocking, no nested event loops.
    {
        ...
        t.async_handler = [](const QJsonObject& args, ToolContext ctx,
            ...
            QMetaObject::invokeMethod(qApp, [topic, duration, ctx, promise]() {
                ...
                openmarketterminal::datahub::DataHub::instance().subscribe(
                ...
                QTimer::singleShot(duration, qApp, [topic, duration, collected, owner, promise, ctx]() {
                    openmarketterminal::datahub::DataHub::instance().unsubscribe(owner);
                    owner->deleteLater();
