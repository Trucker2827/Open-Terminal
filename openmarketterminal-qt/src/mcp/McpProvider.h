...
class McpProvider {
  public:
    ...
    /// proceed. The hook receives the tool name + args plus the required
    /// AuthLevel and is_destructive flag; it is responsible for checking the
    /// active session, prompting the user (per-action confirmation modal), and
    /// returning the verdict synchronously. The name+args let the hook describe
    /// exactly what is being approved (e.g. "Place LIVE order: BUY 100 AAPL").
    /// Hook lives in the app layer to avoid pulling auth/UI headers into McpTypes.h.
    ///
    /// When unset, tools with auth_required > Authenticated or
    /// is_destructive=true fail closed; lesser tools pass through.
    using AuthChecker =
        std::function<bool(const QString& tool_name, const QJsonObject& args, AuthLevel required, bool is_destructive)>;
    ...
    ///      Server prefix `openmarketterminal__` is 18 chars; longer tool names
