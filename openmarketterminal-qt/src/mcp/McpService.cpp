...
// both the OpenAI-format path and the Anthropic / Gemini / OpenMarketTerminal catalog
...
static bool tool_rag_enabled() {
    return openmarketterminal::AppConfig::instance()
...
ToolResult McpService::execute_tool(const QString& server_id, const QString& tool_name, const QJsonObject& args) {
    // Route to internal provider (gated by McpProvider's fail-closed AuthChecker).
    if (server_id == INTERNAL_SERVER_ID)
        return McpProvider::instance().call_tool(tool_name, args);

    // External (user-added) MCP-server tools carry no is_destructive flag and
    // run arbitrary code in the external server's own process (filesystem,
    // shell, etc.). Unlike internal tools they do NOT pass through McpProvider's
    // AuthChecker, so without this a prompt-injected agent could invoke them
    // silently. Gate them here, FAIL-CLOSED, the same way as internal
    // destructive tools: allow without a prompt only when running under an agent
    // the user explicitly opted into autonomous (allow_destructive_tools);
    // otherwise require an explicit human approval, and deny on any dismissal.
    if (!TerminalMcpBridge::is_destructive_allowed()) {
        const bool approved = ToolConfirmationGate::instance().request(
            QObject::tr("Approve external MCP tool '%1'?").arg(tool_name),
            QObject::tr("Server: %1\nRuns in an external MCP server you added.\nargs: %2")
                .arg(server_id,
                     QString::fromUtf8(QJsonDocument(args).toJson(QJsonDocument::Compact)).left(400)));
        if (!approved) {
            LOG_WARN(TAG, QString("External tool '%1' on server '%2' DENIED — not approved by user.")
                              .arg(tool_name, server_id));
            return ToolResult::fail(
                QString("External tool '%1' was not approved by the user").arg(tool_name));
        }
    }
