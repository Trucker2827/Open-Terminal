...
QWidget* PlannerViewPanel::build_templates_panel() {
    ...
    {
        const auto pr = openmarketterminal::LlmProfileRepository::instance().list_profiles();
        const auto profiles = pr.is_ok() ? pr.value() : QVector<openmarketterminal::LlmProfile>{};
    ...
    {
        auto& repo = openmarketterminal::PortfolioRepository::instance();
