...
QWidget* DocsScreen::page_quantlib() {
    return make_page(
        tr("QUANTLIB SUITE"), tr("18-module client for an external QuantLib pricing server"),
        {
            {tr("OVERVIEW"), tr("The QuantLib Suite is a thin client that forwards requests to an external, "
                                "self-hosted QuantLib REST server (set connectors.quantlib_url in Settings). "
                                "It performs no pricing locally; with no server configured it does nothing. "
                                "When connected, it covers everything from basic statistics to derivative "
