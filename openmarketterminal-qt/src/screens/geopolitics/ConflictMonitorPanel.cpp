...
void ConflictMonitorPanel::build_ui() {
    ...
    // Data-source badge (set by GeopoliticsScreen from the connector result).
    badge_ = new openmarketterminal::ui::ProvenanceBadge(map_toolbar);
    mt_hl->addWidget(badge_);
    ...
    map_widget_ = new openmarketterminal::ui::WorldMapWidget(map_container);
    ...
    connect(map_widget_, &openmarketterminal::ui::WorldMapWidget::pin_clicked, this, [this](int event_id) {
        if (!events_table_)
            return;
...
void ConflictMonitorPanel::set_provenance(const connectors::Provenance& prov) {
    if (badge_)
...
void ConflictMonitorPanel::update_map(const QVector<NewsEvent>& events) {
    QVector<openmarketterminal::ui::MapPin> pins;
