...
GeopoliticsScreen::GeopoliticsScreen(QWidget* parent) : QWidget(parent) {
    ...
    connect(&openmarketterminal::ui::ThemeManager::instance(), &openmarketterminal::ui::ThemeManager::theme_changed, this,
            [this](const openmarketterminal::ui::ThemeTokens&) { refresh_theme(); });
...
void GeopoliticsScreen::connect_service() {
    ...
    // OpenMarketTerminal: feed the conflict map from NASA EONET (live global
    // natural events) via the connector framework — the OpenMarketTerminal news-events API
    // is gone. Maps EONET events into geo::NewsEvent for the existing map/table.
    auto& nasa = connectors::NasaConnector::instance();
    connect(&nasa, &connectors::NasaConnector::events_ready, this,
            [this](QVector<connectors::NaturalEvent> events, connectors::Provenance prov) {
                QVector<services::geo::NewsEvent> mapped;
                mapped.reserve(events.size());
                for (const auto& e : events) {
                    services::geo::NewsEvent n;
                    n.source = QStringLiteral("NASA EONET");
                    n.title = e.title;
                    n.event_category = e.category;
                    n.extracted_date = e.date;
                    n.latitude = e.lat;
                    n.longitude = e.lon;
                    n.has_coords = e.has_coord;
                    mapped.append(n);
                }
                if (monitor_panel_) {
                    monitor_panel_->set_provenance(prov);
                    monitor_panel_->set_events(mapped);
                }
            });
    nasa.fetch_events(60);
}
