...
void AiChatScreen::build_sidebar() {
    ...
    auto* title = new QLabel("OpenMarketTerminal AI");
    ...
    labels->addWidget(provider_lbl_);
    labels->addWidget(model_lbl_);
    fl->addLayout(labels);
    fl->addStretch();

    // Quick access to provider/API-key setup (OpenAI, Anthropic, etc.).
    auto* cfg_btn = new QPushButton(tr("⚙  Configure LLM"));
    cfg_btn->setCursor(Qt::PointingHandCursor);
    cfg_btn->setToolTip(tr("Add an API provider (OpenAI, Anthropic, …) or pick a model"));
    cfg_btn->setStyleSheet(QString("QPushButton{background:%1;color:%2;border:1px solid %3;border-radius:3px;"
                                   "padding:4px 10px;font-size:%4px;font-weight:600;}"
                                   "QPushButton:hover{border-color:%2;}")
                               .arg(col::BG_RAISED(), col::AMBER(), col::BORDER_MED())
                               .arg(fnt::SMALL));
    connect(cfg_btn, &QPushButton::clicked, this, []() {
        CommandContext ctx;
        ctx.shell = &TerminalShell::instance();
        ctx.focused_frame = WindowCycler::instance().focused_frame();
        ActionRegistry::instance().invoke("screen.settings", ctx);
    });
    fl->addWidget(cfg_btn);
...
QWidget* AiChatScreen::build_header_bar() {
    ...
    hdr_model_lbl_->setCursor(Qt::PointingHandCursor);
    hdr_model_lbl_->installEventFilter(this); // click opens Settings (handled in eventFilter)
...
QWidget* AiChatScreen::build_input_area() {
    ...
    input_box_->setPlaceholderText(tr("Message OpenMarketTerminal AI..."));
