...
namespace fnt = openmarketterminal::ui::fonts;
namespace col = openmarketterminal::ui::colors;
...
void AiChatScreen::on_send() {
    ...
    if (!attached_file_path_.isEmpty()) {
        ...
        if (f.open(QIODevice::ReadOnly | QIODevice::Text)) {
            ...
            text = QString("[Attached file: %1 — raw text, not parsed]\n\n%2\n\n---\n\n%3")
...
void AiChatScreen::on_stream_chunk(const QString& chunk, bool done) {
    ...
    if (chunk.startsWith("\x01__TOOL_CALL_CLEAR__")) {
        ...
        openmarketterminal::ai_chat::ChatBubbleFactory::replace_streaming_text(streaming_bubble_, tr("Calling tool..."));
        scroll_to_bottom();
        return;
    }
    ...
    if (!streaming_bubble_)
        streaming_bubble_ = add_streaming_bubble();
    openmarketterminal::ai_chat::ChatBubbleFactory::append_streaming_chunk(streaming_bubble_, chunk);
    scroll_to_bottom();
}
...
void AiChatScreen::on_streaming_done(ai_chat::LlmResponse response) {
    ...
    if (!response.success) {
        if (streaming_bubble_) {
            ...
            openmarketterminal::ai_chat::ChatBubbleFactory::replace_streaming_text(streaming_bubble_, err);
        }
    ...
    if (response.content.isEmpty()) {
        if (streaming_bubble_) {
            ...
            openmarketterminal::ai_chat::ChatBubbleFactory::replace_streaming_text(streaming_bubble_, hint);
        }
    ...
    if (streaming_bubble_) {
        ...
        openmarketterminal::ai_chat::ChatBubbleFactory::finalize_streaming(streaming_bubble_, final_text);
        streaming_bubble_ = nullptr;
    }
    if (!content.isEmpty()) {
...
void AiChatScreen::add_message_bubble(const QString& role, const QString& content, const QString& timestamp) {
    openmarketterminal::ai_chat::ChatBubbleFactory::Options opts;
    opts.role               = role;
    ...
    auto b = openmarketterminal::ai_chat::ChatBubbleFactory::build(opts);
...
QLabel* AiChatScreen::add_streaming_bubble() {
    ...
    opts.user_col_max_width = kUserColMaxWidth;
    opts.ai_col_max_width   = kAiColMaxWidth;
    auto b = openmarketterminal::ai_chat::ChatBubbleFactory::build_streaming(opts);
    messages_layout_->insertWidget(messages_layout_->count() - 1, b.row);
    return b.body;
}
...
void AiChatScreen::update_stats() {
    ...
    if (llm.is_configured()) {
        ...
        const bool is_openmarketterminal = (provider_raw.toLower() == "openmarketterminal");

        // Display names. The legacy "openmarketterminal" provider is the removed
        // hosted proxy; it is coerced to a LOCAL model, so label it honestly as
        // local — there is no managed/hosted OpenMarketTerminal AI service.
        const QString prov_display = is_openmarketterminal ? tr("Local LLM") : provider_raw.toUpper();
        const QString model_raw = llm.active_model();
        const QString model_display = is_openmarketterminal ? tr("Local LLM") : model_raw;
        ...
        model_lbl_->setText(is_openmarketterminal ? tr("Local model") : model_short);
        model_lbl_->setToolTip(is_openmarketterminal ? tr("Local LLM (Ollama) — runs on your machine") : model_raw);
        ...
        if (is_openmarketterminal) {
            hdr_model_lbl_->setText(tr("Local LLM"));
            hdr_model_lbl_->setToolTip(tr("Local LLM (Ollama) — runs on your machine\n\nChange in Settings > LLM Configuration"));
        } else {
