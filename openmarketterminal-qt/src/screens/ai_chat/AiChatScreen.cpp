// AiChatScreen.cpp — OpenMarketTerminal AI Chat, Obsidian design system.
//
...
void AiChatScreen::showEvent(QShowEvent* e) {
    ...
    if (link_group_ != openmarketterminal::SymbolGroup::None) {
        const openmarketterminal::SymbolRef rehydrated =
            openmarketterminal::SymbolContext::instance().group_symbol(link_group_);
...
void AiChatScreen::retranslateUi() {
    ...
    // Input area
    if (input_box_)          input_box_->setPlaceholderText(tr("Message OpenMarketTerminal AI..."));
...
bool AiChatScreen::eventFilter(QObject* obj, QEvent* event) {
    if (obj == hdr_model_lbl_ && event->type() == QEvent::MouseButtonRelease) {
        // Same target as the "Configure LLM" button (AiChatScreen_Layout.cpp).
        CommandContext ctx;
...
void AiChatScreen::set_group(openmarketterminal::SymbolGroup g) {
    if (link_group_ == g)
        return;
    link_group_ = g;
...
void AiChatScreen::on_group_symbol_changed(const openmarketterminal::SymbolRef& ref) {
