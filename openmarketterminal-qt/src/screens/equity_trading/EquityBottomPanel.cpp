...
QColor order_status_color(const QString& status) {
    ...
    if (s == "filled" || s == "complete" || s == "completed")
        return {openmarketterminal::ui::colors::POSITIVE()};
    if (s == "rejected" || s == "cancelled" || s == "canceled")
        return {openmarketterminal::ui::colors::NEGATIVE()};
    return {openmarketterminal::ui::colors::AMBER()}; // pending / partial / open / accepted
}
...
QTableWidgetItem* EquityBottomPanel::ensure_item(QTableWidget* table, int row, int col) {
    ...
    if (!item) {
        ...
        item->setForeground(QColor(openmarketterminal::ui::colors::TEXT_PRIMARY()));
...
void EquityBottomPanel::setup_positions_tab() {
    ...
    action_bar->setStyleSheet(QString("#eqPositionsActionBar{background:%1;border-bottom:1px solid %2;}")
                                  .arg(openmarketterminal::ui::colors::PANEL(), openmarketterminal::ui::colors::BORDER()));
    ...
    positions_total_pnl_label_->setStyleSheet(
        QString("font-size:11px;font-weight:700;color:%1;").arg(openmarketterminal::ui::colors::TEXT_PRIMARY()));
    ...
    auto make_group_btn = [](const QString& color, const QString& tip) {
        ...
        b->setStyleSheet(QString("QPushButton{background:transparent;color:%1;border:1px solid %1;"
                             ...
                             .arg(color, openmarketterminal::ui::colors::TEXT_SECONDARY()));
        return b;
    };
    ...
    positions_win_label_->setStyleSheet(
        QString("font-size:11px;font-weight:600;color:%1;").arg(openmarketterminal::ui::colors::POSITIVE()));
    bar_layout->addWidget(positions_win_label_);
    win_squareoff_btn_ = make_group_btn(openmarketterminal::ui::colors::POSITIVE(), tr("Square off all winning positions"));
    ...
    positions_loss_label_->setStyleSheet(
        QString("font-size:11px;font-weight:600;color:%1;").arg(openmarketterminal::ui::colors::NEGATIVE()));
    bar_layout->addWidget(positions_loss_label_);
    loss_squareoff_btn_ = make_group_btn(openmarketterminal::ui::colors::NEGATIVE(), tr("Square off all losing positions"));
    ...
    close_all_btn_->setStyleSheet(
        QString("QPushButton{background:rgba(220,38,38,0.12);color:%1;border:1px solid %2;"
            ...
            .arg(openmarketterminal::ui::colors::NEGATIVE(), openmarketterminal::ui::colors::NEGATIVE_DIM()));
...
void EquityBottomPanel::setup_holdings_tab() {
    ...
    auto make_stat = [&](const QString& caption, QLabel*& caption_out) -> QLabel* {
        ...
        cap->setStyleSheet(QString("color:%1;font-size:10px;letter-spacing:0.5px;")
                               .arg(openmarketterminal::ui::colors::TEXT_SECONDARY()));
        caption_out = cap;
        auto* val = new QLabel("--");
        val->setStyleSheet(QString("color:%1;font-size:13px;font-weight:700;")
                               .arg(openmarketterminal::ui::colors::TEXT_PRIMARY()));
        cv->addWidget(cap);
        cv->addWidget(val);
    ...
    holdings_import_btn_->setStyleSheet(
        QString("QPushButton{background:%1;color:%2;border:1px solid %3;padding:4px 12px;"
            ...
            .arg(openmarketterminal::ui::colors::PANEL(), openmarketterminal::ui::colors::ORANGE(),
                 openmarketterminal::ui::colors::BORDER(), openmarketterminal::ui::colors::ORANGE(),
                 openmarketterminal::ui::colors::TEXT_SECONDARY()));
...
void EquityBottomPanel::setup_orders_tab() {
    ...
    cancel_all_btn_->setStyleSheet(
        QString("QPushButton{background:rgba(217,119,6,0.12);color:%1;border:1px solid %2;"
            ...
            .arg(openmarketterminal::ui::colors::AMBER(), openmarketterminal::ui::colors::AMBER_DIM()));
...
static QFrame* make_metric_card(const QString& caption, QLabel*& cap_out, QLabel*& val_out) {
    ...
    auto* cap = new QLabel(caption);
    cap->setStyleSheet(
        QString("color:%1;font-size:10px;letter-spacing:0.5px;font-weight:600;").arg(openmarketterminal::ui::colors::TEXT_SECONDARY()));
    auto* val = new QLabel(QStringLiteral("--"));
    val->setStyleSheet(QString("color:%1;font-size:15px;font-weight:700;").arg(openmarketterminal::ui::colors::TEXT_PRIMARY()));
    cv->addWidget(cap);
    cv->addWidget(val);
    cap_out = cap;
    val_out = val;
...
void EquityBottomPanel::update_positions_summary() {
    ...
    const QString pos = openmarketterminal::ui::colors::POSITIVE();
    const QString neg = openmarketterminal::ui::colors::NEGATIVE();
    const QString pri = openmarketterminal::ui::colors::TEXT_PRIMARY();
...
void EquityBottomPanel::set_paper_positions(const QVector<trading::PtPosition>& positions) {
    ...
    const QColor pos_color(openmarketterminal::ui::colors::POSITIVE());
    const QColor neg_color(openmarketterminal::ui::colors::NEGATIVE());
...
QWidget* EquityBottomPanel::make_positions_action_cell(const QString& symbol, const QString& product,
    ...
    sell->setStyleSheet(QString("QPushButton#eqTableBtn{background:rgba(239,68,68,0.15);color:%1;"
                            ...
                            .arg(openmarketterminal::ui::colors::NEGATIVE(), openmarketterminal::ui::colors::BORDER_MED()));
    ...
    if (show_convert && !paper_pid.isEmpty()) {
        ...
        cnc->setStyleSheet(QString("QPushButton#eqTableBtn{background:rgba(37,99,235,0.15);color:%1;"
                               ...
                               .arg(openmarketterminal::ui::colors::INFO(), openmarketterminal::ui::colors::BORDER_MED()));
...
void EquityBottomPanel::update_holding_quote(const QString& symbol, double ltp) {
    ...
    if (holdings_pnl_label_) {
        ...
        holdings_pnl_label_->setStyleSheet(
            QString("color:%1;font-size:13px;font-weight:700;")
                .arg(total_pnl >= 0 ? openmarketterminal::ui::colors::POSITIVE() : openmarketterminal::ui::colors::NEGATIVE()));
    }
    if (holdings_pnl_pct_label_) {
        ...
        holdings_pnl_pct_label_->setStyleSheet(
            QString("color:%1;font-size:13px;font-weight:700;")
                .arg(total_pct >= 0 ? openmarketterminal::ui::colors::POSITIVE() : openmarketterminal::ui::colors::NEGATIVE()));
    }
}
    ...
    for (int i = 0; i < orders.size(); ++i) {
        ...
        if (modifiable) {
            ...
            btn->setStyleSheet(QString("QPushButton#eqTableBtn { background: rgba(217,119,6,0.15); "
                                   ...
                                   .arg(openmarketterminal::ui::colors::AMBER())
                                   .arg(openmarketterminal::ui::colors::AMBER_DIM()));
            ...
            connect(btn, &QPushButton::clicked, this, [this, oid, qty, prc]() {
                ...
                dlg->setStyleSheet(QString("QDialog { background: %1; color: %2; }"
                                       ...
                                       .arg(openmarketterminal::ui::colors::BG_SURFACE())
                                       .arg(openmarketterminal::ui::colors::TEXT_PRIMARY())
                                       .arg(openmarketterminal::ui::colors::TEXT_SECONDARY())
                                       .arg(openmarketterminal::ui::colors::BG_BASE())
                                       .arg(openmarketterminal::ui::colors::BORDER_MED())
                                       .arg(openmarketterminal::ui::colors::AMBER()));
                ...
                cancel_btn->setStyleSheet(QString("background: rgba(220,38,38,0.1); color: %1; border: 1px solid %2;")
                                              .arg(openmarketterminal::ui::colors::NEGATIVE())
                                              .arg(openmarketterminal::ui::colors::NEGATIVE_DIM()));
    ...
    for (const auto& auction : auctions) {
        for (const auto& entry : auction.entries) {
            ...
            const QColor type_color =
                is_open ? QColor(openmarketterminal::ui::colors::AMBER()) : QColor(openmarketterminal::ui::colors::INFO());

            auto set = [&](int col, const QString& text,
                           const QColor& fg = QColor(openmarketterminal::ui::colors::TEXT_PRIMARY())) {
            ...
            set(0, auction.date, QColor(openmarketterminal::ui::colors::TEXT_SECONDARY()));
            set(1, is_open ? tr("OPEN") : tr("CLOSE"), type_color);
            set(2, time_str, QColor(openmarketterminal::ui::colors::TEXT_SECONDARY()));
            set(3, QString::number(entry.price, 'f', 2));
            set(4, QString::number(entry.size, 'f', 0), QColor(openmarketterminal::ui::colors::TEXT_SECONDARY()));
            set(5, entry.exchange, QColor(openmarketterminal::ui::colors::TEXT_SECONDARY()));
        }
    }
...
static void fill_trade_row(QTableWidget* table, int row, const trading::BrokerTrade& t,
    ...
    auto set = [&](int col, const QString& text, const QColor& fg = QColor(openmarketterminal::ui::colors::TEXT_PRIMARY())) {
        auto* item = new QTableWidgetItem(text);
        item->setForeground(fg);
        item->setFlags(Qt::ItemIsEnabled | Qt::ItemIsSelectable);
        table->setItem(row, col, item);
    ...
    set(0, time_str.isEmpty() ? t.timestamp.right(12) : time_str, QColor(openmarketterminal::ui::colors::TEXT_SECONDARY()));
    set(1, QString::number(t.price, 'f', 2),
        t.price > 0 ? QColor(openmarketterminal::ui::colors::TEXT_PRIMARY()) : QColor(openmarketterminal::ui::colors::TEXT_SECONDARY()));
    set(2, QString::number(t.size, 'f', 0), QColor(openmarketterminal::ui::colors::TEXT_SECONDARY()));
    set(3, t.exchange, QColor(openmarketterminal::ui::colors::TEXT_SECONDARY()));
    QStringList decoded;
    for (const auto& c : t.conditions)
        decoded.append(codes.contains(c) ? codes[c] : c);
    set(4, decoded.join(", "), QColor(openmarketterminal::ui::colors::TEXT_TERTIARY()));
    set(5, t.tape, QColor(openmarketterminal::ui::colors::TEXT_TERTIARY()));
}
...
void EquityBottomPanel::setup_calendar_tab() {
    ...
    banner->setStyleSheet(QString("QWidget#calClockBanner { background: %1; border-bottom: 1px solid %2; }")
                              .arg(openmarketterminal::ui::colors::BG_BASE())
                              .arg(openmarketterminal::ui::colors::BORDER_DIM()));
    ...
    clock_status_label_->setStyleSheet(
        QString("color: %1; font-size: 11px; font-weight: 700;").arg(openmarketterminal::ui::colors::TEXT_TERTIARY()));

    clock_next_label_ = new QLabel("");
    clock_next_label_->setObjectName("calClockNext");
    clock_next_label_->setStyleSheet(QString("color: %1; font-size: 10px;").arg(openmarketterminal::ui::colors::TEXT_TERTIARY()));
...
void EquityBottomPanel::set_calendar(const QVector<trading::MarketCalendarDay>& days) {
    ...
    for (int i = 0; i < days.size(); ++i) {
        ...
        auto set = [&](int col, const QString& text) {
            ...
            item->setText(text);
            if (is_today) {
                item->setForeground(QColor(openmarketterminal::ui::colors::AMBER()));
...
void EquityBottomPanel::set_clock(const trading::MarketClock& clock) {
    ...
    if (clock.is_open) {
        ...
        clock_status_label_->setStyleSheet(
            QString("color: %1; font-size: 11px; font-weight: 700;").arg(openmarketterminal::ui::colors::POSITIVE()));
    } else {
        clock_status_label_->setText(tr("● MARKET CLOSED"));
        clock_status_label_->setStyleSheet(
            QString("color: %1; font-size: 11px; font-weight: 700;").arg(openmarketterminal::ui::colors::NEGATIVE()));
    }
    ...
    if (!clock.next_open.isEmpty() || !clock.next_close.isEmpty()) {
        ...
        clock_next_label_->setStyleSheet(
            QString("color: %1; font-size: 10px;").arg(openmarketterminal::ui::colors::TEXT_SECONDARY()));
    }
}
