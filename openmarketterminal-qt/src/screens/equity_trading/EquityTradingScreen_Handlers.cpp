...
void EquityTradingScreen::open_chart_order_ticket(bool is_buy, double price) {
    ...
    hdr->setStyleSheet(QString("font-weight:700;font-size:13px;color:%1;")
                           .arg(is_buy ? openmarketterminal::ui::colors::POSITIVE() : openmarketterminal::ui::colors::NEGATIVE()));
    form->addRow(hdr);
...
void EquityTradingScreen::refresh_paper_panels() {
    ...
    try {
        ...
        const QString sym = currency_symbol(portfolio.currency.isEmpty() ? QStringLiteral("USD") : portfolio.currency);
        ...
        bottom_panel_->set_currency(sym); // before positions so the P&L summary formats correctly
