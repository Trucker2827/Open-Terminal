...
EquityChart::EquityChart(QWidget* parent) : QWidget(parent) {
    ...
    // --- Overlay engine ---
    overlay_mgr_ = new openmarketterminal::ui::ChartOverlayManager(this);
    ...
    indicator_picker_ = new openmarketterminal::ui::IndicatorPicker(overlay_mgr_, this);
    layout->insertWidget(layout->indexOf(chart_view_), indicator_picker_);

    connect(indicator_picker_, &openmarketterminal::ui::IndicatorPicker::indicator_requested,
    ...
    connect(indicator_picker_, &openmarketterminal::ui::IndicatorPicker::indicator_removed,
            this, [this](const QString& id) {
...
void EquityChart::set_candles(const QVector<trading::BrokerCandle>& candles) {
    ...
    QVector<openmarketterminal::ui::CandleData> synth_candles;
...
void EquityChart::set_position_line(double price, const QColor& color, const QString& label) {
    if (!overlay_mgr_ || price <= 0.0)
        return;
    if (!pos_line_) {
        pos_line_ = new openmarketterminal::ui::HorizontalLineLayer(QStringLiteral("__position"),
                                                         QStringLiteral("Position"));
        overlay_mgr_->add_layer(pos_line_);
    }
    openmarketterminal::ui::HorizontalLevel lvl;
    lvl.price = price;
    lvl.label = label;
    lvl.color = color;
