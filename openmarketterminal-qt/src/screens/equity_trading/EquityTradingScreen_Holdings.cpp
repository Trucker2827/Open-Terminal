...
void EquityTradingScreen::on_import_holdings_requested(const QVector<trading::BrokerHolding>& holdings) {
    ...
    auto& ps = openmarketterminal::services::PortfolioService::instance();
    QPointer<QComboBox> combo_guard = existing_combo;
    QPointer<QRadioButton> existing_guard = mode_existing;
    auto conn = QObject::connect(
        &ps, &openmarketterminal::services::PortfolioService::portfolios_loaded, &dlg,
        [combo_guard, existing_guard](const QVector<openmarketterminal::portfolio::Portfolio>& list) {
    ...
    auto do_add_assets = [rows](const QString& portfolio_id) {
        auto& svc = openmarketterminal::services::PortfolioService::instance();
    ...
    if (mode_new->isChecked()) {
        ...
        *once = connect(&ps, &openmarketterminal::services::PortfolioService::portfolio_created, this,
                        [self, once, do_add_assets, name, count](const openmarketterminal::portfolio::Portfolio& p) {
                            if (!self || p.name != name)
                                return;
