...
EquityOrderEntry::EquityOrderEntry(QWidget* parent) : QWidget(parent) {
    ...
    // Generic placeholder products shown before a broker profile loads; replaced
    // by the broker's own product_types in configure_for_broker().
    product_combo_->addItems({tr("Intraday"), tr("Delivery"), tr("Margin")});
