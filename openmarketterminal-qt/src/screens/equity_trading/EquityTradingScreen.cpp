...
void EquityTradingScreen::showEvent(QShowEvent* event) {
    ...
    openmarketterminal::feeds::FeedMonitor::instance().resume_all();
...
void EquityTradingScreen::hideEvent(QHideEvent* event) {
    ...
    openmarketterminal::feeds::FeedMonitor::instance().pause_all();
...
void EquityTradingScreen::setup_ui() {
    ...
    // Exchange label — seed from the current default (NASDAQ); updated to the
    // focused account's exchange in on_account_changed.
    exchange_label_ = new QLabel(selected_exchange_);
        ...
        popup->setStyleSheet(
            QString("QAbstractItemView{background:%1;color:%2;border:1px solid %3;outline:0;padding:2px;"
                ...
                .arg(openmarketterminal::ui::colors::BG_SURFACE(), openmarketterminal::ui::colors::TEXT_PRIMARY(),
                     openmarketterminal::ui::colors::BORDER_MED(), openmarketterminal::ui::colors::BG_HOVER()));
    ...
    feed_panel_ = new openmarketterminal::feeds::FeedPanel;
...
void EquityTradingScreen::on_replicate_portfolio_requested() {
    openmarketterminal::screens::PortfolioReplicationDialog dlg(this);
    dlg.exec();
