...
EquityWatchlist::EquityWatchlist(QWidget* parent) : QWidget(parent) {
    ...
    table_->setStyleSheet(
        QString("QTableWidget { background:%1; color:%2; border:none; gridline-color:%3;"
            ...
            .arg(openmarketterminal::ui::colors::BG_BASE(), openmarketterminal::ui::colors::TEXT_PRIMARY(),
                 openmarketterminal::ui::colors::BORDER_DIM())
            .arg(openmarketterminal::ui::fonts::SMALL)
            .arg(openmarketterminal::ui::fonts::DATA_FAMILY(), openmarketterminal::ui::colors::BG_HOVER(),
                 openmarketterminal::ui::colors::BG_RAISED(), openmarketterminal::ui::colors::TEXT_TERTIARY()));
...
void EquityWatchlist::update_quote(const trading::BrokerQuote& quote) {
    ...
    for (int row = 0; row < table_->rowCount(); ++row) {
        ...
        if (auto* chg_item = table_->item(row, 2)) {
            ...
            chg_item->setForeground(entry->change_pct >= 0 ? QColor(openmarketterminal::ui::colors::POSITIVE())
                                                           : QColor(openmarketterminal::ui::colors::NEGATIVE()));
        }
        break;
    }
}
...
void EquityWatchlist::set_active_symbol(const QString& symbol) {
    ...
    for (int row = 0; row < table_->rowCount(); ++row) {
        ...
        for (int col = 0; col < 3; ++col) {
            ...
            if (cell) {
                cell->setBackground(active ? QColor(openmarketterminal::ui::colors::BG_HOVER())
                                           : QColor(openmarketterminal::ui::colors::BG_BASE()));
            }
        }
    }
}
...
void EquityWatchlist::rebuild_table() {
    ...
    for (int i = 0; i < entries_.size(); ++i) {
        ...
        sym_item->setForeground(QColor(openmarketterminal::ui::colors::TEXT_PRIMARY()));
        ...
        ltp_item->setForeground(QColor(openmarketterminal::ui::colors::TEXT_PRIMARY()));
        ...
        chg_item->setForeground(e.change_pct >= 0 ? QColor(openmarketterminal::ui::colors::POSITIVE())
                                                  : QColor(openmarketterminal::ui::colors::NEGATIVE()));
...
void EquityWatchlist::on_add_symbol_entered() {
    ...
    if (any_loaded(ids)) {
        ...
        if (results.isEmpty()) {
            ...
            add_edit_->setStyleSheet(QString("border: 1px solid %1;").arg(openmarketterminal::ui::colors::NEGATIVE()));
