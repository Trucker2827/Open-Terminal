...
FeedPanel::FeedPanel(QWidget* parent) : QWidget(parent) {
    ...
    setStyleSheet(QString("#feedPanel{background:%1;}").arg(openmarketterminal::ui::colors::BG_BASE()));
    ...
    bar->setStyleSheet(QString("#feedToolbar{background:%1;border-bottom:1px solid %2;}")
                           .arg(openmarketterminal::ui::colors::BG_RAISED(), openmarketterminal::ui::colors::BORDER_DIM()));
    auto* bl = new QHBoxLayout(bar);
    bl->setContentsMargins(6, 0, 6, 0);
    auto* heading = new QLabel(tr("FEEDS"));
    heading->setStyleSheet(QString("color:%1;font-size:10px;font-weight:700;").arg(openmarketterminal::ui::colors::TEXT_TERTIARY()));
...
void FeedPanel::rebuild() {
    ...
    if (idx == 0 && floating_.isEmpty()) {
        ...
        empty->setStyleSheet(QString("color:%1;font-size:11px;").arg(openmarketterminal::ui::colors::TEXT_TERTIARY()));
