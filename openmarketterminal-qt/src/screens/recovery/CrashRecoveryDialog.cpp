...
CrashRecoveryDialog::CrashRecoveryDialog(openmarketterminal::CrashRecovery* recovery,
                                         openmarketterminal::WorkspaceSnapshotRing* ring,
                                         QWidget* parent)
    : QDialog(parent), recovery_(recovery), ring_(ring) {
    setWindowTitle(tr("OpenMarketTerminal — Recover Previous Session"));
...
void CrashRecoveryDialog::apply_styles() {
    // OpenMarketTerminal-terminal aesthetic: zero radii, sharp 1px borders, monospaced
    ...
    setStyleSheet(QString(R"(
        ...
        /* Title bar — amber underline like a OpenMarketTerminal header */
...
QString CrashRecoveryDialog::display_name(
    const openmarketterminal::WorkspaceSnapshotRing::Entry& e) const {
    if (!e.name.isEmpty())
        return e.name;
