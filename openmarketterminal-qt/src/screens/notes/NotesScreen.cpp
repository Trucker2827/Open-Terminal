...
void NotesScreen::load_notes() {
    auto r = openmarketterminal::NotesRepository::instance().list_all();
    if (r.is_ok()) {
        notes_ = r.value();
    } else {
        notes_.clear();
...
void NotesScreen::on_save_note() {
    ...
    openmarketterminal::FinancialNote note;
    note.title = title;
    ...
    auto& repo = openmarketterminal::NotesRepository::instance();
...
void NotesScreen::on_delete_note() {
    ...
    openmarketterminal::NotesRepository::instance().remove(selected_note_id_);
    selected_note_id_ = -1;
    right_stack_->setCurrentIndex(0);
    load_notes();
}
...
void NotesScreen::on_toggle_favorite() {
    if (selected_note_id_ <= 0)
        return;
    openmarketterminal::NotesRepository::instance().toggle_favorite(selected_note_id_);
    load_notes();
    // Re-show updated note
    auto r = openmarketterminal::NotesRepository::instance().get(selected_note_id_);
    if (r.is_ok())
        show_note(r.value());
}
...
void NotesScreen::on_toggle_archive() {
    if (selected_note_id_ <= 0)
        return;
    openmarketterminal::NotesRepository::instance().toggle_archive(selected_note_id_);
    selected_note_id_ = -1;
    right_stack_->setCurrentIndex(0);
    load_notes();
}
...
void NotesScreen::show_note(const openmarketterminal::FinancialNote& note) {
