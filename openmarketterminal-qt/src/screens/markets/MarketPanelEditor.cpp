...
void MarketPanelEditor::build_ui() {
    ...
    // Transparency: show the data source / freshness of search results.
    search_badge_ = new ui::ProvenanceBadge(this);
    root->addWidget(search_badge_);
...
void MarketPanelEditor::fire_search(const QString& query) {
    ...
    if (!search_connected_) {
        ...
        connect(&svc, &services::MarketSearchService::provenance_updated, this,
                [this](openmarketterminal::connectors::Provenance prov) {
                    if (search_badge_)
                        search_badge_->set_provenance(prov);
                });
