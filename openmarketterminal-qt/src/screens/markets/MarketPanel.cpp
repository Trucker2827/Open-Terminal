...
void MarketPanel::update_config(const MarketPanelConfig& cfg) {
    // A new symbol set must clear the rate-limit backoff counter, otherwise a
    // panel that exhausted its auto-retries on the old symbols stays stuck on
    // the manual-retry message and never auto-retries the fresh symbols. This
    // is the reconfigure path only — the auto-retry loop goes through
    // refresh()/hub_resubscribe(), so the backoff is preserved there.
    if (cfg.symbols != config_.symbols)
        empty_retry_count_ = 0;
    config_ = cfg;
    if (config_.column_order.isEmpty())
        config_.column_order = default_market_columns();
...
void MarketPanel::rebuild_from_cache() {
    ...
    has_data_ = true;
    empty_retry_count_ = 0; // got data — reset the rate-limit backoff
    title_label_->setText(config_.title.toUpper());
    if (first_data)
        hide_loading();
    show_data();
...
void MarketPanel::hub_resubscribe() {
    ...
    for (const QString& sym : config_.symbols) {
        ...
        hub.subscribe(this, topic, [this, sym](const QVariant& v) {
            ...
            rebuild_from_cache();
            settle_symbol(sym); // data arrived → this symbol is done
        });
    }

    // Also listen for per-symbol fetch errors (e.g. Yahoo "Too Many Requests").
    // Without this the panel never learns a symbol failed and spins on LOADING
    // forever; settling on error lets a fully-failed panel show a retry state and
    // recover on the next auto-refresh / F5.
    ...
    hub.subscribe_pattern_errors(this, QStringLiteral("market:quote:*"),
                                 [this](const QString& topic, const QString&) {
        const QString sym = topic.mid(QStringLiteral("market:quote:").size());
        if (config_.symbols.contains(sym))
            settle_symbol(sym);
    });

    // Backstop: if neither data nor an error ever arrives for some symbols (a
    // dropped request), don't spin indefinitely — finalize after a grace period.
    QPointer<MarketPanel> guard = this;
    QTimer::singleShot(20000, this, [guard, gen]() {
        // Only finalize if this is still the current refresh — a newer
        // hub_resubscribe() bumps refresh_generation_, neutralizing this
        // stale backstop so it can't clip a fetch that's still progressing.
        if (guard && guard->refresh_inflight_ && guard->refresh_generation_ == gen) {
            guard->pending_initial_.clear();
            guard->finalize_refresh();
        }
    });
...
void MarketPanel::settle_symbol(const QString& sym) {
    if (!refresh_inflight_)
        return;
    pending_initial_.remove(sym);
    if (pending_initial_.isEmpty())
        finalize_refresh();
}

void MarketPanel::finalize_refresh() {
    refresh_inflight_ = false;
    // If not a single symbol delivered data, surface a recoverable state instead
    // of the perpetual spinner. Otherwise the (partial) table is already shown.
    if (!has_data_) {
        hide_loading();
        // Yahoo rate-limits bursts of ~100+ symbols, so a panel can come back empty
        // on the initial cold load even though its tickers are valid. Auto-retry a
        // few times with backoff so it self-heals within ~1-2 min instead of waiting
        // for the slow (minutes-long) auto-refresh tick or a manual [RETRY].
        if (empty_retry_count_ < kMaxEmptyRetries) {
            ++empty_retry_count_;
            // Backoff that caps at 60s, then keeps retrying gently (~1/min). A
            // single panel's ~12-symbol re-request is small; spacing attempts lets
            // Yahoo's per-IP throttle recover instead of hammering it.
            const int delay_ms = qMin(8000 * empty_retry_count_, 60000);
            show_error(tr("No data (rate-limited) — auto-retrying…"));
            QPointer<MarketPanel> guard = this;
            QTimer::singleShot(delay_ms, this, [guard]() {
                if (guard && !guard->has_data_)
                    guard->refresh();
            });
        } else {
            show_error(tr("No data — press [RETRY] or wait for refresh"));
        }
    }
    emit refresh_finished();
}
...
void MarketPanel::populate(const QVector<services::QuoteData>& quotes) {
    ...
    for (int row = 0; row < count; ++row) {
        ...
        for (int ci = 0; ci < cols.size(); ++ci) {
            ...
            else if (col == "VOL")    table_->setItem(row, ci, mk(openmarketterminal::ui::formatting::format_compact_volume(static_cast<qint64>(q.volume)), ui::colors::TEXT_DIM()));
...
void MarketPanel::show_row_context_menu(const QPoint& pos) {
    ...
    connect(backtest_act, &QAction::triggered, this, [it, ticker_at]() {
        ...
        openmarketterminal::services::backtest::BacktestingService::instance().set_pending_portfolio_config(config);
        openmarketterminal::EventBus::instance().publish("nav.switch_screen", {{"screen_id", QString("backtesting")}});
    });
