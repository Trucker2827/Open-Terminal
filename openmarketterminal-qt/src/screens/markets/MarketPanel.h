...
class MarketPanel : public QWidget {
  ...
  private:
    ...
    // Mark a symbol as settled (data arrived OR the producer published an error
    // for it) so a panel whose symbols all fail — e.g. Yahoo rate-limiting — stops
    // spinning instead of hanging on LOADING forever. Finalizes once all settle.
    void settle_symbol(const QString& sym);
    void finalize_refresh();
    ...
    // Bumped on every hub_resubscribe(). The 20s backstop timer captures the
    // generation it was scheduled under and only finalizes if it still matches,
    // so a stale backstop from an earlier refresh can't prematurely finalize a
    // newer in-flight one.
    quint64 refresh_generation_ = 0;
    bool hub_active_ = false;
    int  empty_retry_count_ = 0;     // consecutive auto-retries while a panel has no data
    static constexpr int kMaxEmptyRetries = 12;
