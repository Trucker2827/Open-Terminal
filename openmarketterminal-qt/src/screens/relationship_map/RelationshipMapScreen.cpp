...
using namespace openmarketterminal::relmap;
...
void RelationshipMapScreen::build_ui() {
    ...
    status_brand_ = new QLabel(tr("OPENMARKETTERMINAL"));
    ...
    connect(scene_, &relmap::RelationshipGraphScene::center_card_clicked,
            this, [](const QString& ticker) {
                openmarketterminal::EventBus::instance().publish("equity_research.load_symbol",
...
void RelationshipMapScreen::fire_asset_search(const QString& query) {
    auto& svc = openmarketterminal::services::MarketSearchService::instance();
    const QString rid = QString::number(reinterpret_cast<quintptr>(this), 16);
    if (!search_connected_) {
        connect(&svc, &openmarketterminal::services::MarketSearchService::results_ready, this,
                [this](const QString& request_id, const QString& q,
                       const QList<openmarketterminal::services::MarketSearchService::Item>& items) {
...
void RelationshipMapScreen::on_asset_results(
    const QList<openmarketterminal::services::MarketSearchService::Item>& results) {
...
void RelationshipMapScreen::retranslateUi() {
    ...
    if (status_brand_) status_brand_->setText(tr("OPENMARKETTERMINAL"));
