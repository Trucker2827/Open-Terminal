...
static QVector<QPair<QString, int>> range_options_for(const QString& tf) {
    ...
    const int cap = openmarketterminal::services::algo::algo_default_lookback_days(tf);
...
void ScannerPanel::connect_service() {
    auto& scanner = openmarketterminal::algo::AlgoScanner::instance();
    connect(&scanner, &openmarketterminal::algo::AlgoScanner::scan_complete, this, &ScannerPanel::on_scan_result);
    connect(&scanner, &openmarketterminal::algo::AlgoScanner::scan_error, this, [this](const QString& err) {
...
void ScannerPanel::build_ui() {
    ...
    section_ = new openmarketterminal::ui::algo::ConditionSection(
        openmarketterminal::ui::algo::ConditionSection::Type::Entry, left_col);
    ...
    symbols_input_ = new openmarketterminal::ui::algo::SymbolChipInput(right_col);
    ...
    auto accounts = openmarketterminal::trading::AccountManager::instance().list_accounts();
    ...
    create_alert_btn->setStyleSheet(QString("QPushButton { background:transparent; color:%1;"
        ...
        .arg(openmarketterminal::ui::colors::TEXT_SECONDARY(), openmarketterminal::ui::colors::BORDER_DIM())
        .arg(openmarketterminal::ui::fonts::DATA).arg(QString("font-family:%1;").arg(openmarketterminal::ui::fonts::DATA_FAMILY))
        .arg(openmarketterminal::ui::colors::AMBER()));
...
void ScannerPanel::on_scan() {
    ...
    auto source = openmarketterminal::algo::data_source_from_string(
        data_source_combo_->currentData().toString());

    QString broker_id, account_id;
    if (source != openmarketterminal::algo::DataSource::YFinance) {
        account_id = account_combo_->currentData().toString();
        if (!account_id.isEmpty()) {
            auto acct = openmarketterminal::trading::AccountManager::instance().get_account(account_id);
    ...
    openmarketterminal::algo::AlgoScanner::instance().scan(
...
void ScannerPanel::on_scan_result(const QJsonObject& payload) {
    ...
    status_label_->setStyleSheet(
        QString("color: %1; font-size: %2px; %3 background: transparent; border: none;")
            .arg(openmarketterminal::ui::colors::POSITIVE())
            .arg(openmarketterminal::ui::fonts::SMALL)
            .arg(kMonoFont()));
    ...
    for (const auto& mval : matches) {
        ...
        if (bullish_count > 0 && bearish_count == 0) {
            ...
            signal_color = openmarketterminal::ui::colors::POSITIVE();
        } else if (bearish_count > 0 && bullish_count == 0) {
            signal_text  = tr("BEARISH");
            signal_color = openmarketterminal::ui::colors::NEGATIVE();
        } else {
            signal_text  = tr("NEUTRAL");
            signal_color = openmarketterminal::ui::colors::TEXT_SECONDARY();
        }
        ...
        sym_item->setForeground(QColor(openmarketterminal::ui::colors::AMBER()));
        sym_item->setFont(QFont(openmarketterminal::ui::fonts::DATA_FAMILY(),
                                openmarketterminal::ui::fonts::SMALL, QFont::Bold));
        ...
        sig_item->setFont(QFont(openmarketterminal::ui::fonts::DATA_FAMILY(),
                                openmarketterminal::ui::fonts::TINY, QFont::Bold));
        ...
        match_item->setForeground(QColor(openmarketterminal::ui::colors::POSITIVE()));
        ...
        tf_item->setForeground(QColor(openmarketterminal::ui::colors::CYAN()));
        ...
        detail_item->setForeground(QColor(openmarketterminal::ui::colors::TEXT_SECONDARY()));
