...
namespace {
QString uMono() { return QString("font-family: %1;").arg(openmarketterminal::ui::fonts::DATA_FAMILY); }
QString uLabel() {
...
QString uCombo() {
    return QString("QComboBox { background: %1; color: %2; border: 1px solid %3; padding: 4px 8px;"
        ...
        .arg(openmarketterminal::ui::colors::BG_SURFACE(), openmarketterminal::ui::colors::TEXT_PRIMARY(),
             openmarketterminal::ui::colors::BORDER_DIM())
...
UniverseScannerPanel::UniverseScannerPanel(QWidget* parent) : QWidget(parent) {
    ...
    connect(&openmarketterminal::algo::ScanMonitor::instance(), &openmarketterminal::algo::ScanMonitor::realtime_match,
    ...
    if (auto active = openmarketterminal::ScanWatchRepository::instance().list_active(); active.is_ok())
        for (const auto& w : active.value())
            if (w.mode == QStringLiteral("realtime"))
                openmarketterminal::ScanWatchRepository::instance().remove(w.id);
...
void UniverseScannerPanel::on_start_stop() {
    if (running_) {
        ...
        openmarketterminal::algo::ScanMonitor::instance().stop_watch(watch_id_);
        if (!watch_id_.isEmpty())
            openmarketterminal::ScanWatchRepository::instance().remove(watch_id_);
        watch_id_.clear();
        running_ = false;
        start_btn_->setText(tr("START SCAN"));
        set_status(tr("Stopped."), openmarketterminal::ui::colors::TEXT_TERTIARY());
        return;
    }

    AlgoStrategy strat;
    if (!current_strategy(&strat)) {
        set_status(tr("Select a strategy first."), openmarketterminal::ui::colors::NEGATIVE());
        return;
    }
    if (strat.entry_conditions.isEmpty()) {
        set_status(tr("The selected strategy has no entry conditions."),
                   openmarketterminal::ui::colors::NEGATIVE());
        return;
    }
    ...
    w.broker_id = openmarketterminal::trading::AccountManager::instance().get_account(account_id).broker_id;
    ...
    openmarketterminal::algo::ScanMonitor::instance().reload(watch_id_);
...
void UniverseScannerPanel::on_match(const QString& watch_id, const QString& symbol,
    ...
    QString sig = tr("MATCH"), sig_color = openmarketterminal::ui::colors::TEXT_SECONDARY();
    if (detail.contains('>') || detail.contains("crosses_above")) {
        sig = tr("BULLISH");
        sig_color = openmarketterminal::ui::colors::POSITIVE();
    } else if (detail.contains('<') || detail.contains("crosses_below")) {
        sig = tr("BEARISH");
        sig_color = openmarketterminal::ui::colors::NEGATIVE();
    }
...
void UniverseScannerPanel::deploy_symbol(const QString& symbol) {
    ...
    if (dlg->exec() == QDialog::Accepted) {
        ...
        openmarketterminal::algo::AlgoEngine::instance().start_deployment(dep, strat);
        set_status(tr("Deployed %1.").arg(symbol), openmarketterminal::ui::colors::POSITIVE());
    }
    dlg->deleteLater();
}
