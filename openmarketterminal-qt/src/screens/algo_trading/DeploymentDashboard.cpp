...
inline QString kMonoFont() {
    return QString("font-family: %1;").arg(openmarketterminal::ui::fonts::DATA_FAMILY);
}
...
inline QString currency_symbol(const QString& broker_id) {
    if (broker_id.isEmpty())
        return cur::symbol();
    auto* broker = openmarketterminal::trading::BrokerRegistry::instance().get(broker_id);
...
namespace algo_ns = openmarketterminal::algo;
...
static QWidget* build_stat_card(const QString& label, const QString& value, const QString& color, QLabel** out_label,
    ...
    card->setStyleSheet(QString("background: %1; border: 1px solid %2;")
                            .arg(openmarketterminal::ui::colors::BG_SURFACE(), openmarketterminal::ui::colors::BORDER_DIM()));
    ...
    lbl->setStyleSheet(QString("color: %1; font-size: %2px; font-weight: 700; letter-spacing: 0.5px; %3"
                               " background: transparent; border: none;")
                           .arg(openmarketterminal::ui::colors::TEXT_TERTIARY())
                           .arg(openmarketterminal::ui::fonts::TINY)
                           .arg(kMonoFont()));
    vl->addWidget(lbl);
    ...
    val_lbl->setStyleSheet(QString("color: %1; font-size: %2px; font-weight: 700; %3"
                                   " background: transparent; border: none;")
                               .arg(color)
                               .arg(openmarketterminal::ui::fonts::TITLE)
                               .arg(kMonoFont()));
...
QWidget* DeploymentDashboard::build_deployment_card(const AlgoDeployment& d, QWidget* parent) {
    ...
    name_lbl->setStyleSheet(QString("color: %1; font-size: %2px; font-weight: 700; %3"
                                    " background: transparent; border: none;")
                                .arg(openmarketterminal::ui::colors::AMBER())
                                .arg(openmarketterminal::ui::fonts::DATA)
                                .arg(kMonoFont()));
    ...
    tf_lbl->setStyleSheet(QString("color: %1; font-size: %2px; %3"
                              ...
                              .arg(openmarketterminal::ui::colors::CYAN())
                              .arg(openmarketterminal::ui::fonts::TINY)
                              .arg(kMonoFont()));
    ...
    mode_badge->setStyleSheet(QString("color: %1; font-size: %2px; font-weight: 700; %3"
                                  ...
                                  .arg(is_paper ? openmarketterminal::ui::colors::POSITIVE() : openmarketterminal::ui::colors::AMBER())
                                  .arg(openmarketterminal::ui::fonts::TINY)
                                  .arg(kMonoFont())
    ...
    if (!d.broker_id.isEmpty()) {
        const auto cstate = openmarketterminal::trading::AccountManager::instance().connection_state(d.broker_account_id);
        const bool connected = (cstate == openmarketterminal::trading::ConnectionState::Connected);
        auto* broker_badge = new QLabel(
            d.broker_id.toUpper() + (connected ? QStringLiteral("  ● ") : QStringLiteral("  ○ ")) +
                (connected ? tr("CONNECTED") : tr("OFFLINE")),
            card);
        const QString clr = connected ? openmarketterminal::ui::colors::POSITIVE() : openmarketterminal::ui::colors::NEGATIVE();
    ...
    side_badge->setStyleSheet(QString("color: %1; font-size: %2px; font-weight: 700; %3"
                                      " padding: 2px 6px; background: rgba(%4,0.08);"
                                      " border: 1px solid rgba(%4,0.25);")
                                  .arg(is_sell ? openmarketterminal::ui::colors::NEGATIVE() : openmarketterminal::ui::colors::POSITIVE())
                                  .arg(openmarketterminal::ui::fonts::TINY)
                                  .arg(kMonoFont())
    ...
    add_metric(tr("LTP"),
               d.current_price > 0 ? QString("%1%2").arg(cs).arg(d.current_price, 0, 'f', 2)
                                   : QStringLiteral("—"),
               openmarketterminal::ui::colors::CYAN, openmarketterminal::ui::fonts::TITLE, &h.ltp);
    add_metric(tr("P&L"), QString("%1%2%3").arg(total_pnl >= 0 ? "+" : "-", cs).arg(std::abs(total_pnl), 0, 'f', 2),
               total_pnl >= 0 ? openmarketterminal::ui::colors::POSITIVE : openmarketterminal::ui::colors::NEGATIVE,
               openmarketterminal::ui::fonts::TITLE, &h.pnl);
    add_metric(tr("WIN RATE"), QString("%1%").arg(d.win_rate, 0, 'f', 1), openmarketterminal::ui::colors::TEXT_PRIMARY,
               openmarketterminal::ui::fonts::SMALL, nullptr);
    add_metric(tr("POSITION"), tr("FLAT"), openmarketterminal::ui::colors::TEXT_TERTIARY, openmarketterminal::ui::fonts::SMALL, &h.position);
    add_metric(tr("TRADES"), QString::number(d.total_trades), openmarketterminal::ui::colors::TEXT_PRIMARY,
               openmarketterminal::ui::fonts::SMALL, &h.trades);
    add_metric(tr("MAX DD"), QString("-%1%").arg(std::abs(d.max_drawdown), 0, 'f', 2), openmarketterminal::ui::colors::NEGATIVE,
               openmarketterminal::ui::fonts::SMALL, nullptr);
    ...
    win_bar->setStyleSheet(
        QString("QProgressBar { background: %1; border: none; border-radius: 2px; }"
            ...
            .arg(openmarketterminal::ui::colors::BG_RAISED(), openmarketterminal::ui::colors::POSITIVE()));
...
void DeploymentDashboard::update_summary(const QVector<AlgoDeployment>& deployments) {
    ...
    if (total_pnl_) {
        ...
        total_pnl_->setStyleSheet(QString("color: %1; font-size: %2px; font-weight: 700; %3"
                                          " background: transparent; border: none;")
                                      .arg(pnl_sum >= 0 ? openmarketterminal::ui::colors::POSITIVE() : openmarketterminal::ui::colors::NEGATIVE())
                                      .arg(openmarketterminal::ui::fonts::TITLE)
                                      .arg(kMonoFont()));
    }
...
void DeploymentDashboard::build_ui() {
    ...
    scroll->setStyleSheet(QString("QScrollArea { background: %1; border: none; }"
                              ...
                              .arg(openmarketterminal::ui::colors::BG_BASE(), openmarketterminal::ui::colors::BORDER_MED()));
    ...
    summary_row->addWidget(build_stat_card(tr("ACTIVE DEPLOYMENTS"), "0", openmarketterminal::ui::colors::CYAN, &active_count_,
                                           content, &active_caption_));
    summary_row->addWidget(build_stat_card(tr("TOTAL P&L"), "0.00", openmarketterminal::ui::colors::POSITIVE, &total_pnl_,
                                           content, &total_pnl_caption_));
    summary_row->addWidget(build_stat_card(tr("TOTAL TRADES"), "0", openmarketterminal::ui::colors::TEXT_PRIMARY, &total_trades_,
                                           content, &total_trades_caption_));
    summary_row->addWidget(build_stat_card(tr("AVG WIN RATE"), "0.0%", openmarketterminal::ui::colors::TEXT_PRIMARY, &avg_win_rate_,
...
void DeploymentDashboard::on_live_update(const QString& deployment_id,
                                         const openmarketterminal::algo::AlgoLiveSnapshot& snap) {
    ...
    if (h.pnl) {
        ...
        h.pnl->setStyleSheet(QString("color: %1; font-size: %2px; font-weight: 700; %3"
                                     " background: transparent; border: none;")
                                 .arg(total >= 0 ? openmarketterminal::ui::colors::POSITIVE() : openmarketterminal::ui::colors::NEGATIVE())
                                 .arg(openmarketterminal::ui::fonts::TITLE)
                                 .arg(kMonoFont()));
    }
    ...
    if (h.conditions) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            const QString clr = green ? openmarketterminal::ui::colors::POSITIVE()
                                      : (hint.isEmpty() ? openmarketterminal::ui::colors::TEXT_SECONDARY()
                                                        : openmarketterminal::ui::colors::WARNING());
