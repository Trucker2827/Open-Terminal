...
void StrategyBuilderPanel::on_instrument_type_changed() {
    // Equity is the only supported instrument type; keep the backtest setup card
    // visible. (Option/Future strategies were part of the removed F&O subsystem.)
    if (bt_setup_card_)
...
services::algo::AlgoStrategy StrategyBuilderPanel::build_strategy() {
    ...
    strat.instrument_type = QStringLiteral("equity");
    return strat;
}
...
void StrategyBuilderPanel::load_strategy(const services::algo::AlgoStrategy& s) {
    ...
    // Equity is the only supported instrument type now (F&O removed); the combo
    // is fixed on "Equity".
