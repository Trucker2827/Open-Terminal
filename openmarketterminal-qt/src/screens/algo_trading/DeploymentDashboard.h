...
class DeploymentDashboard : public QWidget {
  ...
  private slots:
    void on_deployments_loaded(QVector<openmarketterminal::services::algo::AlgoDeployment> deployments);
    void on_live_update(const QString& deployment_id, const openmarketterminal::algo::AlgoLiveSnapshot& snap);
    void on_error(const QString& context, const QString& msg);

  private:
    void build_ui();
    void connect_service();
    QWidget* build_deployment_card(const openmarketterminal::services::algo::AlgoDeployment& d, QWidget* parent);
    void update_summary(const QVector<openmarketterminal::services::algo::AlgoDeployment>& deployments);
    void refresh_ages();
    void retranslateUi();
