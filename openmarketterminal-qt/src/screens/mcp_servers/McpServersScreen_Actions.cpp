...
static const QList<openmarketterminal::mcp::MarketplaceEntry>& g_catalog = openmarketterminal::mcp::marketplace_catalog();
...
void McpServersScreen::refresh_tools() {
    ...
    for (const auto& t : internal_tools) {
        ...
        // "OpenMarketTerminal" is the product name — kept in English for brand consistency.
        auto* si = new QTableWidgetItem(QStringLiteral("OpenMarketTerminal"));
