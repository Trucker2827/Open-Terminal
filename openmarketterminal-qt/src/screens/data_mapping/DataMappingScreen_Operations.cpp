...
using namespace openmarketterminal::screens::data_mapping_internal;
...
void DataMappingScreen::on_test_api() {
    ...
    using openmarketterminal::services::DataMappingTestClient;
...
void DataMappingScreen::on_run_mapping() {
    ...
    openmarketterminal::services::DataNormalizationService::instance().fetch_and_normalize(
        dm, [self, dm](bool ok, openmarketterminal::services::NormalizedRecord rec) {
            if (!self)
                return;
            if (ok) {
