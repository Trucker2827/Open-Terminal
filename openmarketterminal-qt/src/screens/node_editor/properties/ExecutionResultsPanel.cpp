...
static QWidget* make_result_card(const NodeExecutionResult& result) {
    ...
    // Honest status color/text: Ok=green, Failed=red, Unavailable=amber, Demo=amber.
    // Renders by result.kind so a non-Ok outcome can never paint green.
    QString status_color;
    QString status_text;
    switch (result.kind) {
    case openmarketterminal::OutcomeKind::Ok:
        status_color = ui::colors::POSITIVE.get();
        status_text = "OK";
        break;
    case openmarketterminal::OutcomeKind::Skipped:
        status_color = ui::colors::TEXT_TERTIARY.get(); // neutral/grey — not green
        status_text = "SKIPPED";
        break;
    case openmarketterminal::OutcomeKind::Unavailable:
        status_color = ui::colors::WARNING.get();
        status_text = "UNAVAILABLE";
        break;
    case openmarketterminal::OutcomeKind::Demo:
        status_color = ui::colors::WARNING.get();
        status_text = "DEMO";
        break;
    case openmarketterminal::OutcomeKind::Failed:
    default:
        status_color = ui::colors::NEGATIVE.get();
        status_text = "FAILED";
        break;
    }
    ...
    card->setStyleSheet(QString("QWidget#resultCard {"
                                ...
                                "  margin: 0;"
                                "}")
                            .arg(ui::colors::BG_HOVER(), status_color, ui::colors::BORDER_MED()));
    ...
    // Status badge — text reflects the honest OutcomeKind (sizes to content so
    // "UNAVAILABLE"/"DEMO" are not clipped).
    auto* badge = new QLabel(status_text);
