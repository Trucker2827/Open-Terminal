...
QWidget* ParameterWidgetFactory::create(const ParamDef& param, const QJsonValue& current_value,
    ...
    if (param.type == "string" || param.type == "expression") {
        ...
        auto populate = [combo, allowed_exts](const QString& select_path = {}) {
            ...
            auto& svc = openmarketterminal::services::FileManagerService::instance();
        ...
        QObject::connect(import_btn, &QPushButton::clicked, container,
                         [populate, dialog_filter, key, on_change, import_btn]() {
                             ...
                             QString file_id =
                                 openmarketterminal::services::FileManagerService::instance().import_file(path, "workflow_node");
                             if (file_id.isEmpty())
                                 return;

                             // Find the full managed path for the newly imported file
                             auto f = openmarketterminal::services::FileManagerService::instance().find_by_id(file_id);
                             QString full_path = openmarketterminal::services::FileManagerService::instance().full_path(f.name);
        ...
        auto populate_agents = [combo, current_value]() {
            ...
            const auto agents = openmarketterminal::services::AgentService::instance().cached_agents();
        ...
        auto trigger_discovery = [populate_agents, combo]() {
            auto& svc = openmarketterminal::services::AgentService::instance();
            QObject::connect(&svc, &openmarketterminal::services::AgentService::agents_discovered, combo,
                             [populate_agents, combo](const QVector<openmarketterminal::services::AgentInfo>&,
                                                      const QVector<openmarketterminal::services::AgentCategory>&) {
                                 populate_agents();
                                 QObject::disconnect(&openmarketterminal::services::AgentService::instance(),
                                                     &openmarketterminal::services::AgentService::agents_discovered, combo,
                                                     nullptr);
                             });
            svc.discover_agents();
        };
        ...
        QString saved = current_value.toString();
        auto res = openmarketterminal::LlmProfileRepository::instance().list_profiles();
        if (res.is_ok()) {
        ...
        auto* hint = new QLabel(QCoreApplication::translate(
            "ParameterWidgetFactory", "All OpenMarketTerminal internal tools. Input JSON flows in as arguments."));
        hint->setStyleSheet(QString("color:%1; font-family:Consolas; font-size:10px;").arg(ui::colors::TEXT_TERTIARY()));
        hint->setWordWrap(true);
        layout->addWidget(hint);
