...
void NodeCanvas::dragEnterEvent(QDragEnterEvent* event) {
    if (event->mimeData()->hasFormat("application/x-openmarketterminal-node-type")) {
...
void NodeCanvas::dropEvent(QDropEvent* event) {
    if (event->mimeData()->hasFormat("application/x-openmarketterminal-node-type")) {
        QString type_id = QString::fromUtf8(event->mimeData()->data("application/x-openmarketterminal-node-type"));
