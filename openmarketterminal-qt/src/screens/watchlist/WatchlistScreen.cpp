...
QWidget* WatchlistScreen::build_main_panel() {
    ...
    symbol_dnd::installDropFilter(main_panel_, [this](const SymbolRef& ref, SymbolGroup) {
        if (current_wl_id_.isEmpty() || !ref.is_valid())
            return;
        openmarketterminal::WatchlistRepository::instance().add_stock(current_wl_id_, ref.symbol);
        load_stocks();
    });
...
void WatchlistScreen::load_watchlists() {
    auto r = openmarketterminal::WatchlistRepository::instance().list_all();
    if (r.is_err()) {
    ...
    if (watchlists_.isEmpty()) {
        QColor acc(colors::AMBER());
        auto cr = openmarketterminal::WatchlistRepository::instance().create("Default", acc.name());
        if (cr.is_ok())
...
void WatchlistScreen::load_stocks() {
    ...
    auto r = openmarketterminal::WatchlistRepository::instance().get_stocks(current_wl_id_);
    if (r.is_err()) {
        stocks_.clear();
    } else {
        stocks_ = r.value();
    ...
    for (const auto& s : stocks_) {
        ...
        if (it != quote_map.end()) {
            ...
            table_->add_row({q.symbol, q.name.isEmpty() ? s.name : q.name,
                             ...
                             openmarketterminal::ui::formatting::format_compact_volume(static_cast<qint64>(q.volume))});
...
void WatchlistScreen::on_add_watchlist() {
    ...
    auto r = openmarketterminal::WatchlistRepository::instance().create(name.trimmed());
    if (r.is_ok()) {
        load_watchlists();
...
void WatchlistScreen::on_delete_watchlist() {
    ...
    openmarketterminal::WatchlistRepository::instance().remove(current_wl_id_);
...
void WatchlistScreen::on_remove_stock() {
    ...
    openmarketterminal::WatchlistRepository::instance().remove_stock(current_wl_id_, symbol);
    load_stocks();
}
