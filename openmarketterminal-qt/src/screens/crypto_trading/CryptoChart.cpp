...
void CryptoChart::set_candles(const QVector<trading::Candle>& candles) {
    candles_ = candles;
    rebuild_chart();
    overlay_mgr_->set_candles(openmarketterminal::ui::CandleData::from_candles(candles_));
...
void CryptoChart::append_candle(const trading::Candle& candle) {
    ...
    if (!candles_.isEmpty())
        overlay_mgr_->append_candle(openmarketterminal::ui::CandleData::from(candles_.last()));
}
