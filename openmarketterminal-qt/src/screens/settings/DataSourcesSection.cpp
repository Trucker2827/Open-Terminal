...
QWidget* DataSourcesSection::build_content() {
    ...
    // ── Connector endpoints (free / self-hosted) ────────────────────────────
    // These transparent connectors read their endpoint/key directly from
    // settings — no vendor cloud. Edits save immediately.
    {
        auto* panel = make_panel(tr("CONNECTOR ENDPOINTS"));
        auto* pvl = qobject_cast<QVBoxLayout*>(panel->layout());

        auto add_row = [this, pvl](const QString& label, const QString& key,
            ...
            lab->setStyleSheet(QString("color:%1;font-weight:700;background:transparent;")
                                   .arg(ui::colors::TEXT_SECONDARY()));
            rl->addWidget(lab);
            auto* edit = new QLineEdit;
            edit->setPlaceholderText(placeholder);
            const auto cur = SettingsRepository::instance().get(key);
            if (cur.is_ok())
                edit->setText(cur.value());
            edit->setStyleSheet(
                QString("QLineEdit{background:%1;color:%2;border:1px solid %3;padding:5px 8px;}")
                    .arg(ui::colors::BG_BASE(), ui::colors::TEXT_PRIMARY(), ui::colors::BORDER_MED()));
            connect(edit, &QLineEdit::editingFinished, this, [edit, key]() {
                SettingsRepository::instance().set(key, edit->text().trimmed(), "connectors");
            });
            rl->addWidget(edit);
            auto* h = new QLabel(hint);
            h->setWordWrap(true);
            h->setStyleSheet(
                QString("color:%1;font-size:11px;background:transparent;").arg(ui::colors::TEXT_DIM()));
            rl->addWidget(h);
            pvl->addWidget(row);
        };

        add_row(tr("FRED API KEY"), QStringLiteral("connectors.fred_api_key"), tr("e.g. abcd1234…"),
                tr("Free key from fredaccount.stlouisfed.org — powers the economic calendar + FRED data."));
        add_row(tr("QUANTLIB SERVER URL"), QStringLiteral("connectors.quantlib_url"),
                tr("e.g. http://127.0.0.1:8800"),
                tr("Optional self-hosted QuantLib REST server. Blank = pricing disabled (no network)."));
