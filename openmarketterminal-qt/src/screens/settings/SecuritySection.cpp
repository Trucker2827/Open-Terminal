...
void SecuritySection::build_ui() {
    ...
    auto* row_cli_kill = make_row(
        ...
        tr("PANIC button (cli.kill_switch). When on, ALL AI trading is halted immediately — paper AND live, every venue. The CLI/agent can never clear it. Separate from Predictions → BOT stop (kalshi-bot-stop.json), which only refuses the Kalshi bot tick, and from Strategies → Risk kill."));
    ...
    auto* row_cli_ordval = make_row(
        ...
        tr("Maximum notional value (USD) of any single generic AI order. Unset = 25000. Kalshi live bot ignores this and uses the $2/$3 charter caps."));
...
void SecuritySection::retranslateUi() {
    ...
    if (row_cli_kill_desc_)   row_cli_kill_desc_->setText(tr("PANIC button (cli.kill_switch). When on, ALL AI trading is halted immediately — paper AND live, every venue. The CLI/agent can never clear it. Separate from Predictions → BOT stop (kalshi-bot-stop.json), which only refuses the Kalshi bot tick, and from Strategies → Risk kill."));
    ...
    if (row_cli_ordval_desc_) row_cli_ordval_desc_->setText(tr("Maximum notional value (USD) of any single generic AI order. Unset = 25000. Kalshi live bot ignores this and uses the $2/$3 charter caps."));
