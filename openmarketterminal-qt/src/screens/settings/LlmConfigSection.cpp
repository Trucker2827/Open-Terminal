...
// LOCAL-FIRST FORK: "openmarketterminal" removed from the selectable provider list — the
// hosted OpenMarketTerminal LLM proxy is gone. LlmService coerces any stale "openmarketterminal"
// selection to local Ollama at dispatch, so it can never be contacted.
...
QString LlmConfigSection::provider_display_name(const QString& provider_id) {
    static const QHash<QString, QString> kNames = {
        ...
        {"xai", "xAI"},              {"astraflow", "AstraFlow"},
        {"astraflow_cn", "AstraFlow CN"}, {"aihubmix", "AIHubMix"},   {"atlascloud", "AtlasCloud"},
        // Stale "openmarketterminal" configs are coerced to local Ollama at dispatch.
        {"openmarketterminal", "Local (Ollama)"},
    };
