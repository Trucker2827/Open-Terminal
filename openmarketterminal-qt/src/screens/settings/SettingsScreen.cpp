...
void SettingsScreen::wire_section_signals() {
    ...
    if (auto* voice = qobject_cast<VoiceConfigSection*>(sections_->widget(13))) {
        connect(voice, &VoiceConfigSection::config_changed, this, []() {
            openmarketterminal::services::SpeechService::instance().reload_config();
            openmarketterminal::services::TtsService::instance().reload_config();

            auto& clap = openmarketterminal::services::ClapDetectorService::instance();
            clap.stop();
            if (openmarketterminal::services::ClapDetectorService::is_enabled_in_config())
                clap.start();
        });
    }
}
