...
void StorageSection::build_ui() {
    ...
    // ── SECTION 4: SQL CONSOLE ─────────────────────────────────────────────────
    {
        ...
        sql_db_selector_->addItem("openmarketterminal.db", "main");
    ...
    // ── SECTION 5: DANGER ZONE ─────────────────────────────────────────────────
    {
        ...
        // ── Backup & Restore (local, no cloud) ────────────────────────────────
        {
            ...
            hl->setSpacing(8);
            auto* desc_w = new QWidget(this);
            auto* dvl = new QVBoxLayout(desc_w);
            dvl->setContentsMargins(0, 0, 0, 0);
            dvl->setSpacing(2);
            backup_title_ = new QLabel(tr("Backup & Restore"));
            backup_title_->setStyleSheet(QString("color:%1;font-weight:700;background:transparent;")
                                             .arg(ui::colors::TEXT_PRIMARY()));
            dvl->addWidget(backup_title_);
            backup_desc_ = new QLabel(tr("Export all your data (databases, workspaces, files) to a single "
                                         ".zip file you can copy to another machine or a drive. Nothing is "
                                         "uploaded. Secrets in the OS keychain are not included — re-enter "
                                         "them after restoring."));
            backup_desc_->setWordWrap(true);
            backup_desc_->setStyleSheet(QString("color:%1;background:transparent;").arg(ui::colors::TEXT_DIM()));
            dvl->addWidget(backup_desc_);
            hl->addWidget(desc_w, 1);

            export_btn_ = new QPushButton(tr("Export Backup…"));
            export_btn_->setFixedHeight(26);
            connect(export_btn_, &QPushButton::clicked, this, [this]() {
                const QString dir = QFileDialog::getExistingDirectory(
                    this, tr("Choose a folder for the backup"), QDir::homePath());
                if (dir.isEmpty())
                    return;
                auto r = BackupService::instance().export_backup(dir);
                if (r.is_ok())
                    QMessageBox::information(this, tr("Backup complete"),
                                            tr("Your data was backed up to:\n\n%1\n\nKeep it somewhere safe — "
                                               "it is unencrypted personal data.")
                                                .arg(r.value()));
                else
            ...
            import_btn_ = new QPushButton(tr("Restore Backup…"));
            import_btn_->setFixedHeight(26);
            connect(import_btn_, &QPushButton::clicked, this, [this]() {
                const QString file = QFileDialog::getOpenFileName(
                    this, tr("Choose a backup .zip to restore"), QDir::homePath(),
                    tr("OpenMarketTerminal backup (*.zip)"));
                if (file.isEmpty())
                    return;
                const auto confirm = QMessageBox::warning(
                    this, tr("Restore Backup"),
                    tr("Restoring will REPLACE all current data with the backup. Your current data is "
                       "moved to a 'pre-restore' folder first (so you can undo).\n\n"
                       "The app must restart to finish. Continue?"),
                    QMessageBox::Yes | QMessageBox::Cancel, QMessageBox::Cancel);
                if (confirm != QMessageBox::Yes)
                    return;
                auto r = BackupService::instance().stage_import(file);
                if (r.is_err()) {
                    QMessageBox::warning(this, tr("Restore failed"), QString::fromStdString(r.error()));
                    return;
                }
                QMessageBox::information(
                    this, tr("Restart to finish"),
                    tr("The backup is staged. OpenMarketTerminal will now close — reopen it to complete the restore."));
