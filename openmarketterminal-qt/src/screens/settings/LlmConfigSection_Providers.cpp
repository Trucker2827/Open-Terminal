...
void LlmConfigSection::on_provider_selected(int row) {
    ...
    delete_btn_->setEnabled(provider.toLower() != "openmarketterminal");
...
void LlmConfigSection::on_delete_provider() {
    ...
    if (provider.toLower() == "openmarketterminal") {
        show_status(tr("Cannot remove built-in OpenMarketTerminal provider"), true);
        return;
    }
...
void LlmConfigSection::on_fetch_models() {
    ...
    if (provider == "openmarketterminal") {
        show_status(tr("OpenMarketTerminal manages models automatically"), false);
        return;
    }
