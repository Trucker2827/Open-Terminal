...
void GeopoliticsEventsWidget::populate(const QVariant& payload) {
    ...
    using openmarketterminal::services::geo::EventsPage;
    ...
    for (const auto& ev : page.events) {
        ...
        const QColor cat_col = openmarketterminal::services::geo::category_color(ev.event_category);
        const QString cat_label = openmarketterminal::services::geo::pretty_category(ev.event_category);
