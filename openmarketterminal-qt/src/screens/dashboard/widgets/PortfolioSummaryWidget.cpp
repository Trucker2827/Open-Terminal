...
void PortfolioSummaryWidget::refresh_portfolio_cache() {
    auto r = openmarketterminal::PortfolioRepository::instance().list_portfolios();
    if (r.is_ok())
...
void PortfolioSummaryWidget::load_holdings() {
    ...
    auto assets_r = openmarketterminal::PortfolioRepository::instance().get_assets(selected_portfolio_id_);
