...
void TradeTapeWidget::apply_config(const QJsonObject& cfg) {
    exchange_ = cfg.value("exchange").toString("coinbase").trimmed().toLower();
    if (!trading::ExchangeSessionManager::supported_exchange_ids().contains(exchange_))
        exchange_ = "coinbase";
...
void TradeTapeWidget::on_trade(const QVariant& v) {
    if (!v.canConvert<openmarketterminal::trading::TradeData>())
        return;
    const auto td = v.value<openmarketterminal::trading::TradeData>();
    Trade t;
...
QDialog* TradeTapeWidget::make_config_dialog(QWidget* parent) {
    ...
    ex->addItems(trading::ExchangeSessionManager::supported_exchange_ids());
