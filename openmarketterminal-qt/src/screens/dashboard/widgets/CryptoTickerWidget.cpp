...
constexpr const char* kDefaultExchange = "coinbase";
} // namespace
...
void CryptoTickerWidget::hub_resubscribe() {
    ...
    for (const auto& pair : pairs_) {
        ...
        hub.subscribe(this, topic, [this, pair_copy](const QVariant& v) {
            if (!v.canConvert<openmarketterminal::trading::TickerData>())
                return;
            on_ticker(pair_copy, v.value<openmarketterminal::trading::TickerData>());
        });
    }
    hub_active_ = true;
}
...
void CryptoTickerWidget::on_ticker(const QString& pair, const openmarketterminal::trading::TickerData& t) {
...
QDialog* CryptoTickerWidget::make_config_dialog(QWidget* parent) {
    ...
    // Single canonical exchange list (Kraken, Coinbase, Binance, …) — same
    // source the main crypto screen uses, so this never drifts out of sync.
    combo->addItems(trading::ExchangeSessionManager::supported_exchange_ids());
