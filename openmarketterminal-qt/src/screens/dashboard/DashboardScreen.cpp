...
DashboardScreen::DashboardScreen(QWidget* parent) : QWidget(parent) {
    ...
    notif_toast_ = new openmarketterminal::ui::NotifToast(this);
    connect(&openmarketterminal::notifications::NotificationService::instance(),
            &openmarketterminal::notifications::NotificationService::notification_received, notif_toast_,
            &openmarketterminal::ui::NotifToast::show_notification);
    ...
    connect(toolbar_, &DashboardToolBar::reset_layout_clicked, this, [this]() {
        ...
        connect(dlg, &TemplatePicker::template_selected, this, [this](const QString& tid) {
            ...
            (void)QtConcurrent::run([]() { openmarketterminal::SettingsRepository::instance().remove("dashboard_canvas_layout"); });
...
// Map a raw Yahoo symbol to a human-readable ticker label so the tape reads
// "S&P 500 / NASDAQ / GOLD" instead of "^GSPC / ^IXIC / GC=F". Plain stock
// tickers (AAPL, NVDA, …) pass through unchanged — they're already familiar.
static QString ticker_label(const QString& sym) {
    static const QHash<QString, QString> kNames = {
        {"^GSPC", "S&P 500"},     {"^DJI", "DOW"},          {"^IXIC", "NASDAQ"},
        {"^RUT", "RUSSELL 2000"}, {"^VIX", "VIX"},          {"^NYA", "NYSE COMP"},
        {"^SOX", "SEMIS"},        {"^FTSE", "FTSE 100"},    {"^GDAXI", "DAX"},
        {"^FCHI", "CAC 40"},      {"^N225", "NIKKEI"},      {"^HSI", "HANG SENG"},
        {"000001.SS", "SHANGHAI"},
        {"^STOXX50E", "EURO STOXX 50"}, {"^IBEX", "IBEX 35"}, {"^AEX", "AEX"},
        {"^AXJO", "ASX 200"},     {"^TNX", "US 10Y"},       {"^TYX", "US 30Y"},
        {"^IRX", "US 13W"},       {"^FVX", "US 5Y"},
        {"GC=F", "GOLD"},         {"SI=F", "SILVER"},       {"CL=F", "CRUDE OIL"},
        {"BZ=F", "BRENT"},        {"NG=F", "NAT GAS"},      {"HG=F", "COPPER"},
        {"DX-Y.NYB", "DOLLAR IDX"}, {"BTC-USD", "BITCOIN"}, {"ETH-USD", "ETHEREUM"},
    };
    auto it = kNames.find(sym);
    if (it != kNames.end())
        return it.value();
    // Yahoo forex pairs like "EURUSD=X" → "EUR/USD".
    if (sym.size() == 8 && sym.endsWith(QLatin1String("=X")))
        return sym.left(3) + "/" + sym.mid(3, 3);
    return sym; // plain stock ticker — already recognizable
}
...
void DashboardScreen::rebuild_ticker_from_cache() {
    ...
    for (const auto& sym : ticker_subscribed_) {
        ...
        entries.append({ticker_label(q.symbol), q.price, q.change});
    }
    if (!entries.isEmpty())
...
void DashboardScreen::save_layout() {
    ...
    (void)QtConcurrent::run(
        [encoded]() { openmarketterminal::SettingsRepository::instance().set("dashboard_canvas_layout", encoded, "dashboard"); });
}

void DashboardScreen::restore_layout() {
    QPointer<DashboardScreen> self = this;
    (void)QtConcurrent::run([self]() {
        auto result = openmarketterminal::SettingsRepository::instance().get("dashboard_canvas_layout");
