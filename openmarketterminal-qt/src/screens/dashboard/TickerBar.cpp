...
void TickerBar::load_symbols() {
    // Build default list: US-first indices + a few major global indices + US movers.
    // (The dashboard "Global Indices" widget keeps the full world list; the scrolling
    // tape leads with what a US/EU audience recognizes.)
    const QStringList defaults = QStringList{"^GSPC", "^DJI", "^IXIC", "^RUT", "^VIX",
                                             "^FTSE", "^GDAXI", "^N225"}
    ...
    (void)QtConcurrent::run([self, defaults]() {
        auto result = openmarketterminal::SettingsRepository::instance().get(kSettingsKey);
        QStringList loaded;
...
void TickerBar::save_symbols() {
    ...
    (void)QtConcurrent::run([value]() {
        openmarketterminal::SettingsRepository::instance().set(kSettingsKey, value, kSettingsCategory);
    });
}
