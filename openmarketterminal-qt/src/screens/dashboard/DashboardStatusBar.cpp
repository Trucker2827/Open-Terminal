...
DashboardStatusBar::DashboardStatusBar(QWidget* parent) : QWidget(parent) {
    ...
    // ── Notification bell ──────────────────────────────────────────────────
    notif_bell_ = new openmarketterminal::ui::NotifBell(this);
    rl->addWidget(notif_bell_);

    connect(notif_bell_, &openmarketterminal::ui::NotifBell::bell_clicked, this, &DashboardStatusBar::toggle_notif_panel);
...
void DashboardStatusBar::ping_api() {
    // LOCAL-FIRST FORK: no cloud health ping. The terminal is fully local —
    // there is no vendor backend to measure latency to, so
    // this makes no network request at all. The LAT indicator reads "LOCAL".
    if (latency_label_) {
        latency_label_->setText(tr("LAT: LOCAL"));
...
void DashboardStatusBar::toggle_notif_panel() {
    ...
    if (!notif_panel_) {
        QWidget* root = window();
        notif_panel_ = new openmarketterminal::ui::NotifPanel(root ? root : this);
    }
