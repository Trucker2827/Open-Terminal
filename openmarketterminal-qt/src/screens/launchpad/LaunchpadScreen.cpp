...
LaunchpadScreen::LaunchpadScreen(QWidget* parent) : QMainWindow(parent) {
    setWindowTitle(tr("OpenMarketTerminal Launchpad"));
    ...
    // OpenMarketTerminal wordmark (embedded Qt resource, dark-bg variant).
    auto* wordmark = new QLabel;
    wordmark->setPixmap(QPixmap(QStringLiteral(":/openmarket/wordmark.png"))
                            .scaledToWidth(280, Qt::SmoothTransformation));
    wordmark->setStyleSheet("background: transparent;");
    vl->addWidget(wordmark);
...
void LaunchpadScreen::on_open_layout() {
    ...
    QMetaObject::invokeMethod(this, [ws]() {
        if (openmarketterminal::WindowRegistry::instance().frames().isEmpty()) {
            auto* fr = new openmarketterminal::WindowFrame(openmarketterminal::WindowFrame::next_window_id());
...
void LaunchpadScreen::on_continue() {
    ...
    QMetaObject::invokeMethod(this, []() {
        ...
        if (n == 0) {
            ...
            auto* w = new openmarketterminal::WindowFrame(openmarketterminal::WindowFrame::next_window_id());
...
void LaunchpadScreen::on_template_picked(const QString& persona_id) {
    ...
    QMetaObject::invokeMethod(this, [ws, first_run_tour]() {
        openmarketterminal::WindowFrame* anchor = nullptr;
        if (openmarketterminal::WindowRegistry::instance().frames().isEmpty()) {
            anchor = new openmarketterminal::WindowFrame(openmarketterminal::WindowFrame::next_window_id());
            anchor->setAttribute(Qt::WA_DeleteOnClose);
            anchor->show();
        } else {
            anchor = openmarketterminal::WindowRegistry::instance().frames().first();
        }
        layout::WorkspaceShell::apply(ws);
        ...
        if (first_run_tour && anchor) {
            QPointer<openmarketterminal::WindowFrame> guard(anchor);
