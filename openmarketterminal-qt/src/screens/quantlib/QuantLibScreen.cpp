...
// Real per-module endpoint paths (defined in QuantLibScreen_Data.cpp). Declared
// here so create_sidebar() can count the actually-wired endpoints per module.
...
QuantLibScreen::QuantLibScreen(QWidget* parent) : QWidget(parent) {
    ...
    LOG_INFO("QuantLib", "Screen constructed — remote pricing client (external server required)");
}
...
QWidget* QuantLibScreen::create_header() {
    ...
    header_badge_ = new QLabel(configured ? tr("SERVER CONFIGURED") : tr("NOT CONFIGURED"));
    header_badge_->setObjectName("qlHeaderBadge");
    header_badge_->setStyleSheet(QString("color:%1; background:transparent;")
                                     .arg(configured ? colors::POSITIVE() : colors::WARNING()));
...
QWidget* QuantLibScreen::create_sidebar() {
    ...
    for (int i = 0; i < modules_.size(); ++i) {
        ...
        const int real_count = module_endpoints().value(m.id).size();
        module_item->setText(0, m.name.toUpper() + " (" + QString::number(real_count) + ")");
...
void QuantLibScreen::retranslateUi() {
    ...
    if (header_badge_) {
        auto r = SettingsRepository::instance().get(QStringLiteral("connectors.quantlib_url"));
        const bool configured = r.is_ok() && !r.value().trimmed().isEmpty();
        header_badge_->setText(configured ? tr("SERVER CONFIGURED") : tr("NOT CONFIGURED"));
    }
...
// Real API endpoint paths per module (from api.example.com OpenAPI spec)
...
// Forward declaration — we're already inside `namespace openmarketterminal::screens` (opened
