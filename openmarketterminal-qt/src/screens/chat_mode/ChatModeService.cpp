...
QString ChatModeService::base_url() const {
    // LOCAL-FIRST FORK: the OpenMarketTerminal-hosted Chat Mode backend is removed. Every
    // request in this service (HTTP verbs, optimize-prompt, agent chat, and both
    // SSE streams) builds its URL via build_request() -> base_url() + path. By
    // returning an empty base URL, every request becomes a host-less relative URL
    // that Qt fails locally before any connection is made — so no chat session,
    // message text, or prompt is ever sent to api.example.com. The Chat Mode screen
    // degrades to inert (replies error out gracefully); the main AI Chat is
    // unaffected and runs on the local LLM provider.
    return {};
}
...
QNetworkRequest ChatModeService::build_request(const QString& path) const {
    ...
    req.setHeader(QNetworkRequest::UserAgentHeader, "OpenMarketTerminal/0.1.0");
...
void ChatModeService::handle_reply(QNetworkReply* reply,
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply, cb = std::move(cb)]() mutable {
        ...
        if (status == 402) {
            LOG_WARN("ChatModeService", "HTTP 402 (Chat Mode backend disabled)");
            cb(false, {}, tr("Chat Mode unavailable"));
            return;
        }
