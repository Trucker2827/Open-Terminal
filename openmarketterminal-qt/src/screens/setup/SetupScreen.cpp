...
SetupScreen::SetupScreen(QWidget* parent) : QWidget(parent) {
    ...
    net_meter_ = new openmarketterminal::net::NetSpeedMeter(this);
    connect(net_meter_, &openmarketterminal::net::NetSpeedMeter::speed_changed,
...
void SetupScreen::build_ui() {
    ...
    // ── Title block ───────────────────────────────────────────────────────────
    // OPENMARKET TERMINAL — brand, not translated.
    title_lbl_ = new QLabel(QStringLiteral("OPENMARKET TERMINAL"), center);
    ...
    sparkline_ = new openmarketterminal::ui::SpeedSparkline(live_row_);
...
void SetupScreen::update_status_label() {
    ...
    switch (status_state_) {
        ...
        case StatusState::AllDone:
            text  = tr("Everything is ready! Launching OpenMarketTerminal...");
            color = colors::GREEN();
            break;
...
void SetupScreen::on_net_speed(qint64 down_bps, qint64 up_bps) {
    ...
    up_bps_ = up_bps;
    if (down_lbl_)
        down_lbl_->setText(QStringLiteral("↓ ") + openmarketterminal::net::NetSpeedMeter::format_bps(down_bps));
    if (up_lbl_)
        up_lbl_->setText(QStringLiteral("↑ ") + openmarketterminal::net::NetSpeedMeter::format_bps(up_bps));
    if (sparkline_)
