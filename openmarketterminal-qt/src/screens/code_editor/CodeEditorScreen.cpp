...
// kStyle() pattern so the OpenMarketTerminal Notebook chrome matches the MCP design system
...
void CodeEditorScreen::on_new_notebook() {
    ...
    cell.id = new_cell_id();
    cell.cell_type = "code";
    cell.source = "# OpenMarketTerminal Notebook\n"
                  "# Write Python code and press Ctrl+Enter to run\n"
                  "\n"
                  "print(\"Hello from OpenMarketTerminal!\")";
    cells_.append(cell);
...
void CodeEditorScreen::on_open_notebook() {
    const QString path =
        QFileDialog::getOpenFileName(this, tr("Open Notebook"), {}, tr("OpenMarketTerminal Notebooks (*.ipynb);;All Files (*)"));
    if (path.isEmpty())
        return;
...
bool CodeEditorScreen::load_notebook_from_path(const QString& path) {
    ...
    for (const auto& jc : json_cells) {
        ...
        cell.title = cell_metadata["openmarketterminal_title"].toString().trimmed();
        cell.execution_count = co["execution_count"].toInt(0);

        // Parse persisted nbformat outputs back into the cell model (symmetric
        // with the serialization in on_save_notebook). set_cell_data() pushes
        // these into the widget via set_outputs() during rebuild_cells().
        for (const auto& ov : co["outputs"].toArray()) {
            QJsonObject oo = ov.toObject();
            CellOutput out;
            const QString otype = oo["output_type"].toString();
            if (otype == "error") {
                out.type = "error";
                out.error_name = oo["ename"].toString();
                out.error_value = oo["evalue"].toString();
                QStringList tb;
                for (const auto& t : oo["traceback"].toArray())
                    tb << t.toString();
                out.traceback = tb;
                out.text = tb.join("\n");
            } else {
                // "stream" and any other type render as text.
                out.type = "stream";
                out.name = oo["name"].toString("stdout");
                const QJsonValue tv = oo["text"];
                if (tv.isArray()) {
                    QString text;
...
void CodeEditorScreen::on_save_notebook() {
    for (int i = 0; i < cells_.size(); ++i) {
        ...
        if (cw) {
            // cell_data() does not carry outputs; preserve the run-time outputs
            // already stored on the model so they survive the save.
            QVector<CellOutput> preserved_outputs = cells_[i].outputs;
            cells_[i] = cw->cell_data();
            cells_[i].outputs = preserved_outputs;
        }
    }

    QString path = notebook_path_;
    if (path.isEmpty()) {
        path = QFileDialog::getSaveFileName(this, tr("Save Notebook"), "notebook.ipynb", tr("OpenMarketTerminal Notebooks (*.ipynb)"));
        if (path.isEmpty())
            return;
    }
    ...
    for (const auto& cell : cells_) {
        ...
        if (!cell.title.trimmed().isEmpty())
            cell_metadata["openmarketterminal_title"] = cell.title.trimmed();
        co["metadata"] = cell_metadata;

        if (cell.cell_type == "code") {
            co["execution_count"] = cell.execution_count > 0 ? QJsonValue(cell.execution_count) : QJsonValue();

            // Serialize the cell's real outputs as nbformat-4 output objects so a
            // save->reopen round-trip preserves results (parsed back in load_notebook_from_path).
            QJsonArray outputs_json;
            for (const auto& out : cell.outputs) {
                QJsonObject oj;
                if (out.type == "error") {
                    oj["output_type"] = "error";
                    oj["ename"] = out.error_name;
                    oj["evalue"] = out.error_value;
                    QJsonArray tb;
                    for (const auto& line : out.traceback)
                        tb.append(line);
                    oj["traceback"] = tb;
                } else {
                    // "stream" and any non-typed output fall back to a stdout stream.
                    oj["output_type"] = "stream";
                    oj["name"] = out.name.isEmpty() ? QStringLiteral("stdout") : out.name;
                    // Store the text as a single-element array (valid nbformat,
                    // exact round-trip: the loader joins the parts with "").
                    QJsonArray text_lines;
                    text_lines.append(out.text);
                    oj["text"] = text_lines;
                }
                outputs_json.append(oj);
            }
            co["outputs"] = outputs_json;
        }
...
void CodeEditorScreen::query_python_version() {
    const QString path = python::PythonRunner::instance().python_path();
    if (path.isEmpty() || !py_label_)
        return;
    ...
    connect(proc, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this,
            [this, proc](int code, QProcess::ExitStatus) {
                ...
                if (out.isEmpty())
                    out = QString::fromUtf8(proc->readAllStandardError()).trimmed(); // older pythons print to stderr
                if (out.startsWith("Python"))
                    py_label_->setText(out); // the real interpreter version string
            });
