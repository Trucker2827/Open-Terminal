...
float SurfaceAnalyticsScreen::spot_for(const QString& sym) const {
    ...
    if (v.isValid()) {
        if (v.canConvert<openmarketterminal::services::QuoteData>()) {
            auto q = v.value<openmarketterminal::services::QuoteData>();
            if (q.price > 0.0)
...
void SurfaceAnalyticsScreen::on_group_symbol_changed(const openmarketterminal::SymbolRef& ref) {
...
openmarketterminal::SymbolRef SurfaceAnalyticsScreen::current_symbol() const {
    if (!control_panel_)
        return {};
    QString s = control_panel_->state().symbol;
    if (s.isEmpty())
        return {};
    return openmarketterminal::SymbolRef::equity(s);
}
