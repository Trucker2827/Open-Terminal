...
namespace openmarketterminal::screens::alpha_arena {

using openmarketterminal::services::alpha_arena::AlphaArenaEngine;
using openmarketterminal::services::alpha_arena::AlphaArenaRepo;
using openmarketterminal::services::alpha_arena::AgentRow;
using openmarketterminal::services::alpha_arena::EventRow;
using openmarketterminal::services::alpha_arena::Position;
...
void PositionsPanel::refresh() {
    ...
    for (int i = 0; i < rows.size(); ++i) {
        ...
        it_qty->setForeground(QColor(p.qty >= 0 ? openmarketterminal::ui::colors::POSITIVE()
        ...
        it_pnl->setForeground(QColor(p.unrealized_pnl >= 0 ? openmarketterminal::ui::colors::POSITIVE()
                                                            : openmarketterminal::ui::colors::NEGATIVE()));
        ...
        if (dist < 0.05) it_dist->setForeground(QColor(openmarketterminal::ui::colors::NEGATIVE()));
...
void RiskPanel::refresh() {
    ...
    for (int i = 0; i < agents.value().size(); ++i) {
        ...
        if (a.state == QLatin1String("circuit_open") || a.state == QLatin1String("halted"))
            it_state->setForeground(QColor(openmarketterminal::ui::colors::NEGATIVE()));
        ...
        it_dd->setForeground(QColor(openmarketterminal::ui::colors::NEGATIVE()));
...
void AuditPanel::refresh() {
    ...
    for (const auto& e : rows.value()) {
        ...
        if (e.type.contains(QLatin1String("error")) ||
            ...
            it->setForeground(QColor(openmarketterminal::ui::colors::NEGATIVE()));
        } else if (e.type.contains(QLatin1String("decision")) ||
                   e.type.contains(QLatin1String("fill"))) {
            it->setForeground(QColor(openmarketterminal::ui::colors::POSITIVE()));
        }
        list_->addItem(it);
