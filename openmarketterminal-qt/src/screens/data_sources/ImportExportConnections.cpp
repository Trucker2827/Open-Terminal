...
bool export_connections(QWidget* parent) {
    const QString path = QFileDialog::getSaveFileName(
        parent, QObject::tr("Export Connections"), "openmarketterminal_connections.json",
...
bool download_connector_template(QWidget* parent) {
    const QString path = QFileDialog::getSaveFileName(
        parent, QObject::tr("Save Connector Template"), "openmarketterminal_connections_template.json",
        QObject::tr("JSON Files (*.json);;All Files (*)"));
    if (path.isEmpty())
        return false;
