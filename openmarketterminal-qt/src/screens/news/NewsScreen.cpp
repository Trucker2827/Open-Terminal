...
void NewsScreen::connect_signals() {
    ...
    connect(detail_panel_, &NewsDetailPanel::bookmark_requested, this, [this](const services::NewsArticle& article) {
        ...
        if (r.is_ok()) {
            auto saved_r = openmarketterminal::NewsArticleRepository::instance().load_saved();
            if (saved_r.is_ok())
    ...
    connect(seen_flush_timer_, &QTimer::timeout, this, [this]() {
        ...
        (void)QtConcurrent::run([ids, self]() {
            for (const auto& id : ids)
                openmarketterminal::NewsArticleRepository::instance().mark_seen(id);
            if (self) {
...
void NewsScreen::showEvent(QShowEvent* e) {
    ...
    if (first_show_) {
        ...
        openmarketterminal::NewsArticleRepository::instance().ensure_seen_column();
        openmarketterminal::NewsArticleRepository::instance().ensure_saved_column();

        int64_t since_ts = QDateTime::currentSecsSinceEpoch() - (30LL * 86400);
        auto seen_result = openmarketterminal::NewsArticleRepository::instance().load_seen_ids(since_ts);
        if (seen_result.is_ok()) {
            for (const auto& id : seen_result.value())
                feed_panel_->model()->mark_seen(id);
        }

        auto saved_result = openmarketterminal::NewsArticleRepository::instance().load_saved();
...
void NewsScreen::apply_filters_async() {
    ...
    if (time_range == "7D" || time_range == "30D") {
        ...
        if (!search_lower.isEmpty()) {
            auto fts_result = openmarketterminal::NewsArticleRepository::instance().search_fts(search_lower, cutoff, 1000);
            if (fts_result.is_ok())
                merge_into(fts_result.value());
        } else {
            auto db_result = openmarketterminal::NewsArticleRepository::instance().load_recent(cutoff, {}, 5000);
            if (db_result.is_ok())
