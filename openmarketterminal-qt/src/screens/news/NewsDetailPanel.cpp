...
void NewsDetailPanel::show_article(const services::NewsArticle& article) {
    ...
    {
        auto r = openmarketterminal::NewsArticleRepository::instance().load_saved();
        bool is_saved = false;
        if (r.is_ok()) {
...
void NewsDetailPanel::show_analysis(const services::NewsAnalysis& analysis) {
    ...
    // No credits/metering in OpenMarketTerminal — the credits footer is never
    // shown. (AI news analysis is free; the old hosted backend's credit fields
