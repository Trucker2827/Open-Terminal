...
void ReportBuilderScreen::on_open() {
    QString path =
        QFileDialog::getOpenFileName(this, tr("Open Report"), "", tr("OpenMarketTerminal Report (*.openmarketterminal);;JSON (*.json)"));
    if (path.isEmpty())
        return;
...
void ReportBuilderScreen::on_save() {
    ...
    if (path.isEmpty()) {
        path = QFileDialog::getSaveFileName(this, tr("Save Report"), svc.metadata().title,
                                            tr("OpenMarketTerminal Report (*.openmarketterminal);;JSON (*.json)"));
        if (path.isEmpty())
            return;
    }
