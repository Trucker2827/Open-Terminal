...
namespace rep = ::openmarketterminal::report;
using Service = ::openmarketterminal::services::ReportBuilderService;
    ...
    connect(properties_, &PropertiesPanel::config_changed, this,
            [this](int idx, const QString& key, const QString& val) {
                ...
                if (comp_type == "stats_block" && key == "fetch_ticker") {
                    ...
                    openmarketterminal::services::MarketDataService::instance().fetch_info(
                        sym, [self, comp_id, sym](bool ok, openmarketterminal::services::InfoData info) {
                            if (!self)
                                return;
                ...
                if (comp_type == "chart" && key == "fetch_history") {
                    ...
                    auto apply = [self, comp_id, sym](const QVector<openmarketterminal::services::HistoryPoint>& history) {
                        if (!self)
                            return;
                        auto& s = Service::instance();
                        int idx2 = s.index_of(comp_id);
                    ...
                    if (cached.isValid() && cached.canConvert<QVector<openmarketterminal::services::HistoryPoint>>()) {
                        apply(cached.value<QVector<openmarketterminal::services::HistoryPoint>>());
                        return;
                    }

                    hub.subscribe<QVector<openmarketterminal::services::HistoryPoint>>(
                        this, topic,
                        [self, topic, apply](const QVector<openmarketterminal::services::HistoryPoint>& history) {
                            if (!self)
                                return;
                            apply(history);
                            openmarketterminal::datahub::DataHub::instance().unsubscribe(self, topic);
                        });
                ...
                if (comp_type == "market_data" && key == "status" && val == "loading") {
                    ...
                    auto apply = [self, comp_id](const openmarketterminal::services::QuoteData& q) {
                        if (!self)
                            return;
                        auto& s = Service::instance();
                        int idx2 = s.index_of(comp_id);
                    ...
                    QVariant cached = hub.peek(topic);
                    if (cached.isValid() && cached.canConvert<openmarketterminal::services::QuoteData>()) {
                        apply(cached.value<openmarketterminal::services::QuoteData>());
                        return;
                    }
                    hub.subscribe<openmarketterminal::services::QuoteData>(
                        this, topic,
                        [self, topic, apply](const openmarketterminal::services::QuoteData& q) {
                            if (!self)
                                return;
                            apply(q);
                            openmarketterminal::datahub::DataHub::instance().unsubscribe(self, topic);
