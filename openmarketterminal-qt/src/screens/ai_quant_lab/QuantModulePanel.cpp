...
using namespace openmarketterminal::screens::quant_styles;
using namespace openmarketterminal::screens::quant_common;
using namespace openmarketterminal::screens::quant_gs_helpers;
...
// "DEMO (synthetic data)" opt-in checkbox. UNCHECKED by default so the module
// trains on real qlib market data unless the user explicitly opts in to the
// clearly-labeled synthetic path. Never call setChecked(true) here — the
// default-unchecked state is the contract.
QCheckBox* QuantModulePanel::make_demo_checkbox(QWidget* parent) {
    auto* cb = new QCheckBox(tr("DEMO (synthetic data)"), parent);
    cb->setToolTip(tr("Off = train on real qlib market data. "
                      "On = clearly-labeled synthetic data for testing."));
    cb->setCursor(Qt::PointingHandCursor);
    cb->setStyleSheet(QString("QCheckBox { color:%1; background:transparent; font-family:%2; font-size:%3px; }")
