...
namespace openmarketterminal::screens::quant_advanced_helpers {
...
inline QString verdict_color_for(const QString& v) {
    ...
    if (u == "MODERATE" || u == "MAINTAIN" || u == "FAIR" || u == "MIXED")
        return openmarketterminal::ui::colors::TEXT_PRIMARY();
    if (u == "WEAK" || u == "WARNING" || u == "ELEVATED" || u == "NEUTRAL")
        return openmarketterminal::ui::colors::WARNING();
...
inline QTableWidget* build_weights_table(const QJsonObject& weights,
    ...
    table->setStyleSheet(openmarketterminal::screens::quant_styles::table_ss());
    ...
    for (int r = 0; r < rows.size(); ++r) {
        ...
        if (w > 0.001) wi->setForeground(QColor(openmarketterminal::ui::colors::POSITIVE()));
        else if (w < -0.001) wi->setForeground(QColor(openmarketterminal::ui::colors::NEGATIVE()));
