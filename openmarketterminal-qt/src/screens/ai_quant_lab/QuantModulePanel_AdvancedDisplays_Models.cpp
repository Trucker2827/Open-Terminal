...
using namespace openmarketterminal::screens::quant_advanced_helpers;
...
void QuantModulePanel::display_advanced_models_result(const QString& command, const QJsonObject& payload) {
    ...
    if (payload.value("demo").toBool(false)) {
        auto* demo_badge = new QLabel(tr("DEMO · SYNTHETIC DATA — not real results"));
        demo_badge->setStyleSheet(QString("color: %1; font-family: Consolas; font-size: 10px;"
                                          " font-weight: 700; padding: 2px 6px; border: 1px solid %1;")
                                      .arg(ui::colors::WARNING()));
        results_layout_->addWidget(demo_badge);
    }
