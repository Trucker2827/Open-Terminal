...
QWidget* QuantModulePanel::build_meta_learning_panel() {
    ...
    // Panel-level DEMO opt-in: shared by all tabs (selection/ensemble/tune) so
    // whichever run action the user fires sends params["demo"] consistently.
    auto* demo = make_demo_checkbox(w);
    bool_inputs_["meta_demo"] = demo;
    vl->addWidget(demo);
    ...
    connect(sel_run, &QPushButton::clicked, this, [this]() {
        ...
        params["demo"] = bool_inputs_["meta_demo"]->isChecked();
...
QWidget* QuantModulePanel::build_rolling_retraining_panel() {
    ...
    auto* lt_info = new QLabel(
        tr("Schedules are persisted in ~/.openmarketterminal/rolling_schedules.json. "
