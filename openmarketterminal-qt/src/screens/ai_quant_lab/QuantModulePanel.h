...
class QuantModulePanel : public QWidget {
    Q_OBJECT
  public:
    explicit QuantModulePanel(const openmarketterminal::services::quant::QuantModule& mod, QWidget* parent = nullptr);
  ...
  private:
    ...
    // "DEMO (synthetic data)" opt-in checkbox, UNCHECKED by default. Off = train
    // on real qlib market data; On = clearly-labeled synthetic data for testing.
    QCheckBox* make_demo_checkbox(QWidget* parent);
    ...
    openmarketterminal::services::quant::QuantModule module_;
    ...
    QHash<QString, QCheckBox*> bool_inputs_;
};
