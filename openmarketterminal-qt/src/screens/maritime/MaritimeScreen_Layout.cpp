...
using namespace openmarketterminal::services::maritime;
using namespace openmarketterminal::screens::maritime_internal;
...
QWidget* MaritimeScreen::build_top_bar() {
    ...
    brand_label_ = new QLabel(tr("OPENMARKETTERMINAL MARITIME INTELLIGENCE"), bar);
...
QWidget* MaritimeScreen::build_left_panel() {
    ...
    connect(sq_btn, &QPushButton::toggled, this, [this, ci_btn](bool on) {
        ...
        map_widget_->set_draw_mode(on ? openmarketterminal::ui::MapDrawMode::Rectangle
                                      : openmarketterminal::ui::MapDrawMode::None);
    });
    connect(ci_btn, &QPushButton::toggled, this, [this, sq_btn](bool on) {
        if (!map_widget_) return;
        if (on) sq_btn->setChecked(false);
        map_widget_->set_draw_mode(on ? openmarketterminal::ui::MapDrawMode::Circle
                                      : openmarketterminal::ui::MapDrawMode::None);
    });
    connect(clr_btn, &QPushButton::clicked, this, [this, sq_btn, ci_btn]() {
        ...
        if (map_widget_) {
            map_widget_->set_draw_mode(openmarketterminal::ui::MapDrawMode::None);
...
QWidget* MaritimeScreen::build_center_panel() {
    ...
    map_widget_ = new openmarketterminal::ui::WorldMapWidget(splitter);
    ...
    map_loader_ = new openmarketterminal::ui::LoadingOverlay(map_widget_);
    ...
    if (map_type_combo_) {
        map_type_combo_->addItems(openmarketterminal::ui::WorldMapWidget::basemap_labels());
    ...
    connect(map_widget_, &openmarketterminal::ui::WorldMapWidget::shapes_changed, this,
    ...
    connect(map_widget_, &openmarketterminal::ui::WorldMapWidget::pin_clicked, this, [this](int id) {
...
QWidget* MaritimeScreen::build_right_panel() {
    ...
    connect(imo_edit_, &QLineEdit::textChanged, this,
            [this](const QString&) { openmarketterminal::ScreenStateManager::instance().notify_changed(this); });
    ...
    connect(ports_completer_, QOverload<const QString&>::of(&QCompleter::activated), this,
            [this](const QString& text) {
                ...
                for (const auto& p : port_results_) {
                    if (p.name == text) {
                        openmarketterminal::ui::MapPin pin;
