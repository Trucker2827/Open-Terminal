...
openmarketterminal::screens::LockScreen*
LockOverlayController::lock_screen_for(openmarketterminal::WindowFrame* frame) {
    if (!frame)
        return nullptr;
    ...
    auto* ls = new openmarketterminal::screens::LockScreen(nullptr);

    Tracked t;
    t.frame = QPointer<openmarketterminal::WindowFrame>(frame);
    t.lock_screen = QPointer<openmarketterminal::screens::LockScreen>(ls);
...
void LockOverlayController::on_frame_added(openmarketterminal::WindowFrame* w) {
    if (!w)
        return;
    ...
    if (!tracked_frames_.contains(w))
        tracked_frames_.insert(w, Tracked{QPointer<openmarketterminal::WindowFrame>(w), {}});
}

void LockOverlayController::on_frame_removing(openmarketterminal::WindowFrame* w) {
    if (!w)
        return;
