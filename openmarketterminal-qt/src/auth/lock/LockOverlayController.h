...
class LockOverlayController : public QObject {
  ...
  public:
    ...
    openmarketterminal::screens::LockScreen* lock_screen_for(openmarketterminal::WindowFrame* frame);

    /// Test/debug accessor.
    int tracked_frame_count() const { return tracked_frames_.size(); }

  private slots:
    void on_frame_added(openmarketterminal::WindowFrame* w);
    void on_frame_removing(openmarketterminal::WindowFrame* w);
  ...
  private:
    ...
    /// frame teardown).
    struct Tracked {
        QPointer<openmarketterminal::WindowFrame> frame;
        QPointer<openmarketterminal::screens::LockScreen> lock_screen;
    };
    QHash<openmarketterminal::WindowFrame*, Tracked> tracked_frames_;
};
