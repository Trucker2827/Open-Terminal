...
class WebSocketClient : public QObject {
  ...
  public:
    ...
    void connect_to(const QString& url, const QMap<QString, QString>& headers);

    /// Connect with handshake headers regenerated on every (re)connect attempt.
    /// Required for servers whose auth is time-sensitive (e.g. Kalshi signs a
    /// timestamp that goes stale): a fixed header set would be replayed on
    /// reconnect and rejected with HTTP 401.
    using HeaderProvider = std::function<QMap<QString, QString>()>;
    void connect_to(const QString& url, HeaderProvider provider);
  ...
  private:
    ...
    HeaderProvider header_provider_;  // when set, regenerates headers per (re)connect
