...
WindowFrame::WindowFrame(int window_id, QWidget* parent, const WindowId& adopted_uuid)
    ...
    setProperty("openmarketterminal.active_for_work", true);
    ...
    // "Open Terminal" is the product brand and is intentionally not
    // translated — the bracketed profile name is the only variable part.
    const QString profile = ProfileManager::instance().active();
    setWindowTitle(profile == "default" ? QStringLiteral("Open Terminal")
                                        : QStringLiteral("Open Terminal [%1]").arg(profile));
    ...
    if (app_icon.isNull()) {
        ...
        const QString ico = QCoreApplication::applicationDirPath() + "/openmarketterminal.ico";
        if (QFile::exists(ico))
            app_icon = QIcon(ico);
    }
#else
    ...
    connect(toolbar, &ui::ToolBar::action_triggered, this, [this](const QString& action) {
        ...
        if (action == "new_window") {
            ...
            QString path = QFileDialog::getOpenFileName(
                ...
                tr("OpenMarketTerminal Layout (*.flayout *.fwsp);;All Files (*)"));
            if (!path.isEmpty()) {
            ...
            if (cur_id.is_null()) {
                ...
                QString path = QFileDialog::getSaveFileName(
                    ...
                    tr("OpenMarketTerminal Layout (*.flayout)"));
                if (!path.isEmpty()) {
            ...
            QString path = QFileDialog::getSaveFileName(
                ...
                QDir::homePath() + "/openmarketterminal_" + QDateTime::currentDateTime().toString("yyyyMMdd_HHmmss") + ".png",
                tr("PNG Images (*.png)"));
            if (!path.isEmpty()) {
                px.save(path, "PNG");
    ...
    // LOCAL-FIRST FORK: the periodic user-data refresh timer was removed with
    // AuthManager::refresh_user_data() — there is no remote account/credits to
    // poll. The only remaining focus-driven work is the resume-from-sleep lock
    // check below.
...
void WindowFrame::update_window_title() {
    ...
    QString title = QStringLiteral("Open Terminal");
...
bool WindowFrame::is_active_for_work() const {
    ...
    // openmarketterminal.active_for_work to false, and any DataHub topic with
...
constexpr const char* kActiveForWorkProp = "openmarketterminal.active_for_work";
} // namespace
