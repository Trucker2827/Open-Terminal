...
void WindowFrame::setup_auth_screens() {
    // LOCAL-FIRST FORK: the Register and Forgot-Password screens were removed.
    // The auth stack now holds only the guest-entry LoginScreen (index 0) and
    // the shared info stack (index 1).
    ...
    auth_stack_->addWidget(info_stack_); // index 1

    // ── Auth navigation ──────────────────────────────────────────────────────
    connect(login, &screens::LoginScreen::continue_as_guest, this, &WindowFrame::continue_as_guest);
    ...
    // LOCAL-FIRST FORK: HelpScreen's register / forgot-password links route into
    // the local shell (show_login → enter_local_shell) — the dedicated Register
    // and Forgot-Password screens were removed.
    connect(help, &screens::HelpScreen::navigate_register, this, &WindowFrame::show_login);
    connect(help, &screens::HelpScreen::navigate_forgot_password, this, &WindowFrame::show_login);
}
...
void WindowFrame::setup_dock_screens() {
    ...
    dock_router_->register_factory("crypto_trading", [this]() {
        ...
        // screen, which carries the Kalshi connect/trade flow.
