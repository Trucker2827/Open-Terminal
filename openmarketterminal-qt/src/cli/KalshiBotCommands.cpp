// `kalshi bot` command family — the PAPER Kalshi bot, ladder rungs 1–2.
//
// One tick (`once`/`run`) is: settle whatever the exchange has actually
// resolved, then read calibrator.json and journal one decision row for every
// contract it offers. Every row lands in kalshi-bot-decisions.jsonl, passes
// included: a tick that bids nothing still has to say why.
//
...
using services::prediction::kalshi_ns::KalshiBotGate;
...
int kalshi_bot_command(const GlobalOpts& opts, QStringList args) {
    ...
    if (opts.help || sub.isEmpty() ||
        (sub != QStringLiteral("once") && sub != QStringLiteral("run") &&
         sub != QStringLiteral("gate"))) {
        bot_usage();
        return opts.help ? 0 : 2;
    }

    // The gate reads a ledger and sealed criteria; none of the tick's mode or
    // cap flags apply to it, so it branches before they are parsed.
