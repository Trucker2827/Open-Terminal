...
void PositionManager::record_entry_legs(const QVector<openmarketterminal::algo::AlgoLegPosition>& legs,
                                        int64_t time_ms) {
    QMutexLocker lock(&mutex_);
...
QVector<openmarketterminal::algo::AlgoLegPosition> PositionManager::legs() const {
    QMutexLocker lock(&mutex_);
    return legs_;
}
