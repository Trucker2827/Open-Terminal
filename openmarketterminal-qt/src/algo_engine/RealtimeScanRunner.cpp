...
void RealtimeScanRunner::start() {
    ...
    // `this` is the owner.
    auto& dsm = openmarketterminal::trading::DataStreamManager::instance();
    for (const QString& sym : universe_) {
        dsm.open_quote_feed(this, consumer_id(sym), account_id_, sym,
                            [this, sym](const openmarketterminal::trading::BrokerQuote& q) { on_quote(sym, q); });
    }
...
void RealtimeScanRunner::on_quote(const QString& symbol, const openmarketterminal::trading::BrokerQuote& q) {
    auto it = states_.find(symbol);
    if (it == states_.end())
        return;
    SymbolState* st = it.value();
