...
void DeploymentRunner::start() {
    ...
    // SAFETY GATE: a LIVE autonomous deployment must have BOTH risk limits set —
    // a per-order value cap AND a daily-loss kill switch. Without them a strategy
    // that triggers every tick can place unbounded live orders and drain the
    // account (PositionManager treats 0 as "no limit"). Refuse to start until the
    // user sets explicit limits. Paper mode is unrestricted.
    if (deployment_.is_live() &&
        (deployment_.max_order_value <= 0.0 || deployment_.max_daily_loss <= 0.0)) {
        const QString msg = QStringLiteral(
            "Live deployment blocked for safety: set both a max order value and a "
            "max daily loss before running a strategy with real money "
            "(both are currently unset).");
        LOG_ERROR("AlgoEngine", QString("Deployment %1 refused — %2").arg(deployment_.id, msg));
        emit error_occurred(deployment_.id, msg);
        emit status_changed(deployment_.id, QStringLiteral("error"));
        return;  // running_ stays false — nothing is scheduled, no orders can fire
    }
...
void DeploymentRunner::on_tick_data(const QVariant& data) {
    ...
    if (data.canConvert<openmarketterminal::trading::BrokerQuote>()) {
        auto quote = data.value<openmarketterminal::trading::BrokerQuote>();
        price = quote.ltp;
        volume = quote.volume;
...
bool DeploymentRunner::in_position() const {
    return position_mgr_->has_position();
}
