...
using openmarketterminal::ui::ToastService;
using openmarketterminal::notifications::NotificationService;
using openmarketterminal::notifications::NotificationRequest;
using openmarketterminal::notifications::NotifLevel;
using openmarketterminal::notifications::NotifTrigger;
...
void ScanMonitor::start_realtime(const ScanWatch& w) {
    ...
    const int lookback = w.lookback_days > 0
                             ? w.lookback_days
                             : openmarketterminal::services::algo::algo_default_lookback_days(tf);
    const DataSource src = data_source_from_string(w.data_source);
...
void ScanMonitor::dispatch(const ScanWatch& w, const QString& symbol, const QString& detail) {
    ...
    openmarketterminal::ScanEventRepository::instance().record(w.id, symbol, detail,
                                                    QDateTime::currentMSecsSinceEpoch());
