...
// — encoding pins, OpenMarketTerminal_DATA_DIR, FINAGENT_DATA_DIR, base PYTHONPATH — lives
...
QProcessEnvironment PythonRunner::build_python_env() const {
    ...
    env.insert("OpenMarketTerminal_DATA_DIR", install_dir);

    // FINAGENT_DATA_DIR: default to <OpenMarketTerminal_DATA_DIR>/finagent. Always overwrite
...
QString PythonRunner::find_scripts_dir() const {
    ...
    // Also check project root (for dev: openmarketterminal-qt/scripts)
    QDir dir(exe_dir);
    while (dir.cdUp()) {
...
void PythonRunner::run_code(const QString& code, Callback cb) {
    ...
    QString temp_path = temp_dir + "/openmarketterminal_cell_" + QString::number(QDateTime::currentMSecsSinceEpoch()) + "_" +
...
void PythonRunner::start_next() {
    ...
    while (active_count_ < max_concurrent_ && !queue_.isEmpty()) {
        ...
        if (!is_code) {
            ...
            for (const QString& arg : req.args) {
                if (arg.size() > kArgSpillThreshold) {
                    QString temp_dir = QStandardPaths::writableLocation(QStandardPaths::TempLocation);
                    QString temp_path = temp_dir + "/openmarketterminal_arg_" +
        ...
        // Start from the shared base env (encoding pins, OpenMarketTerminal_DATA_DIR,
