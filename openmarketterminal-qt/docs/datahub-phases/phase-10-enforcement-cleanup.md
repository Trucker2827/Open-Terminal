...
- **D2.** Services that fetch external data (HTTP, Python, WS)
  must implement `openmarketterminal::datahub::Producer` and publish results
  via `DataHub::publish(...)`.
...
match inside `openmarketterminal-qt/src/screens/`:

```bash
# Screens must not spawn Python directly.
rg -t cpp 'PythonRunner::instance\(\).run\(' openmarketterminal-qt/src/screens/ && exit 1

# Screens must not call the deprecated fetch_* APIs.
rg -t cpp '(MarketDataService|NewsService|EconomicsService)::instance\(\)\.fetch_' \
   openmarketterminal-qt/src/screens/ && exit 1
...
Remove the `#ifdef OPENMARKETTERMINAL_DATAHUB_ENABLED` guard from the
inspector's inclusion in `SCREEN_SOURCES`; the feature flag no
longer gates anything (hub is now the only data path).

### 6. Flag removal

Remove `OPENMARKETTERMINAL_DATAHUB_ENABLED` CMake option. The hub is no
longer optional. Delete the flag from `CMakeLists.txt:123`, all
its `#ifdef OPENMARKETTERMINAL_DATAHUB_ENABLED` blocks (roughly: `main.cpp`
...
- `OPENMARKETTERMINAL_DATAHUB_ENABLED` no longer appears anywhere in
  `git grep`.
