...
- Season-1 seeds (exact `params` objects, all with `"notional_usd": 50.0`):
  ...
  - `kalshi` / `BTC-USD,ETH-USD,SOL-USD` / `{"notional_usd":50.0,"source":"edge_journal","journal_source":"edge journal-kalshi-scan","prediction":true}` (must exactly match the `edge journal-kalshi-scan` producer's `source` string — see `CommandDispatch.cpp` ~19481 — or the book opens nothing, ever; task-11-follow-up fix)
