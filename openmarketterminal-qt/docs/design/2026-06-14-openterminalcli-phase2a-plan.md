...
- [ ] **Step 4: Attach enforcement** — ~~`TerminalMcpBridge.cpp`: include the `destructive_token` in `bridge.json` only when `cli_trading_allowed()`, re-writing on settings change.~~ **SUPERSEDED by the Task 6 security fix:** the `destructive_token` is NOT written to `bridge.json` at all. With no CLI consumer in 2a it was an unconsumed, non-revocable replay secret (a process that read it while the toggle was on kept access after revocation). Attach-mode CLI is read + non-destructive; CLI destructive/trading is deferred to a future phase. Settings-write tools remain gated server-side in `McpProvider`/SettingsTools on `cli_settings_write_allowed()`, and headless trading via `cli.allow_trading` + the `HeadlessRuntime` auth-checker is the real, working trading gate.
