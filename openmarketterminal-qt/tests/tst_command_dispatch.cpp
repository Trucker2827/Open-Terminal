...
private slots:
    ...
    // does both.
    // Proof protocol v2 plus the producer-backed honest lane grid: 29 rows,
    // spot 1h/4h/1d, 9 Kalshi horizon/cohort books (managed exit only; the
    // `settlement` control cohort was retired after the A/B), 1 kalshi_weather
    // lane, long_short,
    ...
    void sandbox_seed_list_pause_resume_retire() {
        ...
        QCOMPARE(rc, 0);
        QCOMPARE(active_after_pause.value("strategies").toArray().size(), 28);
        ...
        QJsonObject active_after_resume = json_object_from_dispatch(
            QStringList{"--json", "sandbox", "list", "--status", "active"}, &rc);
        QCOMPARE(rc, 0);
        QCOMPARE(active_after_resume.value("strategies").toArray().size(), 29);
        ...
        QJsonObject active_after_retire = json_object_from_dispatch(
            QStringList{"--json", "sandbox", "list", "--status", "active"}, &rc);
        QCOMPARE(rc, 0);
        QCOMPARE(active_after_retire.value("strategies").toArray().size(), 28);
