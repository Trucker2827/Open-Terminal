"""Strip OpenMarketTerminal_DATAHUB_ENABLED preprocessor guards.
...
flag is being removed. This tool rewrites source files to:
  ...
  - `#ifdef OpenMarketTerminal_DATAHUB_ENABLED` ... `#endif`               -> keep body, drop guard lines
  - `#ifdef OpenMarketTerminal_DATAHUB_ENABLED` ... `#else` ... `#endif`   -> keep "if" branch, drop else branch + guard lines
  - `#ifndef OpenMarketTerminal_DATAHUB_ENABLED` ... `#endif`              -> drop whole block (legacy-only)
  - `#ifndef OpenMarketTerminal_DATAHUB_ENABLED` ... `#else` ... `#endif`  -> keep "else" branch, drop legacy + guard lines

Handles nested `#if` / `#ifdef` / `#ifndef` / `#endif` for unrelated macros by
tracking depth only when inside a matching OpenMarketTerminal block. Preserves indentation
(only strips the preprocessor directive lines themselves).

Does NOT handle `#if defined(OpenMarketTerminal_DATAHUB_ENABLED)` / `#elif` variants; the
...
MACRO = "OpenMarketTerminal_DATAHUB_ENABLED"
...
def _strip_file(text: str, filename: str) -> str:
    ...
    Walk line-by-line. When we hit a matching `#ifdef OpenMarketTerminal` / `#ifndef OpenMarketTerminal`,
    ...
    while i < n:
        ...
        while i < n:
            ...
            if depth == 0 and RE_ELSE.match(cur):
                if in_else:
                    raise StripError(
                        f"{filename}:{i+1}: duplicate #else inside OpenMarketTerminal guard at line {guard_start+1}"
                    )
                in_else = True
                i += 1
                continue
            if RE_ENDIF.match(cur):
                if depth == 0:
                    # End of our OpenMarketTerminal guard.
                    i += 1
                    break
                depth -= 1
                (else_branch if in_else else if_branch).append(cur)
            ...
            raise StripError(
                f"{filename}:{guard_start+1}: unterminated OpenMarketTerminal guard"
            )
