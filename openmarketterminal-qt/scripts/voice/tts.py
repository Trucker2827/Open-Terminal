...
Environment variables (optional, all set by TtsService):
    OpenMarketTerminal_TTS_RATE     — words/min override (default = engine default).
    OpenMarketTerminal_TTS_VOLUME   — 0.0…1.0 (default 1.0).
    OpenMarketTerminal_TTS_VOICE    — substring match against voice id/name; first hit wins.
"""
...
def main() -> int:
    ...
    rate_env = os.environ.get("OpenMarketTerminal_TTS_RATE", "").strip()
    if rate_env:
        try:
    ...
    vol_env = os.environ.get("OpenMarketTerminal_TTS_VOLUME", "").strip()
    if vol_env:
        try:
    ...
    voice_pref = os.environ.get("OpenMarketTerminal_TTS_VOICE", "").strip().lower()
    if voice_pref:
        try:
