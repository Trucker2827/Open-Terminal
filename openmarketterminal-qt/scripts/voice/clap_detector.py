...
Environment variables (set by ClapDetectorService in C++):
    OpenMarketTerminal_CLAP_MODE         "double" (default) | "single"
    OpenMarketTerminal_CLAP_PEAK_MIN     int 0..32768  — min peak amplitude to count as a
                              candidate. Default 12000 (~ -8 dBFS). Lower = more
                              sensitive, more false positives.
    OpenMarketTerminal_CLAP_PR_RATIO     float — min peak/rms ratio. Claps are sharp; this
                              filters out steady loud noise like music or a fan.
                              Default 4.0.
    OpenMarketTerminal_CLAP_MAX_GAP_MS   int — max ms between two candidates to count as a
                              double-clap. Default 1500.
    OpenMarketTerminal_CLAP_DEBOUNCE_MS  int — silence window after a fired event. Default 1500.
    OpenMarketTerminal_STT_DEVICE        same key as STT — input device name substring.
"""
...
def main() -> None:
    ...
    mode = os.environ.get("OpenMarketTerminal_CLAP_MODE", "double").strip().lower() or "double"
    if mode not in ("single", "double"):
        mode = "double"

    try:
        peak_min = int(os.environ.get("OpenMarketTerminal_CLAP_PEAK_MIN", "12000"))
    except ValueError:
        peak_min = 12000
    try:
        pr_ratio = float(os.environ.get("OpenMarketTerminal_CLAP_PR_RATIO", "4.0"))
    except ValueError:
        pr_ratio = 4.0
    try:
        max_gap_ms = int(os.environ.get("OpenMarketTerminal_CLAP_MAX_GAP_MS", "1500"))
    except ValueError:
        max_gap_ms = 1500
    try:
        debounce_ms = int(os.environ.get("OpenMarketTerminal_CLAP_DEBOUNCE_MS", "1500"))
    except ValueError:
        debounce_ms = 1500
    device_pref = os.environ.get("OpenMarketTerminal_STT_DEVICE", "").strip()
