...
Environment variables (set by DeepgramSttProvider in C++):
    ...
    OpenMarketTerminal_STT_MODEL    default "nova-3"
    OpenMarketTerminal_STT_LANGUAGE default "en"
    OpenMarketTerminal_STT_KEYTERMS comma-separated boosted terms (optional)
    OpenMarketTerminal_STT_GAIN     audio gain multiplier (default "3.0").  Try 5-10 if
                         the mic is far-field; leave at 1.0 for a headset.
    OpenMarketTerminal_STT_DEVICE   case-insensitive substring of input-device name.  If
    ...
    if pref:
        ...
        for d in inputs:
            if pref_l in d["name"].lower():
                diag(f"selected input by OpenMarketTerminal_STT_DEVICE='{pref}': "
                     f"#{d['index']} {d['name']}")
                return d["index"]
        diag(f"no device matched OpenMarketTerminal_STT_DEVICE='{pref}', "
...
def main() -> None:
    ...
    model       = os.environ.get("OpenMarketTerminal_STT_MODEL", "nova-3").strip() or "nova-3"
    language    = os.environ.get("OpenMarketTerminal_STT_LANGUAGE", "en").strip() or "en"
    keyterms_raw = os.environ.get("OpenMarketTerminal_STT_KEYTERMS", "").strip()
    keyterms    = [t.strip() for t in keyterms_raw.split(",") if t.strip()]
    device_pref = os.environ.get("OpenMarketTerminal_STT_DEVICE", "").strip()
    try:
        gain = float(os.environ.get("OpenMarketTerminal_STT_GAIN", "3.0"))
    except ValueError:
        gain = 3.0
