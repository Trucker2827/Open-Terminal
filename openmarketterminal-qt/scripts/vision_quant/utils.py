...
def get_data_dir():
    ...
    openmarketterminal_dir = os.environ.get("OpenMarketTerminal_DATA_DIR", "")
    if openmarketterminal_dir:
        return os.path.join(openmarketterminal_dir, "vision_quant")
    # Fallback: ~/.openmarketterminal/vision_quant
    home = os.path.expanduser("~")
    return os.path.join(home, ".openmarketterminal", "vision_quant")
...
def json_response(status, data=None, error=None):
    """Build a standard JSON response for the OpenMarketTerminal protocol."""
    resp = {"status": status}
    if data is not None:
        resp["data"] = data
...
def output_json(obj):
    """Print JSON to stdout (OpenMarketTerminal protocol)."""
...
def parse_args():
    """Parse command + JSON params from sys.argv (OpenMarketTerminal protocol)."""
