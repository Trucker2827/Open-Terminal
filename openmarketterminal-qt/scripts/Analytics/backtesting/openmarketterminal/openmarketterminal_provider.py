#!/usr/bin/env python3
"""
OpenMarketTerminal Engine Backtesting Provider
Executes strategies from the 418+ OpenMarketTerminal strategy registry.
...
from a log line like "[OpenMarketTerminal Engine] Executing...".
"""
...
def _log(msg: str) -> None:
    ...
    print(f"[OpenMarketTerminal Engine] {msg}", file=sys.stderr)
...
class OpenMarketTerminalProvider:
    """
    OpenMarketTerminal Engine backtesting provider.
    Bridges OpenMarketTerminal frontend to 418+ QCAlgorithm strategies.
    """

    def __init__(self):
        self.runner = OpenMarketTerminalStrategyRunner()
    ...
    def get_strategies(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for s in flat:
            ...
            grouped.setdefault(cat, []).append({
                'id':     s['id'],
                'name':   s['name'],
                'params': [],  # OpenMarketTerminal strategies use free-form strategy_params dict
            })
        return {'success': True, 'data': {'provider': 'openmarketterminal', 'strategies': grouped}}

    def get_indicators(self, request: Dict[str, Any]) -> Dict[str, Any]:
        """OpenMarketTerminal strategies are self-contained — no separate indicator catalog."""
        return {'indicators': {}}
    ...
    def _execute_one(
        ...
        return {
            ...
            'data': {
                ...
                'logs': [
                    f"OpenMarketTerminal Engine backtest of {strategy_info['name']} ({strategy_id})",
                    f"Symbols: {symbols}",
    ...
    def run_backtest(self, request: Dict[str, Any]) -> Dict[str, Any]:
        """Execute a single OpenMarketTerminal Engine strategy backtest."""
        try:
            ...
            print(f'[OpenMarketTerminal Engine] Error: {e}', file=sys.stderr)
    ...
    def optimize(self, request: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            import traceback
            tb = traceback.format_exc()
            print(f'[OpenMarketTerminal Engine] Optimize error: {e}', file=sys.stderr)
            print(tb, file=sys.stderr)
            return {'success': False, 'error': str(e), 'traceback': tb}
    ...
    def walk_forward(self, request: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            for i in range(n_splits):
                ...
                # OpenMarketTerminal strategies are not parameterised per fold (no per-fold
            ...
            import traceback
            tb = traceback.format_exc()
            print(f'[OpenMarketTerminal Engine] Walk-forward error: {e}', file=sys.stderr)
            print(tb, file=sys.stderr)
            return {'success': False, 'error': str(e), 'traceback': tb}
...
def main():
    ...
    if len(sys.argv) < 3:
        print(json_response({
            'success': False,
            'error': 'Usage: openmarketterminal_provider.py <command> <args_json>',
        }))
        sys.exit(1)
    ...
    provider = OpenMarketTerminalProvider()

    if command in ('run_backtest', 'execute_openmarketterminal_strategy'):
