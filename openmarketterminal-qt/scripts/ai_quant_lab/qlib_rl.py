...
class RLTradingAgent:
    ...
    def create_trading_env(self,
        ...
        try:
            ...
            if demo:
                # demo=True: clearly-labeled SYNTHETIC random-walk data (NOT real market data).
                ...
                result_meta = {
                    'demo': True,
                    'message': 'Trading environment created (SYNTHETIC demo data)',
                }
            else:
                # default: REAL qlib market data. load_ohlcv returns exactly the columns
                # TradingEnvironment expects (close/open/high/low/volume/returns/
                # volatility/rsi/macd/signal). NEVER fall back to synthetic silently.
                symbol = str(instruments[0]).strip().upper() if instruments else ''
                market_data = load_ohlcv(symbol) if symbol else None
                if market_data is None:
                    return {
                        'success': False,
                        'error_kind': 'no_input',
                        'error': (f"No qlib market data available for ticker '{symbol or '?'}'. "
...
def main():
    ...
    if command == 'list_algorithms':
        ...
        env_params = {k: params.pop(k) for k in ['tickers', 'initial_cash', 'start_date', 'end_date', 'demo']
                      if k in params}
        ...
        env_params.setdefault('demo', False)
        ...
        if not env_result.get('success'):
            ...
            if isinstance(result, dict):
                result['demo'] = env_params.get('demo', False)
                # Propagate provenance from the env (train_agent's own result has none).
                if env_result.get('source'):
                    result['source'] = env_result['source']
                if env_result.get('data_points') is not None:
                    result['data_points'] = env_result.get('data_points')
                if env_result.get('ticker'):
                    result['ticker'] = env_result['ticker']
