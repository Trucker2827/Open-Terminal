...
class MetaLearningManager:
    ...
    """
    CACHE_DIR = Path.home() / '.openmarketterminal' / 'meta_learning'
    ...
    def run_model_selection(self,
        ...
        if X_train is None and not demo:
            # default: REAL qlib feature matrix, split 80/20. NEVER fabricate silently.
            mat = load_matrix(ticker, classification=(task_type == 'classification'))
            if mat is None:
                return {
                    'success': False,
                    'error_kind': 'no_input',
            ...
            real_used = True
        # demo / no real data: clearly-labeled SYNTHETIC data (NOT real features).
    ...
    def hyperparameter_tuning(self,
        ...
        real_used = False
        if X_train is None and not demo:
            # default: REAL qlib feature matrix. NEVER fabricate silently.
            mat = load_matrix(ticker, classification=(task_type == 'classification'))
            if mat is None:
                return {
                    'success': False,
...
def main():
    ...
    try:
        if command == 'list_models':
            ...
            demo = bool(params.get('demo', False))
            ticker = params.get('ticker', 'AAPL')
            result = manager.hyperparameter_tuning(model_id, param_grid, task_type=task_type,
                                                   search_method=search_method, demo=demo, ticker=ticker)
