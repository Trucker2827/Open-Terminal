...
class FeatureExpressionEngine:
    ...
    def evaluate(self, expression: str) -> pd.Series:
        ...
        """
        # Honesty: there is no real expression parser. Refuse rather than silently
        # returning `close` for every input. (A real AST evaluator is future work.)
        raise NotImplementedError(
            "The feature expression engine is not implemented yet. It cannot evaluate "
            f"'{expression}'. (It previously returned `close` for any input — removed as deceptive.)")
...
def main():
    ...
    try:
        ...
        kind = "not_implemented" if isinstance(e, NotImplementedError) else "runtime"
        print(json.dumps({"success": False, "error_kind": kind, "error": str(e)}))
        sys.exit(1)
