"""
config.py — LiteLLM / rdagent LLM config wiring for OpenMarketTerminal rdagents.

rdagent uses environment variables for its LLM backend (LiteLLMAPIBackend).
This module translates the OpenMarketTerminal LLM config dict into the correct env vars
...
def apply_llm_config(config: dict[str, Any]) -> dict[str, str]:
    """
    Translate OpenMarketTerminal LLM config dict into rdagent env vars.
