"""
cli.py — Entry point for OpenMarketTerminal rdagents integration.
...
def cmd_start_mcp_server(params: dict[str, Any]) -> dict[str, Any]:
    """Task 20 — Start the OpenMarketTerminal MCP tool server for RD-Agent loop use."""
    try:
    ...
    return {
        ...
        "tools":    avail["tools"],
        "message":  "OpenMarketTerminal MCP server started" if toolset else "MCP server failed to start",
        "already_running": proc is None and toolset is not None,
    }


def cmd_stop_mcp_server(params: dict[str, Any]) -> dict[str, Any]:
    """Stop the OpenMarketTerminal MCP server on the given port."""
    try:
