"""
mcp_tools.py — Helpers for wiring the OpenMarketTerminal MCP server into RD-Agent loops.
...
DEFAULT_PORT = int(os.environ.get("OpenMarketTerminal_MCP_PORT", "18765"))
...
def get_mcp_toolset(host: str = "127.0.0.1", port: int = DEFAULT_PORT) -> Any:
    """
    Return an MCPServerStreamableHTTP instance pointing at the OpenMarketTerminal MCP server.

    The server must already be running (use start_mcp_server_process() to launch it).

    Args:
        host: Server host (default 127.0.0.1)
        port: Server port (default 18765 or OpenMarketTerminal_MCP_PORT env var)
...
def start_mcp_server_process(
    ...
    """
    Start the OpenMarketTerminal MCP server as a background subprocess.
    ...
    logger.info("Starting OpenMarketTerminal MCP server on %s:%d", host, port)
    try:
