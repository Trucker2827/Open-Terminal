"""
orchestrator.py — Fallback for non-tool-calling LLMs (OpenMarketTerminal API, Ollama).

Single responsibility:
  - Execute multi-step agent workflows via prompt-loop when the configured LLM
    does not support LangChain tool calling (e.g. OpenMarketTerminal hosted LLM, Ollama)
...
# OpenMarketTerminal hosted LLM endpoint
_OpenMarketTerminal_LLM_URL   = "https://api.example.com/research/llm"
_OpenMarketTerminal_LLM_ASYNC = "https://api.example.com/research/llm/async"
...
class OpenMarketTerminalOrchestrator:
    """
    ...
    def _call_llm(self, prompt: str, max_tokens: int = 1000) -> str:
        """Call the OpenMarketTerminal LLM endpoint with retry on async failure."""
        payload = {
            "prompt":     prompt,
