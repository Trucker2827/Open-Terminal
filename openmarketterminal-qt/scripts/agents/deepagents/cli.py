"""
cli.py — C++ entry point for OpenMarketTerminal Deep Agents.
...
def cmd_stream_task(params: dict[str, Any]) -> None:
    ...
    try:
        from agent import create_agent, build_openmarketterminal_context
        from models import create_model

        model    = create_model(config)
        compiled = create_agent(model=model, config={**config, "agent_type": agent_type})
        ctx      = build_openmarketterminal_context({**params, "agent_type": agent_type})
...
def cmd_create_plan(params: dict[str, Any]) -> dict[str, Any]:
    ...
    from orchestrator import OpenMarketTerminalOrchestrator
    subagent_names = [s["name"] for s in get_subagents_for_type(agent_type)]
    orch = OpenMarketTerminalOrchestrator(api_key=config.get("llm_api_key"))
...
def cmd_resume_thread(params: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        ctx      = build_openmarketterminal_context(params)
...
def _execute_with_deepagents(
    ...
    try:
        ...
        ctx = build_openmarketterminal_context({"agent_type": agent_type, "context": context_data})
...
def _execute_with_orchestrator(
    ...
    """Execute via OpenMarketTerminalOrchestrator (non-tool-calling LLM fallback)."""
    from orchestrator import OpenMarketTerminalOrchestrator
    subagent_names = [s["name"] for s in get_subagents_for_type(agent_type)]
    orch = OpenMarketTerminalOrchestrator(api_key=config.get("llm_api_key"))
