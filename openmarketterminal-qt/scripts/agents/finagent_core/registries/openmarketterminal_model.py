"""
OpenMarketTerminal LLM Model - Custom adapter for OpenMarketTerminal's built-in LLM endpoint.
...
# ── Cross-process rate limiter: max 1 request/second to OpenMarketTerminal endpoint ──────
...
_RATE_LOCK_FILE = os.path.join(tempfile.gettempdir(), "openmarketterminal_llm_rate.lock")
_RATE_TS_FILE   = os.path.join(tempfile.gettempdir(), "openmarketterminal_llm_rate.ts")
_openmarketterminal_proc_lock = threading.Lock()  # within-process serialisation

def _rate_limit_openmarketterminal() -> None:
    """
    ...
    """
    with _openmarketterminal_proc_lock:
...
@dataclass
class OpenMarketTerminalChat(Model):
    """
    Agno-compatible model for OpenMarketTerminal's /research/llm endpoint.
    ...
    id: str = "openmarketterminal-llm"
    name: str = "OpenMarketTerminalChat"
    provider: str = "OpenMarket"

    api_key: Optional[str] = None
    base_url: str = OpenMarketTerminal_DEFAULT_URL
    temperature: Optional[float] = None
    max_tokens: Optional[int] = None

    # Max tool call rounds before forcing a final answer
    # Keep low for OpenMarketTerminal endpoint — each round = 1 API call, gateway times out after ~60s
            ...
            try:
                with httpx.Client(timeout=250.0) as client:
                    ...
                    if resp.status_code == 429:
                        ...
                        logger.warning(f"OpenMarketTerminalChat: 429 from {url} attempt {attempt + 1}, waiting {wait}s")
                        time.sleep(wait)
                        last_error = RuntimeError(f"OpenMarketTerminal API error (429): rate limited")
                        continue

                    if resp.status_code in (502, 504):
                        wait = 2.0 * (attempt + 1)  # 2s, 4s, 6s
                        logger.warning(f"OpenMarketTerminalChat: {resp.status_code} from {url} attempt {attempt + 1}, waiting {wait}s")
                        time.sleep(wait)
                        last_error = RuntimeError(f"OpenMarketTerminal API error ({resp.status_code}): gateway error")
                        continue
                    ...
                    if resp.status_code != 200:
                        ...
                        raise RuntimeError(f"OpenMarketTerminal API error ({resp.status_code}): {error_detail}")
                ...
                last_error = RuntimeError(f"OpenMarketTerminal API request timed out (250s) for {url}")
                logger.warning(f"OpenMarketTerminalChat: timeout from {url} attempt {attempt + 1}")
                continue
            except httpx.ConnectError as e:
                last_error = RuntimeError(f"Cannot connect to {url}: {e}")
                logger.warning(f"OpenMarketTerminalChat: connect error for {url}: {e}")
                break  # Connection refused — no point retrying same URL
            except RuntimeError:
                raise
            except Exception as e:
                raise RuntimeError(f"OpenMarketTerminal API call failed: {e}")

        raise last_error or RuntimeError(f"OpenMarketTerminal API failed after 3 attempts on {url}")
    ...
    def _call_endpoint(
        ...
        OpenMarketTerminal_DEFAULT_URL and it fails with a gateway error, automatically
        ...
        """
        _rate_limit_openmarketterminal()
        ...
        try:
            ...
            if self.base_url == OpenMarketTerminal_DEFAULT_URL:
                raise

            # Custom/tunnel URL failed — fall back to the real OpenMarketTerminal API
            logger.warning(
                f"OpenMarketTerminalChat: custom base_url '{self.base_url}' failed ({primary_err}). "
                f"Falling back to {OpenMarketTerminal_DEFAULT_URL}"
            )
            _rate_limit_openmarketterminal()  # honour rate limit before fallback attempt
            return self._try_url(OpenMarketTerminal_DEFAULT_URL, payload, headers)
    ...
    def _execute_tool_call(
        ...
        logger.info(f"OpenMarketTerminalChat: calling {tool_name}({args})")
        try:
    ...
    # OpenMarketTerminal endpoint hard limit is ~50k chars, but gateway times out on large prompts.
    ...
    def _run_tool_loop(
        ...
        for round_num in range(self.max_tool_rounds):
            logger.debug(f"OpenMarketTerminalChat: tool round {round_num + 1}/{self.max_tool_rounds}")
            ...
            logger.debug(
                f"OpenMarketTerminalChat: response={response_text[:100]!r}, "
        ...
        logger.warning(
            f"OpenMarketTerminalChat: hit tool round limit ({self.max_tool_rounds}), "
    ...
    def response(
        ...
        **kwargs,
        ...
        """
        Override base response() to use OpenMarketTerminal's native function calling.
        ...
        if _functions:
            logger.info(
                f"OpenMarketTerminalChat: native tool calling with "
    ...
    def _extract_content(self, data: Any) -> str:
        ...
        if isinstance(data, dict):
            # OpenMarketTerminal: {"success": true, "data": {"response": "..."}}
