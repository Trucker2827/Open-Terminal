...
def base_dir() -> str:
    ...
    <LOCALAPPDATA>/org.openmarketterminal.OpenMarketTerminal/finagent on Windows, or
    $XDG_DATA_HOME/org.openmarketterminal.OpenMarketTerminal/finagent (~/.local/share fallback)
    elsewhere.
    """
    ...
    if os.name == "nt":
        ...
        return str(Path(root) / "org.openmarketterminal.OpenMarketTerminal" / "finagent")
    xdg = os.getenv("XDG_DATA_HOME") or str(Path.home() / ".local" / "share")
    return str(Path(xdg) / "org.openmarketterminal.OpenMarketTerminal" / "finagent")
