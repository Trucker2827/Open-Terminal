"""
Cross-Platform Path Utilities for OpenMarketTerminal
...
- Windows: %APPDATA%\openmarketterminal\
- macOS: ~/Library/Application Support/openmarketterminal/
- Linux: ~/.local/share/openmarketterminal/
"""
...
def get_app_data_dir() -> Path:
    """
    Get the OpenMarketTerminal application data directory.
    ...
    Returns:
        Path to openmarketterminal app data directory
    """
    ...
    app_dir = base / "openmarketterminal"
...
def get_rentech_data_dir() -> Path:
    ...
    Returns:
        ...
        (e.g., %APPDATA%\openmarketterminal\rentech\)
    """
...
def get_knowledge_base_path() -> Path:
    ...
    Returns:
        ...
        (e.g., %APPDATA%\openmarketterminal\rentech\knowledge\)
    """
...
def get_memory_db_path() -> Path:
    ...
    Returns:
        ...
        (e.g., %APPDATA%\openmarketterminal\rentech\memory.db)
    """
...
def get_logs_dir() -> Path:
    ...
    Returns:
        ...
        (e.g., %APPDATA%\openmarketterminal\rentech\logs\)
    """
...
def get_cache_dir() -> Path:
    ...
    Returns:
        ...
        (e.g., %APPDATA%\openmarketterminal\rentech\cache\)
    """
...
def get_frontend_db_path() -> Optional[Path]:
    """
    Get the path to frontend SQLite database (openmarketterminal.db).

    Searches common locations for the frontend database.

    Returns:
        Path to openmarketterminal.db if found, None otherwise
    """
    # Check environment variable first
    env_path = os.getenv("OpenMarketTerminal_DB_PATH")
    if env_path:
        path = Path(env_path)
        if path.exists():
            return path
    ...
    db_path = app_data / "openmarketterminal.db"
    if db_path.exists():
        return db_path

    # Alternative locations
    alternatives = [
        app_data.parent / "openmarketterminal.db",  # One level up
        Path.home() / ".openmarketterminal" / "openmarketterminal.db",
        Path("./openmarketterminal.db"),  # Current directory (dev mode)
    ]
    ...
    logger.warning("Frontend database (openmarketterminal.db) not found")
    return None
...
def print_paths():
    ...
    print("=== OpenMarketTerminal - RenTech Agent Paths ===")
